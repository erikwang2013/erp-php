...
- 销售趋势、客户排行 Top10
- 支持时间范围切换
- 30 日销售趋势（含 0 值补全）、Top5 热销商品（近 30 日订购数量）、订单状态分布（待审核/已审核/部分发货/已发货/已取消）
...
- 应收/应付总额、本月收付款
- 现金银行余额汇总
- 应收/应付账龄汇总（未到期/逾期 1-30/31-60/61-90/90+ 天分桶，按未核销余额）
...
| 单元测试 | 107 个测试文件 <!-- stats:test_files=109 --> / 940 个测试用例 <!-- stats:tests=968 --> / 4575 条断言 <!-- stats:assertions=4619 -->（静态计数：测试方法数 + 断言调用点数，与运行环境无关）|
...
| 后端 API | 39 | 4 | 1 | 0 | 89% |
| 业务逻辑 | 33 | 7 | 4 | 0 | 75% |
| Flutter 前端 | 12 | 12 | 18 | 2 | 29% |
| HarmonyOS | 0 | 13 | 29 | 2 | 0%（✅ 计；13 行已有页面 ⚠️）|
...
> **v1.17.0 批次（2026-09-15）**：新增「多语言 (i18n)」行（44 行中 1 行标注 v1.17.0）——后端 13 语种词典
...
> **v1.4.0 批次（2026-09-05）**：44 行中 21 行标注 v1.4.0（含 1 行「部分启用」= 多租户 (B5) 行），
