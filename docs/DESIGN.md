...
| 表名 | 字段数 | 说明 |
|------|-------|------|
| `erp_admin_user` | 14 | 管理用户，phone/email/id_card 加密存储，支持软删除 |
| `erp_admin_role` | 7 | 角色，slug 唯一 |
| `erp_admin_permission` | 10 | 权限树（parent_id 自引用），type: 1=菜单 2=按钮 3=API |
| `erp_admin_user_role` | 2 | 用户-角色多对多中间表 |
| `erp_admin_role_permission` | 2 | 角色-权限多对多中间表 |
| `erp_system_config` | 8 | 键值对配置，group+key 联合唯一 |
| `erp_operation_log` | 9 | 操作审计日志（含 source 来源端） |
...
| `app` | 本地 `Dockerfile` 构建 | 8788 | PHP 8.3 + OPcache + webman |
