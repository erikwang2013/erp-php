...
| Baseline kualitas | PHPUnit 1046<!-- stats:tests=1046 --> test / 5029<!-- stats:assertions=5029 --> assertion, PHPStan + baseline, CS-Fixer, matriks multi-versi CI | Disiplin sudah ada, peran pengujian/review langsung tertanam ke pipeline |
...
|------|-----------------|--------------------|
| **Project Manager Lead** | `planner` / `swarm/hierarchical-coordinator` | Pemecahan kebutuhan → routing → penerimaan; memelihara antrean tugas 23 modul; memutuskan mode pipeline / fan-out / supervisor; perantara pesan lintas peran |
| **Arsitek Sistem** | `sparc/architecture` | Desain struktur tabel (227 tabel, schema dengan database/install.sql sebagai satu-satunya sumber kebenaran); alur data lintas modul (penerimaan pembelian→stok→hutang, pengiriman penjualan→piutang→keluar gudang, dll.); keputusan batas pemecahan mikroservice |
| **Developer Backend** | `core` / kustom `backend-dev` | Implementasi controller / service / model; mengikuti lapisan `app/service` dan rantai middleware (Cors→SecurityFilter→RateLimit→TracingId→middleware bisnis) |
| **Insinyur Pengujian** | `testing/tdd-london-swarm` + `production-validator` | Kasus PHPUnit lebih dulu (pengujian batas mesin); verifikasi regresi jalur tunggal `main`; melengkapi celah cakupan `tests/` |
| **Reviewer Kode** | `consensus/security-manager` | PHPStan tanpa penambahan baseline, kepatuhan CS-Fixer, pemeriksaan pola keamanan 7 lapis; menjaga gerbang kualitas sebelum commit |
...
| **Insinyur Frontend (Flutter)** | kustom `frontend-flutter` | Perubahan apa pun yang melibatkan `apps/flutter/` | Halaman panel admin Web, status GetX, kaitan ApiService/ekspor, pemeliharaan 102 rute |
| **Insinyur Frontend (HarmonyOS)** | kustom `frontend-harmonyos` | Perubahan apa pun yang melibatkan `apps/harmonyos/` | Halaman ArkTS, refresh token tanpa rasa sakit, penyelarasan kumpulan fitur dengan Flutter (pemeliharaan 41 halaman) |
| **Insinyur Keamanan/DevOps** | `consensus/security-manager` + `performance-benchmarker` | Penguatan keamanan, performa, deployment | Regresi proteksi 7 lapis, sub-layanan Docker/gRPC, migrasi rollback, observabilitas, metrik Prometheus |
...
| **Koordinator Edisi** | kustom `edition-coordinator` | Melibatkan perubahan matriks versi: validasi tabel perbandingan `docs/EDITIONS.md` (kolom `lite`/`standard` adalah nilai rencana, sudah tidak ada cabang terkait), konsistensi tag versi dan catatan rilis |
...
| Selisih tag versi / rilis versi | pipeline | Lead → Koordinator Edisi → Pengujian (regresi jalur tunggal main) → Review |

### 3.4 Gerbang Kualitas (wajib sebelum commit, dijaga reviewer)

```
phpunit            # 1046<!-- stats:tests=1046 --> test / 5029<!-- stats:assertions=5029 --> assertion semua hijau, kasus baru ikut dikirim bersama perubahan
...
Perubahan yang melibatkan database harus melalui arsitek (227 tabel, schema dengan database/install.sql sebagai satu-satunya sumber kebenaran); perubahan yang melibatkan frontend harus menjalankan Flutter `flutter analyze` 0 error / 0 warning.
