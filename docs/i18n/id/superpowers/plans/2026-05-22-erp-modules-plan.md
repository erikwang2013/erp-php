# Rencana Implementasi Modul Bisnis ERP

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implementasikan modul bisnis ERP lengkap (pembelian, penjualan, inventaris + keuangan + CRM) di atas basis manajemen sistem webman v2 yang ada

**Architecture:** Arsitektur tiga lapis Controller → Service → Model. Semua model berbagi `app/model/`, controller dikelompokkan per modul di `app/controller/{module}/`, logika bisnis di `app/service/{module}/`. Sekitar 50 tabel database menggunakan prefix `erik_` yang seragam, primary key BIGINT snowflake. Versi API dikontrol melalui header `API-Version` + middleware `ApiVersion`.

**Tech Stack:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / paket seri erikwang2013/*
...
- Versi API: header request `API-Version` (mis. `v1`, `v2`), default `v1` jika tidak ada, tidak tercermin di URL

---

## Task 1: Migrasi Database — Data Dasar Produk

**Files:**
- Create: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Step 1: Buat SQL migrasi data dasar produk**
...
## Task 2: Migrasi Database — Modul Pembelian

**Files:**
- Create: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Step 1: SQL migrasi 9 tabel modul pembelian**
...
## Task 3: Migrasi Database — Modul Penjualan

**Files:**
- Create: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Step 1: SQL migrasi 9 tabel modul penjualan**
...
## Task 4: Migrasi Database — Modul Inventaris + Keuangan + CRM
...
- [ ] **Step 1: 11 tabel modul inventaris**
...
- [ ] **Step 2: 11 tabel modul keuangan**
...
## Task 5: Models Data Dasar Produk
...
- [ ] **Step 2: Buat Model data dasar lainnya (mengikuti pola yang sama)**
...
- [ ] **Step 2: Verifikasi model dapat dimuat**
...
## Task 6: Models Pembelian + Penjualan
...
- [ ] **Step 1: Buat semua Model pembelian dan penjualan**

Semua model mengikuti pola yang sama: trait `SoftDeletes` (untuk tabel utama dan kelas order), `$incrementing = false; $keyType = 'int';`, relasi BelongsTo. Berikut hanya menampilkan PurchaseOrder (yang paling representatif):
...
**Perintah pembuatan Model pembelian/penjualan lainnya:**
...
Karena total 19 model pembelian dan penjualan dengan pola yang seragam, dibuat secara batch melalui skrip. `$fillable`, `$casts`, dan metode relasi spesifik setiap Model akan dilengkapi sesuai kebutuhan saat implementasi Controller berikutnya.
...
## Task 7: Models Inventaris + Keuangan + CRM
...
- [ ] **Step 1: Buat Model Inventaris/Keuangan/CRM secara batch**

```bash
cd /home/wwwroot/erp-php/service

# Models Inventaris (11)
...
- [ ] **Step 2: Lengkapi $fillable dan relasi untuk Model kunci**

```php
<?php
// File: app/model/Inventory.php — dilengkapi penuh
declare(strict_types=1);
namespace app\model;
use support\Model;
...
class Inventory extends Model
{
    ...
    public $incrementing = false;
    protected $keyType = 'int';
    public $timestamps = false; // hanya created_at/updated_at yang otomatis
...
```php
<?php
// File: app/model/FinanceArAp.php — dilengkapi penuh
declare(strict_types=1);
namespace app\model;
use support\Model;
...
```php
<?php
// File: app/model/CrmContact.php — dilengkapi penuh
declare(strict_types=1);
namespace app\model;
use Erikwang2013\Encryptable\Encryptable;
use support\Model;
...
## Task 8: Controllers Data Dasar Produk
...
Semua controller mewarisi `app\admin\controller\BaseController`, pola CRUD sama dengan `UserController` yang ada. Berikut contoh lengkap dengan `ProductController`:

- [ ] **Step 1: Buat ProductController**
...
{
    /**
     * Daftar produk (paginasi + pencarian)
     */
    ...
    /**
     * Buat produk (termasuk SKU dan harga)
     */
    public function store(Request $request): Response
    {
        ...
        return $this->success($this->encodeIds($product->toArray()), 'Berhasil dibuat');
    }
    ...
    public function show(Request $request, string $hashid): Response
    {
        ...
        if (!$product) {
            return $this->fail('Produk tidak ada', 404);
        }
    ...
    public function update(Request $request, string $hashid): Response
    {
        ...
        return $this->success($this->encodeIds($product->toArray()), 'Berhasil diperbarui');
    }

    /**
     * Hapus produk (soft delete, konfirmasi kata sandi dua kali)
     */
...
- [ ] **Step 2: Buat Controllers data dasar lainnya**

6 controller lainnya (Category, Brand, Warehouse, Location, Supplier, Customer) semuanya CRUD standar, strukturnya sama seperti di atas. Setelah dibuat, tulis ke file secara batch:
...
## Task 9: Layanan Inventaris — Logika Inti Masuk/Keluar Stok

Ini adalah inti integrasi lintas modul; penerimaan pembelian/pengiriman penjualan dll. bergantung padanya.

**Files:**
- Create: `app/service/inventory/InventoryService.php`

- [ ] **Step 1: Buat InventoryService**
...
/**
 * Layanan inventaris — masuk/keluar stok/kalkulasi biaya
 */
class InventoryService
{
     ...
     * @param int $skuId ID SKU (0=level produk)
     * @param int $warehouseId ID gudang
     * @param int $locationId ID lokasi
     * @param string $batchCode Kode batch
     * @param float $quantity Jumlah
     * @param float $unitCost Harga satuan
     * @param string $sourceType Tipe sumber (purchase_receive|sales_return|transfer_in|check_profit)
     * @param int $sourceId ID dokumen sumber
     * @return int ID alur
     */
    public function stockIn(
        ...
        return DB::transaction(function () use (
            ...
            // 1. Tulis alur masuk/keluar stok
            ...
            // 2. Perbarui/buat stok real-time
    ...
    public function stockOut(
        ...
        return DB::transaction(function () use (
            ...
            // 1. Periksa apakah stok mencukupi
            $inv = Inventory::where([
                'product_id' => $productId,
                'sku_id' => $skuId,
                'warehouse_id' => $warehouseId,
    ...
    /**
     * Perhitungan biaya rata-rata bergerak
     *
     * Rumus: Harga rata-rata baru = (Total nilai stok awal + Total nilai stok masuk kali ini) / (Jumlah stok awal + Jumlah stok masuk kali ini)
     */
    private function recalcAverageCost(
        ...
        if ($type === 2) {
            // Keluar stok tidak mengubah harga rata-rata
            $cost = new CostRecord();
        ...
        // Hitung ulang harga rata-rata saat masuk stok
        $totalInventory = Inventory::where('product_id', $productId)
            ->where('sku_id', $skuId)
            ->sum('quantity');

        // Ambil harga rata-rata sebelumnya
...
- [ ] **Step 2: Buat tes untuk memverifikasi layanan inventaris**
...
class InventoryServiceTest extends TestCase
{
    ...
    {
        // Tes ini memerlukan koneksi database, buat kerangka terlebih dahulu
...
## Task 10: Layanan Keuangan — Pembuatan Otomatis Piutang/Hutang dan Penghapusbukuan

**Files:**
- Create: `app/service/finance/FinanceService.php`

- [ ] **Step 1: Buat FinanceService**
...
{
    /**
     * Buat otomatis catatan piutang (dipanggil setelah pengiriman penjualan)
     */
    public function createAr(
        ...
        $ar->amount = $amount;
        $ar->settled_amount = 0;
        $ar->status = 0; // belum dihapusbukukan
        $ar->due_date = $dueDate;
        $ar->save();
        return $ar->id;
    }
    ...
    /**
     * Buat otomatis catatan hutang (dipanggil setelah penerimaan pembelian)
     */
    ...
    /**
     * Penghapusbukuan piutang saat penerimaan pembayaran
     */
    public function settleReceipt(int $receiptId, int $arApId, float $amount): void
    {
        DB::transaction(function () use ($receiptId, $arApId, $amount) {
            ...
            if ($amount > $remain) {
                throw new \RuntimeException("Jumlah penghapusbukuan melebihi saldo yang belum dihapusbukukan");
            }

            // Perbarui status piutang
            $arAp->settled_amount += $amount;
            $arAp->status = $arAp->settled_amount >= $arAp->amount ? 2 : 1;
            $arAp->save();

            // Tulis catatan penghapusbukuan
            ...
            $settlement->type = 1; // penghapusbukuan piutang
    ...
    /**
     * Penghapusbukuan hutang saat pembayaran
     */
    ...
    /**
     * Perbarui jurnal setelah penerimaan/pembayaran
     */
...
## Task 11: Controller Penerimaan Pembelian (Contoh Integrasi Lintas Modul)

Ini adalah operasi bisnis lintas modul yang paling khas: penerimaan → masuk stok otomatis → buat hutang. Menampilkan kode integrasi lengkap.

**Files:**
- Create: `app/controller/purchase/ReceiveController.php`

- [ ] **Step 1: Buat controller penerimaan pembelian**
...
class ReceiveController extends BaseController
{
    ...
    /**
     * Buat nota penerimaan + masuk stok otomatis + buat hutang
    ...
    public function store(Request $request): Response
    {
        ...
        if (!$order) {
            return $this->fail('Pesanan pembelian tidak ada', 404);
        }
        ...
        try {
            ...
            $receive->status = 1; // masuk stok langsung
            $receive->received_at = date('Y-m-d H:i:s');
            $receive->save();

            $totalAmount = 0;

            // 2. Proses detail penerimaan
            foreach ($items as $item) {
                ...
                // 4. Perbarui jumlah yang sudah diterima pada detail pesanan
            ...
            if ($allReceived >= $totalItems) {
                $order->status = 3; // sudah diterima
            } else {
                $order->status = 2; // sebagian diterima
            }
            $order->save();

            // 6. Buat otomatis catatan hutang
            ...
            DB::commit();
            return $this->success($this->encodeIds($receive->toArray()), 'Penerimaan berhasil');
        } catch (\Throwable $e) {
            DB::rollBack();
            return $this->fail('Penerimaan gagal: ' . $e->getMessage(), 500);
        }
    }

    /**
     * Detail nota penerimaan
     */
    public function show(Request $request, string $hashid): Response
    {
        $id = $this->decodeId($hashid);
        ...
        if (!$receive) {
            return $this->fail('Nota penerimaan tidak ada', 404);
        }
...
## Task 12: Ekstensi Rute + Seed Izin

**Files:**
- Modify: `config/route.php`
- Create: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Step 1: Perluas file rute**

Di dalam grup rute `/admin` di `config/route.php` (sebelum `->middleware([...])`), tambahkan rute berikut:

```php
// Tambahkan di dalam Route::group('/admin', function () {:
...
- [ ] **Step 3: Verifikasi sintaks rute**
...
## Task 13: Ekstensi Panel Dasbor

**Files:**
- Modify: `app/admin/controller/DashboardController.php`

- [ ] **Step 1: Perbarui DashboardController untuk menambahkan panel penjualan, inventaris, dan keuangan**

Tambahkan metode berikut ke `DashboardController` yang ada:

```php
<?php
// Tambahkan ke app/admin/controller/DashboardController.php
...
Bagian atas DashboardController perlu ditambahkan `use`:
```php
...
{
    /**
     * Daftar produk klien (ringan, tanpa data sensitif seperti harga beli)
     */
    public function index(Request $request): Response
    {
        $page = (int) $request->input('page', 1);
    ...
    /**
     * Detail produk (termasuk SKU dan harga, tanpa harga beli)
     */
    public function show(Request $request, string $hashid): Response
    {
        $id = $this->decodeId($hashid);
...
Pendaftaran rute klien (tambahkan di dalam grup `/api` di `config/route.php`):
...
## Task 15: Halaman Frontend Flutter + HarmonyOS

**Direktori halaman Flutter Web PC:**
```
apps/flutter/lib/app/pages/
├── product/      # halaman daftar produk + halaman formulir
├── purchase/     # halaman pesanan pembelian/penerimaan
├── sales/        # halaman pesanan penjualan/pengiriman
├── inventory/    # halaman pencarian stok/opname/transfer
├── finance/      # halaman penerimaan/pembayaran/reimburse/laporan
└── crm/          # halaman peluang/tindak lanjut/funnel
```

Setiap halaman mengikuti pola kode Flutter yang ada (GetX Controller + singleton ApiService + tema Material 3).

**Direktori halaman HarmonyOS:**
```
...
- [ ] **Step 1: Contoh Flutter — halaman daftar produk**
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (_, i) => ListTile(
              ...
              subtitle: Text('Kode: ${items[i]['code']}'),
            ),
          );
        },
      ),
...
- [ ] **Step 2: Contoh HarmonyOS — halaman daftar produk**
...
## Task 16: Verifikasi Akhir dan Pengujian Integrasi

- [ ] **Step 1: Pemeriksaan sintaks PHP**
...
- [ ] **Step 3: Verifikasi kelengkapan rute**
...
- [ ] **Step 4: Pemeriksaan kelengkapan tabel database**
...
"
# Perkiraan: sekitar 55 tabel (5 tabel sistem + 50 tabel bisnis)
```
...
| Lapisan | Jumlah | Lokasi File |
|------|------|----------|
| Tabel database | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 inti | `app/service/inventory/`, `app/service/finance/` |
| Grup rute | 7 modul | `config/route.php` |
| Entri izin | ~65 | seed SQL |
| Panel dasbor | 4 | DashboardController |
| API klien | dikembangkan sesuai kebutuhan | `app/api/v1/controller/` |
| Halaman Flutter | 12+ | `apps/flutter/lib/app/pages/` |
| Halaman HarmonyOS | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**Estimasi total baris kode:** PHP ~8.000 baris / SQL ~2.000 baris / Dart ~3.000 baris / ArkTS ~1.500 baris

**Keputusan desain utama:**
1. Semua ID ditransmisikan terenkripsi di lapisan API (hashids), database menyimpan BIGINT asli
2. Bidang sensitif dienkripsi/dekripsi otomatis di tingkat database (cast Encryptable)
3. Integrasi lintas modul dipicu melalui peristiwa lapisan Service, tidak saling memanggil langsung
4. Kalkulasi biaya menggunakan metode rata-rata bergerak
5. Piutang/hutang dibuat otomatis dari dokumen pembelian/penjualan, dihapusbukukan manual
6. Data dasbor di-cache Redis selama 5 menit

**Item yang belum tercakup (perlu iterasi berikutnya):**
- Phase 2: Manajemen manufaktur, manajemen proyek
- Push real-time WebSocket (peringatan stok, pengingat persetujuan)
- Pengubahan multi-tenant
- Granularitas izin yang lebih kaya (izin data)
