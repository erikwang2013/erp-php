...
**البنية المعمارية:** Controller → Service → Model ثلاث طبقات. جميع النماذج تشترك في `app/model/`، وتنقسم المتحكمات حسب الوحدات في `app/controller/{module}/`، ويكون منطق الأعمال في `app/service/{module}/`. حوالي 50 جدولًا في قاعدة البيانات ببادئة موحدة `erp_` ومفاتيح أساسية BIGINT snowflake. إصدارات API عبر رأس الطلب `API-Version` + وسيط `ApiVersion` للتحويل.
