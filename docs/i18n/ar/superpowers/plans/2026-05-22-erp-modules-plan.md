# خطة تنفيذ وحدات الأعمال في ERP

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**الهدف:** تنفيذ وحدات أعمال ERP كاملة (المشتريات + المبيعات + المخزون + المالية + CRM) فوق قاعدة إدارة النظام الحالية webman v2

**البنية المعمارية:** Controller → Service → Model ثلاث طبقات. جميع النماذج تشترك في `app/model/`، وتنقسم المتحكمات حسب الوحدات في `app/controller/{module}/`، ويكون منطق الأعمال في `app/service/{module}/`. حوالي 50 جدولًا في قاعدة البيانات ببادئة موحدة `erik_` ومفاتيح أساسية BIGINT snowflake. إصدارات API عبر رأس الطلب `API-Version` + وسيط `ApiVersion` للتحويل.

**حزمة التقنيات:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / حزم erikwang2013/*

**أنماط قاعدة الكود** (من الكود الحالي):
- المتحكمات تمتد من `app\admin\controller\BaseController` — توفر `success()` و `fail()` و `encodeId()` و `decodeId()` و `encodeIds()` و `generateId()` و `confirmPassword()`
- النماذج تستخدم `support\Model` مع `SoftDeletes` و `Searchable` و cast `Encryptable`
- المفتاح الأساسي: `$incrementing = false; $keyType = 'int';`
- التوجيه: `Route::resource()` وتوجيهات صريحة تحت مجموعة `/admin` بسلسلة وسطاء كاملة
- الأذونات: صيغة `slug` هي `method.admin/module/action` (مثل `get.admin/product`)
- التحقق: دالة `validator()` المدمجة في webman
- تنسيق الاستجابة: `{code: 0, message: 'success', data: {...}}`
- إصدارات API: رأس الطلب `API-Version` (مثل `v1` و `v2`)، عند غيابه يكون الافتراضي `v1`، ولا يظهر في URL

---

## المهمة 1: ترحيل قاعدة البيانات — البيانات الأساسية للمنتجات

**الملفات:**
- إنشاء: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **الخطوة 1: إنشاء SQL ترحيل البيانات الأساسية للمنتجات**
...
## المهمة 2: ترحيل قاعدة البيانات — وحدة الشراء

**الملفات:**
- إنشاء: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **الخطوة 1: SQL ترحيل 9 جداول لوحدة الشراء**
...
## المهمة 3: ترحيل قاعدة البيانات — وحدة البيع

**الملفات:**
- إنشاء: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **الخطوة 1: SQL ترحيل 9 جداول لوحدة البيع**
...
## المهمة 4: ترحيل قاعدة البيانات — وحدات المخزون + المالية + CRM

**الملفات:**
- إنشاء: `database/migrations/2026_05_22_000006_inventory_tables.sql`
- إنشاء: `database/migrations/2026_05_22_000007_finance_tables.sql`
- إنشاء: `database/migrations/2026_05_22_000008_crm_tables.sql`

- [ ] **الخطوة 1: 11 جدولًا لوحدة المخزون**
...
- [ ] **الخطوة 2: 11 جدولًا لوحدة المالية**
...
## المهمة 5: نماذج البيانات الأساسية للمنتجات

**الملفات:**
- إنشاء: `app/model/Product.php`
- إنشاء: `app/model/ProductSku.php`
- إنشاء: `app/model/ProductUnit.php`
- إنشاء: `app/model/ProductPrice.php`
- إنشاء: `app/model/Category.php`
- إنشاء: `app/model/Brand.php`
- إنشاء: `app/model/Warehouse.php`
- إنشاء: `app/model/Location.php`
- إنشاء: `app/model/Supplier.php`
- إنشاء: `app/model/Customer.php`
- إنشاء: `app/model/CustomerLevel.php`

- [ ] **الخطوة 1: إنشاء نموذج Product**
...
- [ ] **الخطوة 2: إنشاء باقي نماذج البيانات الأساسية (بنفس النمط)**
...
- [ ] **الخطوة 2: التحقق من تحميل النماذج**
...
## المهمة 6: نماذج الشراء + البيع

**الملفات:**
- إنشاء: `app/model/PurchaseApply.php`, `app/model/PurchaseApplyItem.php`
- إنشاء: `app/model/PurchaseOrder.php`, `app/model/PurchaseOrderItem.php`
- إنشاء: `app/model/PurchaseReceive.php`, `app/model/PurchaseReceiveItem.php`
- إنشاء: `app/model/PurchaseReturn.php`, `app/model/PurchaseReturnItem.php`
- إنشاء: `app/model/PurchaseSettlement.php`
- إنشاء: `app/model/SalesQuotation.php`, `app/model/SalesQuotationItem.php`
- إنشاء: `app/model/SalesOrder.php`, `app/model/SalesOrderItem.php`
- إنشاء: `app/model/SalesDelivery.php`, `app/model/SalesDeliveryItem.php`
- إنشاء: `app/model/SalesReturn.php`, `app/model/SalesReturnItem.php`
- إنشاء: `app/model/SalesSettlement.php`

- [ ] **الخطوة 1: إنشاء جميع نماذج الشراء والبيع**

تتبع جميع النماذج نفس النمط: trait `SoftDeletes` (للجداول الرئيسية وطلبات الأوامر)، و `$incrementing = false; $keyType = 'int';`، وعلاقات BelongsTo. فيما يلي فقط نموذج PurchaseOrder (الأكثر تمثيلًا):
...
**أوامر إنشاء باقي نماذج الشراء/البيع:**
...
نظرًا لأن نماذج الشراء والبيع يبلغ مجموعها 19 نموذجًا وبنمط موحد، يتم إنشاؤها دفعة واحدة عبر سكربت. سيتم استكمال `$fillable` و `$casts` وطرق العلاقات الخاصة بكل نموذج حسب الحاجة أثناء تنفيذ المتحكمات لاحقًا.

- [ ] **الخطوة 2: التحقق والالتزام**
...
## المهمة 7: نماذج المخزون + المالية + CRM

**الملفات:**
- إنشاء: `app/model/Inventory.php`, `app/model/InventoryBatch.php`, `app/model/InventorySerial.php`
- إنشاء: `app/model/InventoryFlow.php`, `app/model/Transfer.php`, `app/model/TransferItem.php`
- إنشاء: `app/model/CheckTask.php`, `app/model/CheckDetail.php`
- إنشاء: `app/model/InventoryAlertRule.php`, `app/model/InventoryAlertLog.php`, `app/model/CostRecord.php`
- إنشاء: `app/model/FinanceAccount.php`, `app/model/FinanceVoucher.php`, `app/model/FinanceVoucherItem.php`
- إنشاء: `app/model/FinanceArAp.php`, `app/model/FinanceBankAccount.php`
- إنشاء: `app/model/FinanceReceipt.php`, `app/model/FinancePayment.php`, `app/model/FinanceSettlement.php`
- إنشاء: `app/model/FinanceCashJournal.php`, `app/model/FinanceExpense.php`, `app/model/FinanceProfit.php`
- إنشاء: `app/model/CrmFunnelStage.php`, `app/model/CrmOpportunity.php`
- إنشاء: `app/model/CrmFollowRecord.php`, `app/model/CrmContact.php`

- [ ] **الخطوة 1: إنشاء نماذج المخزون/المالية/CRM دفعة واحدة**
...
- [ ] **الخطوة 2: استكمال $fillable والعلاقات للنماذج الرئيسية**
...
## المهمة 8: متحكمات البيانات الأساسية للمنتجات

**الملفات:**
- إنشاء: `app/controller/product/ProductController.php`
- إنشاء: `app/controller/product/CategoryController.php`
- إنشاء: `app/controller/product/BrandController.php`
- إنشاء: `app/controller/product/WarehouseController.php`
- إنشاء: `app/controller/product/LocationController.php`
- إنشاء: `app/controller/product/SupplierController.php`
- إنشاء: `app/controller/product/CustomerController.php`

جميع المتحكمات ترث من `app\admin\controller\BaseController`، ونمط CRUD مطابق لـ `UserController` الحالي. فيما يلي المثال الكامل لـ `ProductController`:

- [ ] **الخطوة 1: إنشاء ProductController**
...
- [ ] **الخطوة 2: إنشاء باقي متحكمات البيانات الأساسية**

المتحكمات الستة المتبقية (Category و Brand و Warehouse و Location و Supplier و Customer) كلها CRUD قياسية بنفس البنية المذكورة أعلاه. بعد الإنشاء، تُكتب إلى الملفات دفعة واحدة:
...
## المهمة 9: خدمة المخزون — منطق الإدخال والإخراج الأساسي

هذا هو جوهر الترابط بين الوحدات، حيث تعتمد عليه استلام المشتريات وشحن المبيعات وغيرها.

**الملفات:**
- إنشاء: `app/service/inventory/InventoryService.php`

- [ ] **الخطوة 1: إنشاء InventoryService**
...
- [ ] **الخطوة 2: إنشاء اختبار للتحقق من خدمة المخزون**
...
## المهمة 10: خدمة المالية — التوليد التلقائي للمستحقات والمطلوبات وتسويتها

**الملفات:**
- إنشاء: `app/service/finance/FinanceService.php`

- [ ] **الخطوة 1: إنشاء FinanceService**
...
## المهمة 11: متحكم استلام الشراء (مثال على الترابط عبر الوحدات)

هذه هي العملية التجارية الأكثر تمثيلًا للترابط عبر الوحدات: الاستلام ← الإدخال التلقائي للمخزون ← توليد المطلوبات. تعرض الكود الكامل للترابط.

**الملفات:**
- إنشاء: `app/controller/purchase/ReceiveController.php`

- [ ] **الخطوة 1: إنشاء متحكم استلام الشراء**
...
## المهمة 12: توسيع التوجيه + بذور الأذونات

**الملفات:**
- تعديل: `config/route.php`
- إنشاء: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **الخطوة 1: توسيع ملف التوجيه**

داخل مجموعة التوجيه `/admin` في `config/route.php` (قبل `->middleware([...])`)، أضف التوجيهات التالية:
...
- [ ] **الخطوة 2: إنشاء SQL بذور أذونات ERP**
...
- [ ] **الخطوة 3: التحقق من صحة بناء التوجيه**
...
## المهمة 13: توسيع لوحات المعلومات

**الملفات:**
- تعديل: `app/admin/controller/DashboardController.php`

- [ ] **الخطوة 1: تحديث DashboardController بإضافة لوحات البيع والمخزون والمالية**

أضف الطرق التالية إلى `DashboardController` الحالي:
...
يجب إضافة `use` أعلى DashboardController:
```php
...
## المهمة 14: API العميل (استدعاء Flutter/HarmonyOS)

**الملفات:**
- إنشاء: `app/api/v1/controller/ProductController.php`
- إنشاء: `app/api/v1/controller/OrderController.php`

- [ ] **الخطوة 1: مثال API العميل**
...
تسجيل توجيه العميل (أضف داخل مجموعة `/api` في `config/route.php`):
...
## المهمة 15: صفحات الواجهة الأمامية Flutter + HarmonyOS

**دليل صفحات Flutter Web PC:**
...
تتبع كل صفحة نمط كود Flutter الحالي (GetX Controller + ApiService singleton + Material 3 theme).

**دليل صفحات HarmonyOS:**
...
- [ ] **الخطوة 1: مثال Flutter — صفحة قائمة المنتجات**
...
- [ ] **الخطوة 2: مثال HarmonyOS — صفحة قائمة المنتجات**
...
## المهمة 16: التحقق النهائي واختبارات التكامل

- [ ] **الخطوة 1: فحص صيغة PHP**
...
- [ ] **الخطوة 3: التحقق من اكتمال التوجيه**
...
- [ ] **الخطوة 4: فحص اكتمال جداول قاعدة البيانات**
...
| الطبقة | العدد | موقع الملف |
|------|------|----------|
| جداول قاعدة البيانات | 50 جدولًا | `database/migrations/` |
| النماذج | 44 نموذجًا | `app/model/` |
| المتحكمات | ~35 متحكمًا | `app/controller/{module}/` |
| الخدمات | خدمتان أساسيتان | `app/service/inventory/`, `app/service/finance/` |
| مجموعات التوجيه | 7 وحدات | `config/route.php` |
| بنود الأذونات | ~65 بندًا | seed SQL |
| لوحات المعلومات | 4 لوحات | DashboardController |
| API العميل | توسعة حسب الحاجة | `app/api/v1/controller/` |
| صفحات Flutter | 12+ صفحة | `apps/flutter/lib/app/pages/` |
| صفحات HarmonyOS | 6+ صفحات | `apps/harmonyos/entry/src/main/ets/pages/` |

**تقدير إجمالي حجم الكود:** PHP ~8,000 سطر / SQL ~2,000 سطر / Dart ~3,000 سطر / ArkTS ~1,500 سطر

**قرارات التصميم الرئيسية:**
1. جميع المعرفات مشفرة في طبقة API (hashids)، وتُخزن في قاعدة البيانات كـ BIGINT خام
2. الحقول الحساسة تُشفَّر وتُفكَّ تلقائيًا على مستوى قاعدة البيانات (Encryptable cast)
3. الترابط عبر الوحدات يتم عبر الأحداث على مستوى طبقة Service، دون استدعاء مباشر متبادل
4. حساب التكلفة يستخدم طريقة المتوسط المرجح المتحرك
5. المستحقات والمطلوبات تُولَّد تلقائيًا من مستندات الشراء/البيع، وتُسوَّى يدويًا
6. بيانات لوحات المعلومات تُخزَّن مؤقتًا في Redis لمدة 5 دقائق

**البنود غير المغطاة (تتطلب تكرارات لاحقة):**
- المرحلة 2: إدارة التصنيع، إدارة المشاريع
- الدفع الفوري عبر WebSocket (تنبيهات المخزون، إشعارات الموافقات)
- إعادة هيكلة تعدد المستأجرين
- دقة أذونات أعمق (أذونات البيانات)
- وضع عدم الاتصال للجوال
