# P0 لوحة إدارة Flutter Web — خطة التنفيذ

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**الهدف:** توسيع لوحة إدارة Flutter Web من ~12 صفحة حاليًا إلى 40+ صفحة تغطي جميع وحدات الخلفية الـ 14، وبناء مكتبة مكونات عامة قابلة لإعادة الاستخدام.

**البنية:** إعادة هيكلة إلى نظام قوائم ديناميكي مدفوع بالتكوين (`menu_config.dart`)، تخطيط الكمبيوتر ثلاثي الأعمدة (شريط جانبي قابل للطي + شريط علوي + منطقة محتوى)، ومكونات CRUD عامة (DataTableWrapper/FormDialog/ConfirmDialog) تعيد استخدامها صفحات الوحدات.

**الحزمة التقنية:** Flutter 3.x, Dart 3.11+, GetX 4.6, Dio 5.4, data_table_2 2.5, fl_chart 0.68, printing 5.12, excel 4.0, responsive_framework 1.4
...
├── main.dart                          # تعديل: تسجيل مركزي للمسارات
├── config/
│   └── menu_config.dart               # جديد: تكوين شجري لقائمة التنقل
├── layouts/
│   └── admin_layout.dart              # إعادة كتابة: عرض القوائم الديناميكي
├── services/                          # دون تغيير
├── widgets/                           # جديد 5 مكونات عامة
│   ├── data_table_wrapper.dart
│   ├── form_dialog.dart
│   ├── confirm_dialog.dart
│   ├── stat_card.dart
│   └── search_bar.dart
└── pages/                             # إعادة تنظيم إلى أدلة وحدات
...
### المهمة 1: تكوين القوائم وإعادة هيكلة المسارات

**الملفات:**
- إنشاء: `apps/flutter/lib/app/config/menu_config.dart`
- تعديل: `apps/flutter/lib/app/layouts/admin_layout.dart` (إعادة كتابة كاملة)
- تعديل: `apps/flutter/lib/main.dart` (تسجيل المسارات)
- نقل: ترحيل ملفات الصفحات الحالية إلى بنية الأدلة الجديدة

- [ ] **الخطوة 1: إنشاء menu_config.dart**
...
- [ ] **الخطوة 2: إعادة كتابة AdminLayout لدعم عرض القوائم الديناميكي**
...
- [ ] **الخطوة 3: تحديث main.dart بتسجيل مركزي للمسارات**
...
- [ ] **الخطوة 4: ترحيل أدلة ملفات الصفحات الحالية**
...
- [ ] **الخطوة 5: التحقق من الترجمة** — Run: `cd apps/flutter && flutter analyze`

- [ ] **الخطوة 6: الالتزام**

---

### المهمة 2: المكون العام — DataTableWrapper

**الملفات:** إنشاء: `apps/flutter/lib/app/widgets/data_table_wrapper.dart`
...
### المهمة 3: المكونات العامة — FormDialog + ConfirmDialog + StatCard

**الملفات:**
- إنشاء: `apps/flutter/lib/app/widgets/form_dialog.dart`
- إنشاء: `apps/flutter/lib/app/widgets/confirm_dialog.dart`
- إنشاء: `apps/flutter/lib/app/widgets/stat_card.dart`

- [ ] **الخطوة 1: إنشاء FormDialog**
...
### المهام 4-20: الإنشاء الجماعي لصفحات الوحدات

> كل مهمة تغطي وحدة واحدة، وتتبع جميع صفحات القوائم نمط CRUD موحدًا.

**قالب صفحة قائمة CRUD القياسي** (كل صفحة ~70 سطرًا):
...
| المهمة | الوحدة | الدليل | عدد الصفحات |
|------|------|------|--------|
| 4 | المنتج | `pages/product/` | 4 |
| 5 | الأطراف المقابلة | `pages/partner/` | 4 |
| 6 | المشتريات | `pages/purchase/` | 4 |
| 7 | المبيعات | `pages/sales/` | 4 |
| 8 | المخزون | `pages/inventory/` | 5 |
| 9 | المالية 1 (سندات/مستحقات ومطلوبات/مقبوضات ومدفوعات/دفتر يومي/مصاريف) | `pages/finance/` | 6 |
| 10 | المالية 2 (دفتر عام/تقارير/أصول/ضرائب/عملات/ميزانيات/تكاليف) | `pages/finance/` | 7 |
| 11 | CRM | `pages/crm/` | 8 |
| 12 | OMS | `pages/oms/` | 4 |
| 13 | WMS | `pages/wms/` | 7 |
| 14 | TMS | `pages/tms/` | 5 |
| 15 | التصنيع | `pages/manufacturing/` | 5 |
| 16 | HR | `pages/hr/` | 6 |
| 17 | المشاريع | `pages/project/` | 3 |
| 18 | سير عمل الموافقات | `pages/workflow/` | 2 |
| 19 | الإشعارات | `pages/notification/` | 1 |
| 20 | التقارير المخصصة | `pages/report/` | 2 |

بعد اكتمال كل مهمة: تحديث `main.dart` لإضافة المسار المقابل + التحقق عبر `flutter analyze`.

---

### المهمة 21: توسيع لوحة المعلومات

**الملفات:**
- تعديل: `apps/flutter/lib/app/pages/dashboard/dashboard_page.dart`
- تعديل: `apps/flutter/lib/app/pages/dashboard/dashboard_controller.dart`

- [ ] **الخطوة 1: إضافة لوحات OMS/WMS/TMS فوق التبويبات الأربعة الحالية**

إضافة إلى TabBar: `const Tab(text: 'OMS')`, `const Tab(text: 'WMS')`, `const Tab(text: 'TMS')`
وإضافة طرق `_omsTab()` و`_wmsTab()` و`_tmsTab()` المقابلة في TabBarView، تحتوي كل منها على 3-4 بطاقات StatCard.

- [ ] **الخطوة 2: الالتزام**

---

### المهمة 22: تسجيل جميع المسارات + التحقق النهائي

- [ ] **الخطوة 1: استكمال تسجيلات GetPage في main.dart**
- [ ] **الخطوة 2: `flutter analyze` → No issues found**
- [ ] **الخطوة 3: `flutter run -d chrome` قبول يدوي لقوائم وصفحات جميع الوحدات الـ 14**
- [ ] **الخطوة 4: الالتزام**

---

### المهمة 23: محاذاة HarmonyOS

**الملفات:** إنشاء صفحات جديدة تحت `apps/harmonyos/entry/src/main/ets/pages/`

- [ ] إنشاء صفحات ArkTS لوحدات OMS/WMS/TMS/تصنيع/HR/موافقات/إشعارات/تقارير بإجمالي 8 وحدات
- [ ] تحديث `main_pages.json` لتسجيل المسارات
- [ ] التحقق من الترجمة

---

**معايير القبول (اكتمال P0):**
- [ ] الشريط الجانبي لـ Flutter Web يسمح بالتنقل بين جميع الوحدات الـ 14
- [ ] جميع صفحات قوائم CRUD تعمل بشكل طبيعي: التحميل وترقيم الصفحات والبحث
- [ ] نوافذ الإنشاء/التعديل + تأكيد كلمة المرور للحذف تعمل
- [ ] تخطيط متجاوب طبيعي لـ PC/الجهاز اللوحي/الموبايل
- [ ] انتهاء صلاحية JWT يجدد تلقائيًا
