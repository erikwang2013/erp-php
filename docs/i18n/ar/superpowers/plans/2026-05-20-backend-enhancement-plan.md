# تعزيز الخلفية — خطة التنفيذ

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**الهدف:** تنفيذ 13 تحسينًا للخلفية: 3 وسائط جديدة (CORS, RateLimit, OperationLog) و6 وحدات تحكم جديدة وتعديل نموذجين وتكوين المسارات + الوسائط.

**البنية:** اتباع نمط webman الحالي — الوسائط تنفذ `MiddlewareInterface::process()`، ووحدات التحكم ترث `BaseController`، والمسارات تُعرف بإغلاقات أو `[class, method]`. تُسجل الوسائط العامة في `config/middleware.php`، وتُثبت وسائط مستوى المسار على مجموعات المسارات في `config/route.php`.

**الحزمة التقنية:** PHP 8.3+, webman v2, Redis (إضافة), PhpSpreadsheet (استيراد)

**ترتيب الاعتماديات:** تعديل النماذج ← الوسائط ← وحدات التحكم ← المسارات/التكوين

---

### المهمة 1: إضافة SoftDeletes + trait Searchable إلى نموذج AdminUser

**الملفات:**
- تعديل: `app/model/AdminUser.php`

- [ ] **الخطوة 1: تعديل نموذج AdminUser**

إدراج استيرادَين `use` بعد السطر 10 في `app/model/AdminUser.php`، واستخدام traitَين في أول جسم الفئة:
...
وفي الوقت نفسه إضافة طريقة `toSearchableArray()` في نهاية الفئة (بعد طريقة `roles()`، قبل قوس الإغلاق للفئة):
...
**الملفات:**
- إنشاء: `app/middleware/Cors.php`

- [ ] **الخطوة 1: إنشاء وسيط Cors**
...
### المهمة 3: وسيط تحديد المعدل RateLimit

**الملفات:**
- إنشاء: `app/middleware/RateLimit.php`

- [ ] **الخطوة 1: إنشاء وسيط RateLimit**
...
### المهمة 4: وسيط سجلات العمليات OperationLog

**الملفات:**
- إنشاء: `app/middleware/OperationLog.php`

- [ ] **الخطوة 1: إنشاء وسيط OperationLog**
...
### المهمة 5: تكوين الوسائط العامة

**الملفات:**
- تعديل: `config/middleware.php`
...
### المهمة 6: وحدة تحكم فحص الصحة

**الملفات:**
- إنشاء: `app/admin/controller/HealthController.php`
...
### المهمة 7: وحدة تحكم CRUD لإعدادات النظام

**الملفات:**
- إنشاء: `app/admin/controller/ConfigController.php`
...
### المهمة 8: وحدة تحكم استعلام سجلات العمليات

**الملفات:**
- إنشاء: `app/admin/controller/LogController.php`
...
### المهمة 9: وحدة تحكم المركز الشخصي (تشمل الخروج)

**الملفات:**
- إنشاء: `app/admin/controller/ProfileController.php`
- تعديل: `app/middleware/AdminAuth.php` (التحقق من القائمة السوداء JWT)

- [ ] **الخطوة 1: تعديل وسيط AdminAuth، إضافة التحقق من القائمة السوداء JWT**

في طريقة `AdminAuth::process()`، إدراج فحص القائمة السوداء بعد استخراج الرمز وقبل فك تشفير JWT:
...
يلزم في الوقت نفسه إضافة استيراد Redis. تعديل رأس الملف:
...
### المهمة 10: وحدة تحكم رفع الملفات

**الملفات:**
- إنشاء: `app/admin/controller/UploadController.php`
...
### المهمة 11: العمليات الدفعية للمستخدمين

**الملفات:**
- تعديل: `app/admin/controller/UserController.php`
...
### المهمة 12: وحدة تحكم استيراد Excel

**الملفات:**
- إنشاء: `app/admin/controller/ImportController.php`
...
### المهمة 13: تحديث المسارات — جميع المسارات الجديدة + ربط الوسائط
...
إضافة مسارات جديدة داخل مجموعة مسارات `/admin`، مع إلحاق وسيط OperationLog. ملف المسارات الكامل:
...
بعد اكتمال التنفيذ، تحقق عنصرًا بعنصر:
