# План реализации ERP-модулей

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在现有 webman v2 系统管理基座上实现进销存+财务+CRM 完整ERP业务模块

**Architecture:** Controller → Service → Model 三层。所有模型共享 `app/model/`，控制器按模块分目录 `app/controller/{module}/`，业务逻辑在 `app/service/{module}/`。数据库约50张表统一 `erik_` 前缀、BIGINT snowflake主键。API版本通过 `API-Version` 请求头 + `ApiVersion` 中间件转发。
...
## Задача 1: Миграция БД — базовые данные товаров

**Файлы:**
- Создать: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Шаг 1: Создание SQL-миграции базовых данных товаров**
...
## Задача 2: Миграция БД — модуль закупок

**Файлы:**
- Создать: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Шаг 1: SQL-миграция 9 таблиц модуля закупок**
...
## Задача 3: Миграция БД — модуль продаж

**Файлы:**
- Создать: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Шаг 1: SQL-миграция 9 таблиц модуля продаж**
...
## Задача 4: Миграция БД — модули склад + финансы + CRM

**Файлы:**
- Создать: `database/migrations/2026_05_22_000006_inventory_tables.sql`
- Создать: `database/migrations/2026_05_22_000007_finance_tables.sql`
- Создать: `database/migrations/2026_05_22_000008_crm_tables.sql`

- [ ] **Шаг 1: 11 таблиц модуля склада**
...
- [ ] **Шаг 2: 11 таблиц модуля финансов**
...
## Задача 5: Models базовых данных товаров

**Файлы:**
- Создать: `app/model/Product.php`
- Создать: `app/model/ProductSku.php`
- Создать: `app/model/ProductUnit.php`
- Создать: `app/model/ProductPrice.php`
- Создать: `app/model/Category.php`
- Создать: `app/model/Brand.php`
- Создать: `app/model/Warehouse.php`
- Создать: `app/model/Location.php`
- Создать: `app/model/Supplier.php`
- Создать: `app/model/Customer.php`
- Создать: `app/model/CustomerLevel.php`
...
- [ ] **Шаг 2: Создание остальных моделей базовых данных (по тому же шаблону)**
...
- [ ] **Шаг 2: Проверить загрузку моделей**
...
## Задача 6: Models закупок и продаж

**Файлы:**
- Создать: `app/model/PurchaseApply.php`, `app/model/PurchaseApplyItem.php`
- Создать: `app/model/PurchaseOrder.php`, `app/model/PurchaseOrderItem.php`
- Создать: `app/model/PurchaseReceive.php`, `app/model/PurchaseReceiveItem.php`
- Создать: `app/model/PurchaseReturn.php`, `app/model/PurchaseReturnItem.php`
- Создать: `app/model/PurchaseSettlement.php`
- Создать: `app/model/SalesQuotation.php`, `app/model/SalesQuotationItem.php`
- Создать: `app/model/SalesOrder.php`, `app/model/SalesOrderItem.php`
- Создать: `app/model/SalesDelivery.php`, `app/model/SalesDeliveryItem.php`
- Создать: `app/model/SalesReturn.php`, `app/model/SalesReturnItem.php`
- Создать: `app/model/SalesSettlement.php`

- [ ] **Шаг 1: Создание всех моделей закупок и продаж**

Все модели следуют одному шаблону: trait `SoftDeletes` (главные таблицы и классы заказов), `$incrementing = false; $keyType = 'int';`, связи BelongsTo. Ниже показан только PurchaseOrder (самый показательный):
...
**Команда создания остальных моделей закупок/продаж:**
...
Поскольку моделей закупок и продаж всего 19 и они однотипны, они создаются пакетно через скрипт. Конкретные `$fillable`, `$casts` и методы связей каждой модели будут дополнены по мере необходимости при реализации Controller.
...
## Задача 7: Models склада + финансов + CRM

**Файлы:**
- Создать: `app/model/Inventory.php`, `app/model/InventoryBatch.php`, `app/model/InventorySerial.php`
- Создать: `app/model/InventoryFlow.php`, `app/model/Transfer.php`, `app/model/TransferItem.php`
- Создать: `app/model/CheckTask.php`, `app/model/CheckDetail.php`
- Создать: `app/model/InventoryAlertRule.php`, `app/model/InventoryAlertLog.php`, `app/model/CostRecord.php`
- Создать: `app/model/FinanceAccount.php`, `app/model/FinanceVoucher.php`, `app/model/FinanceVoucherItem.php`
- Создать: `app/model/FinanceArAp.php`, `app/model/FinanceBankAccount.php`
- Создать: `app/model/FinanceReceipt.php`, `app/model/FinancePayment.php`, `app/model/FinanceSettlement.php`
- Создать: `app/model/FinanceCashJournal.php`, `app/model/FinanceExpense.php`, `app/model/FinanceProfit.php`
- Создать: `app/model/CrmFunnelStage.php`, `app/model/CrmOpportunity.php`
- Создать: `app/model/CrmFollowRecord.php`, `app/model/CrmContact.php`

- [ ] **Шаг 1: Пакетное создание моделей склада/финансов/CRM**
...
- [ ] **Шаг 2: Дополнение $fillable и связей ключевых моделей**
...
## Задача 8: Контроллеры базовых данных товаров

**Файлы:**
- Создать: `app/controller/product/ProductController.php`
- Создать: `app/controller/product/CategoryController.php`
- Создать: `app/controller/product/BrandController.php`
- Создать: `app/controller/product/WarehouseController.php`
- Создать: `app/controller/product/LocationController.php`
- Создать: `app/controller/product/SupplierController.php`
- Создать: `app/controller/product/CustomerController.php`

Все контроллеры наследуют `app\admin\controller\BaseController`, шаблон CRUD совпадает с существующим `UserController`. Ниже — полный пример `ProductController`:

- [ ] **Шаг 1: Создание ProductController**
...
- [ ] **Шаг 2: Создание остальных контроллеров базовых данных**

Остальные 6 контроллеров (Category, Brand, Warehouse, Location, Supplier, Customer) — стандартный CRUD, структура та же. После создания файлы записываются пакетно:
...
## Задача 9: Сервис склада — основная логика прихода/расхода

Это ядро межмодульной связки: от него зависят приёмка закупок, отгрузка продаж и т.д.

**Файлы:**
- Создать: `app/service/inventory/InventoryService.php`

- [ ] **Шаг 1: Создание InventoryService**
...
- [ ] **Шаг 2: Создание теста для проверки сервиса склада**
...
## Задача 10: Финансовый сервис — автогенерация дебиторки-кредиторки и зачёт

**Файлы:**
- Создать: `app/service/finance/FinanceService.php`

- [ ] **Шаг 1: Создание FinanceService**
...
## Задача 11: Контроллер приёмки закупок (пример межмодульной связки)

Это самая типичная межмодульная бизнес-операция: приёмка → автооприходование → формирование кредиторки. Показан полный код связки.

**Файлы:**
- Создать: `app/controller/purchase/ReceiveController.php`

- [ ] **Шаг 1: Создание контроллера приёмки закупок**
...
## Задача 12: Расширение маршрутов + сид прав

**Файлы:**
- Изменить: `config/route.php`
- Создать: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Шаг 1: Расширение файла маршрутов**

Внутри группы маршрутов `/admin` в `config/route.php` (перед `->middleware([...])`) добавьте следующие маршруты:
...
- [ ] **Шаг 2: Создание SQL-сида прав ERP**
...
- [ ] **Шаг 3: Проверка синтаксиса маршрутов**
...
## Задача 13: Расширение панелей дашборда

**Файлы:**
- Изменить: `app/admin/controller/DashboardController.php`

- [ ] **Шаг 1: Обновление DashboardController: добавление панелей продаж, склада и финансов**

В существующий `DashboardController` добавьте следующие методы:
...
В начало DashboardController нужно добавить `use`:
```php
...
## Задача 14: Клиентский API (вызовы из Flutter/HarmonyOS)

**Файлы:**
- Создать: `app/api/v1/controller/ProductController.php`
- Создать: `app/api/v1/controller/OrderController.php`

- [ ] **Шаг 1: Пример клиентского API**
...
Регистрация клиентских маршрутов (добавьте в группу `/api` в `config/route.php`):
...
## Задача 15: Фронтенд-страницы Flutter + HarmonyOS

**Каталог страниц Flutter Web (PC):**
```
...
Каждая страница следует существующему шаблону Flutter (GetX Controller + синглтон ApiService + тема Material 3).

**Каталог страниц HarmonyOS:**
```
...
- [ ] **Шаг 1: Пример Flutter — страница списка товаров**
...
- [ ] **Шаг 2: Пример HarmonyOS — страница списка товаров**
...
## Задача 16: Финальная проверка и интеграционные тесты

- [ ] **Шаг 1: Проверка синтаксиса PHP**
...
- [ ] **Шаг 3: Проверка целостности маршрутов**
...
- [ ] **Шаг 4: Проверка целостности таблиц БД**
...
| Уровень | Количество | Расположение файлов |
|------|------|----------|
| Таблицы БД | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 ключевых | `app/service/inventory/`, `app/service/finance/` |
| Группы маршрутов | 7 модулей | `config/route.php` |
| Записи прав | ~65 | seed SQL |
| Панели дашборда | 4 | DashboardController |
| Клиентский API | расширяется по мере необходимости | `app/api/v1/controller/` |
| Страницы Flutter | 12+ | `apps/flutter/lib/app/pages/` |
| Страницы HarmonyOS | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**Оценка общего объёма кода:** PHP ~8000 строк / SQL ~2000 строк / Dart ~3000 строк / ArkTS ~1500 строк

**Ключевые проектные решения:**
1. Все ID передаются в API-слое зашифрованными (hashids), в БД хранятся исходные BIGINT
2. Чувствительные поля автоматически шифруются/дешифруются на уровне БД (cast Encryptable)
3. Межмодульная связка выполняется через события на уровне Service, без прямых взаимных вызовов
4. Калькуляция себестоимости — методом скользящего средневзвешенного
5. Дебиторка-кредиторка формируется автоматически из документов закупок/продаж, зачёт выполняется вручную
6. Данные дашборда кэшируются в Redis на 5 минут

**Не покрыто (требует последующих итераций):**
- Phase 2: управление производством, управление проектами
- Пуш-уведомления WebSocket в реальном времени (предупреждения о запасах, напоминания об одобрении)
- Мультитенантная доработка
- Более детальная гранулярность прав (права на данные)
- Офлайн-режим мобильной части
