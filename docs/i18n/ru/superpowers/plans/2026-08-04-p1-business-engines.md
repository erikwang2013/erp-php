# Движки бизнес-глубины P1 — план реализации

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) to implement this plan task-by-task.

**Цель:** модернизировать три ключевых модуля — финансы, зарплата, MRP — со скелета CRUD до промышленных движков бизнес-расчётов

**Архитектура:** каждый движок — независимый класс Service, следует соглашению о каталоге `app/service/{module}/`, зависимости внедряются через конструктор, `DB::transaction()` гарантирует согласованность данных, тесты PHPUnit покрывают ключевые алгоритмы

**Технологический стек:** PHP 8.3, workerman/webman-framework 2.x, Eloquent ORM, PHPUnit 12

---

## Задача 1: Движок двойной записи в финансах

**Файлы:**
- Создать: `app/service/finance/DoubleEntryService.php`
- Создать: `tests/DoubleEntryServiceTest.php`
- Изменить: `app/controller/finance/VoucherController.php`

### Предыстория

Существующий `FinanceVoucher` делает только простой CRUD, без проверки баланса дебета и кредита. В таблице `FinanceVoucherItem` уже есть поля `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary`. Главное правило: «нет дебета без кредита, дебет и кредит должны быть равны».

- [ ] **Шаг 1: Создать DoubleEntryService**
...
- [ ] **Шаг 3: Интеграция VoucherController**

Изменить `store()` в `app/controller/finance/VoucherController.php`: если запрос содержит `items`, вызывать `DoubleEntryService::createVoucher()`, иначе откатиться на исходный CRUD.
...
## Задача 2: Движок расчёта зарплаты

**Файлы:**
- Создать: `app/service/hr/SalaryEngineService.php`
- Создать: `tests/SalaryEngineServiceTest.php`

- [ ] **Шаг 1: Создать SalaryEngineService**
...
## Задача 3: Движок MRP (планирование потребностей в материалах)

**Файлы:**
- Создать: `app/service/manufacturing/MrpEngineService.php`
- Создать: `tests/MrpEngineServiceTest.php`

- [ ] **Шаг 1: Создать MrpEngineService**
...
## Задача 4: Интеграция в существующие контроллеры

**Файлы:**
- Изменить: `app/controller/finance/VoucherController.php`
- Изменить: `app/controller/hr/SalaryController.php`
- Изменить: `app/controller/manufacturing/MrpController.php`

- [ ] **Шаг 1: Интеграция VoucherController** — `store()` определяет параметр `items`, при наличии использует `DoubleEntryService`, при отсутствии — исходный CRUD
- [ ] **Шаг 2: Интеграция SalaryController** — `store()` вызывает `SalaryEngineService::calculate()` для автоматического расчёта деталей зарплаты
- [ ] **Шаг 3: Интеграция MrpController** — `generate()` вызывает `MrpEngineService::explodeBom()` + `calculateNetRequirement()`
- [ ] **Шаг 4: Запустить все тесты** — `php vendor/bin/phpunit --testdox`
...
- [ ] При сохранении документа с неравными дебетом и кредитом → 422 «借贷不平衡»
- [ ] `SalaryEngineService::calculate(10000)` → gross=10000, si=1050, hf=700, tax правильно рассчитан
- [ ] `MrpEngineService::calculateNetRequirement(100, 30, 0, 0, 20)` → 90
- [ ] BOM разворачивается многоуровневой рекурсией до сырья, с учётом нормы потерь
- [ ] `php vendor/bin/phpunit --testdox` — все пройдены
