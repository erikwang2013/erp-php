# Расширение бэкенда — план реализации

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Цель:** реализовать 13 улучшений бэкенда: 3 новых промежуточных ПО (CORS, RateLimit, OperationLog), 6 новых контроллеров, 2 изменения моделей, настройку маршрутов + промежуточного ПО.

**Архитектура:** следовать существующим паттернам webman — промежуточное ПО реализует `MiddlewareInterface::process()`, контроллеры наследуют `BaseController`, маршруты задаются замыканиями или `[class, method]`. Глобальное промежуточное ПО регистрируется в `config/middleware.php`, маршрутное — монтируется на группы маршрутов в `config/route.php`.

**Технологический стек:** PHP 8.3+, webman v2, Redis (расширение), PhpSpreadsheet (импорт)

**Порядок зависимостей:** изменение моделей → промежуточное ПО → контроллеры → маршруты/конфигурация

---

### Задача 1: добавить моделям AdminUser traits SoftDeletes и Searchable

**Файлы:**
- Изменить: `app/model/AdminUser.php`

- [ ] **Шаг 1: изменить модель AdminUser**

После 10-й строки в `app/model/AdminUser.php` вставить два импорта `use`, а в начале тела класса использовать два trait:
...
В конце класса (после метода `roles()`, перед закрывающей скобкой класса) добавить метод `toSearchableArray()`:
...
### Задача 2: промежуточное ПО CORS

**Файлы:**
- Создать: `app/middleware/Cors.php`

- [ ] **Шаг 1: создать промежуточное ПО Cors**
...
### Задача 3: промежуточное ПО ограничения частоты RateLimit

**Файлы:**
- Создать: `app/middleware/RateLimit.php`

- [ ] **Шаг 1: создать промежуточное ПО RateLimit**
...
### Задача 4: промежуточное ПО журнала операций OperationLog

**Файлы:**
- Создать: `app/middleware/OperationLog.php`

- [ ] **Шаг 1: создать промежуточное ПО OperationLog**
...
### Задача 5: конфигурация глобального промежуточного ПО

**Файлы:**
- Изменить: `config/middleware.php`

- [ ] **Шаг 1: зарегистрировать глобальное промежуточное ПО**

Заменить содержимое `config/middleware.php` на:
...
### Задача 6: контроллер проверки работоспособности

**Файлы:**
- Создать: `app/admin/controller/HealthController.php`

- [ ] **Шаг 1: создать HealthController**
...
### Задача 7: контроллер CRUD системной конфигурации

**Файлы:**
- Создать: `app/admin/controller/ConfigController.php`

- [ ] **Шаг 1: создать ConfigController**
...
### Задача 8: контроллер запроса журнала операций

**Файлы:**
- Создать: `app/admin/controller/LogController.php`

- [ ] **Шаг 1: создать LogController**
...
### Задача 9: контроллер личного кабинета (включая выход)

**Файлы:**
- Создать: `app/admin/controller/ProfileController.php`
- Изменить: `app/middleware/AdminAuth.php` (проверка JWT-чёрного списка)

- [ ] **Шаг 1: изменить промежуточное ПО AdminAuth, добавив проверку JWT-чёрного списка**

В методе `AdminAuth::process()` после извлечения токена, до декодирования JWT, вставить проверку чёрного списка:
...
Одновременно добавить импорт Redis. Изменить шапку файла:
...
- [ ] **Шаг 2: создать ProfileController**
...
### Задача 10: контроллер загрузки файлов

**Файлы:**
- Создать: `app/admin/controller/UploadController.php`

- [ ] **Шаг 1: создать UploadController**
...
### Задача 11: массовые операции с пользователями

**Файлы:**
- Изменить: `app/admin/controller/UserController.php`

- [ ] **Шаг 1: добавить два метода в конец класса UserController (после метода `destroy`)**
...
### Задача 12: контроллер импорта Excel

**Файлы:**
- Создать: `app/admin/controller/ImportController.php`

- [ ] **Шаг 1: создать ImportController**
...
### Задача 13: обновление маршрутов — все новые маршруты + привязка промежуточного ПО

**Файлы:**
- Изменить: `config/route.php`

- [ ] **Шаг 1: обновить конфигурацию маршрутов**

В группе маршрутов `/admin` добавить новые маршруты, одновременно добавив промежуточное ПО OperationLog. Полный файл маршрутов:
...
После завершения реализации проверить по пунктам:
