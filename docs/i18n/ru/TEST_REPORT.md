...
> Поправка: api-tester подозревал отсутствие таблицы `erik_admin_config` — **не дефект**. Настоящее имя таблицы `erik_system_config` (создана в install.sql:133, модель SystemConfig указывает правильно), отчёт исправлен.
