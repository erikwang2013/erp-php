...
- Tabellenpräfix `erp_`, Soft-Delete, globale Funktionen ohne `\`
...
Alle Tabellen mit `erp_`-Präfix, `id` BIGINT nicht auto-increment, enthalten `created_at`/`updated_at`/`deleted_at`.
