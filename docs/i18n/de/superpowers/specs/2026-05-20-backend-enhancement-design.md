...
Die Tabelle `erp_operation_log` hat nur die Spalte `created_at` (kein `updated_at`). Eloquent versucht beim Standard-`save()` `updated_at` zu schreiben, was zu einem SQL-Fehler führt.
