...
**Architecture:** Controller → Service → Model-Drei-Schichten-Architektur. Alle Modelle teilen sich `app/model/`, Controller sind nach Modulen in Verzeichnissen `app/controller/{module}/` organisiert, die Geschäftslogik liegt in `app/service/{module}/`. Etwa 50 Datenbanktabellen mit einheitlichem `erp_`-Präfix und BIGINT-Snowflake-Primärschlüsseln. Die API-Version wird über den Request-Header `API-Version` + `ApiVersion`-Middleware weitergeleitet.
