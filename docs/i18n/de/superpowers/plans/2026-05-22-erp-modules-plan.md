# ERP-Businessmodul-Implementierungsplan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implementierung vollständiger ERP-Businessmodule (Einkauf/Verkauf/Bestand + Finanzwesen + CRM) auf der bestehenden webman-v2-Systemverwaltungsbasis

**Architecture:** Controller → Service → Model-Drei-Schichten-Architektur. Alle Modelle teilen sich `app/model/`, Controller sind nach Modulen in Verzeichnissen `app/controller/{module}/` organisiert, die Geschäftslogik liegt in `app/service/{module}/`. Etwa 50 Datenbanktabellen mit einheitlichem `erik_`-Präfix und BIGINT-Snowflake-Primärschlüsseln. Die API-Version wird über den Request-Header `API-Version` + `ApiVersion`-Middleware weitergeleitet.

**Tech Stack:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / erikwang2013/*-Pakete

**Codebase patterns** (aus vorhandenem Code):
- Controller erben von `app\admin\controller\BaseController` — stellt `success()`, `fail()`, `encodeId()`, `decodeId()`, `encodeIds()`, `generateId()`, `confirmPassword()` bereit
- Modelle verwenden `support\Model` mit `SoftDeletes`-, `Searchable`- und `Encryptable`-Cast
- Primärschlüssel: `$incrementing = false; $keyType = 'int';`
- Routen: `Route::resource()` und explizite Routen unter der `/admin`-Gruppe mit vollständiger Middleware-Kette
- Berechtigungen: `slug`-Format `method.admin/module/action` (z. B. `get.admin/product`)
- Validierung: eingebaute webman-Funktion `validator()`
- Antwortformat: `{code: 0, message: 'success', data: {...}}`
- API-Version: Request-Header `API-Version` (z. B. `v1`, `v2`); fehlt der Header, gilt standardmäßig `v1`; nicht in der URL abgebildet

---

## Task 1: Datenbank-Migration — Artikelstammdaten

**Files:**
- Create: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Step 1: Migrations-SQL für die Artikelstammdaten erstellen**
...
## Task 2: Datenbank-Migration — Einkaufsmodul

**Files:**
- Create: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Step 1: Migrations-SQL für die 9 Tabellen des Einkaufsmoduls**
...
## Task 3: Datenbank-Migration — Verkaufsmodul

**Files:**
- Create: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Step 1: Migrations-SQL für die 9 Tabellen des Verkaufsmoduls**
...
## Task 4: Datenbank-Migration — Bestands-, Finanz- und CRM-Module
...
- [ ] **Step 1: 11 Tabellen des Bestandsmoduls**
...
- [ ] **Step 2: 11 Tabellen des Finanzmoduls**
...
- [ ] **Step 3: 4 Tabellen des CRM-Moduls**
...
- [ ] **Step 4: Alle Migrationen ausführen**
...
## Task 5: Models für Artikelstammdaten
...
- [ ] **Step 1: Product-Model erstellen**
...
- [ ] **Step 2: Übrige Models für Stammdaten erstellen (nach demselben Muster)**
...
## Task 6: Einkaufs- + Verkaufs-Models
...
- [ ] **Step 1: Alle Einkaufs- und Verkaufs-Models erstellen**

Alle Models folgen demselben Muster: `SoftDeletes`-Trait (Stammtabellen und Auftragsklassen), `$incrementing = false; $keyType = 'int';`, BelongsTo-Beziehungen. Im Folgenden wird nur PurchaseOrder gezeigt (am repräsentativsten):
...
**Befehle zum Erstellen der übrigen Einkaufs-/Verkaufs-Models:**
...
Da die 19 Einkaufs- und Verkaufs-Models ein einheitliches Muster haben, werden sie per Skript in Serie erstellt. Die konkreten `$fillable`-, `$casts`- und Beziehungsmethoden jedes Models werden bei der späteren Controller-Implementierung nach Bedarf ergänzt.
...
## Task 7: Bestands-, Finanz- und CRM-Models
...
- [ ] **Step 1: Bestands-/Finanz-/CRM-Models in Serie erstellen**
...
- [ ] **Step 2: $fillable und Beziehungen für Schlüssel-Models ergänzen**
...
## Task 8: Controller für Artikelstammdaten
...
Alle Controller erben von `app\admin\controller\BaseController`; das CRUD-Muster entspricht dem bestehenden `UserController`. Im Folgenden ein vollständiges Beispiel mit `ProductController`:

- [ ] **Step 1: ProductController erstellen**
...
- [ ] **Step 2: Übrige Controller für Stammdaten erstellen**

Die übrigen 6 Controller (Category, Brand, Warehouse, Location, Supplier, Customer) sind alle Standard-CRUD mit derselben Struktur wie oben. Nach der Erstellung in Serie in Dateien schreiben:
...
## Task 9: Bestandsservice — Kernlogik für Ein-/Auslagerung

Dies ist der Kern der modulübergreifenden Kopplung; Wareneingang im Einkauf, Versand im Verkauf usw. hängen davon ab.

**Files:**
- Create: `app/service/inventory/InventoryService.php`

- [ ] **Step 1: InventoryService erstellen**
...
- [ ] **Step 2: Test zur Verifizierung des Bestandsservices erstellen**
...
## Task 10: Finanzservice — automatische Erzeugung und Verrechnung von Forderungen/Verbindlichkeiten

**Files:**
- Create: `app/service/finance/FinanceService.php`

- [ ] **Step 1: FinanceService erstellen**
...
## Task 11: Controller für Einkaufswareneingang (Beispiel für modulübergreifende Kopplung)

Dies ist der typischste modulübergreifende Geschäftsablauf: Wareneingang → automatische Einlagerung → Erzeugung von Verbindlichkeiten. Vollständiger Kopplungscode wird gezeigt.

**Files:**
- Create: `app/controller/purchase/ReceiveController.php`

- [ ] **Step 1: Controller für den Einkaufswareneingang erstellen**
...
## Task 12: Routenerweiterung + Berechtigungs-Seeds

**Files:**
- Modify: `config/route.php`
- Create: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Step 1: Routendatei erweitern**

Innerhalb der `/admin`-Routengruppe in `config/route.php` (vor `->middleware([...])`) die folgenden Routen anhängen:
...
- [ ] **Step 2: SQL-Seeds für ERP-Berechtigungen erstellen**
...
- [ ] **Step 3: Routensyntax verifizieren**
...
## Task 13: Erweiterung der Dashboard-Panels

**Files:**
- Modify: `app/admin/controller/DashboardController.php`

- [ ] **Step 1: DashboardController aktualisieren und Vertriebs-, Bestands- und Finanz-Panels hinzufügen**

Die folgenden Methoden zum bestehenden `DashboardController` hinzufügen:
...
Am Anfang des DashboardController fehlende `use`-Importe ergänzen:
```php
...
Registrierung der Client-Routen (innerhalb der `/api`-Gruppe in `config/route.php` anhängen):
...
## Task 15: Flutter- + HarmonyOS-Frontend-Seiten

**Seitenverzeichnis Flutter Web PC-Client:**
```
...
Jede Seite folgt dem bestehenden Flutter-Codemuster (GetX-Controller + ApiService-Singleton + Material-3-Theme).

**Seitenverzeichnis HarmonyOS:**
```
...
- [ ] **Step 1: Flutter-Beispiel — Artikellistenseite**
...
- [ ] **Step 2: HarmonyOS-Beispiel — Artikellistenseite**
...
## Task 16: Abschließende Verifizierung und Integrationstests

- [ ] **Step 1: PHP-Syntaxprüfung**
...
- [ ] **Step 3: Vollständigkeit der Routen verifizieren**
...
- [ ] **Step 4: Integritätsprüfung der Datenbanktabellen**
...
| Ebene | Anzahl | Dateiposition |
|------|------|----------|
| Datenbanktabellen | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 Kern- | `app/service/inventory/`, `app/service/finance/` |
| Routengruppen | 7 Module | `config/route.php` |
| Berechtigungseinträge | ~65 | Seed-SQL |
| Dashboard-Panels | 4 | DashboardController |
| Client-API | bedarfsweise erweiterbar | `app/api/v1/controller/` |
| Flutter-Seiten | 12+ | `apps/flutter/lib/app/pages/` |
| HarmonyOS-Seiten | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**Geschätzte Gesamtmenge an Code:** PHP ~8.000 Zeilen / SQL ~2.000 Zeilen / Dart ~3.000 Zeilen / ArkTS ~1.500 Zeilen

**Wichtige Design-Entscheidungen:**
1. Alle IDs werden auf API-Ebene verschlüsselt übertragen (hashids), die Datenbank speichert die rohen BIGINT-Werte
2. Sensible Felder werden auf Datenbankebene automatisch ver-/entschlüsselt (Encryptable-Cast)
3. Modulübergreifende Kopplung erfolgt über Ereignisse auf Service-Ebene, keine direkten gegenseitigen Aufrufe
4. Kostenkalkulation nach der Methode der bewegten gewichteten Durchschnittskosten
5. Forderungen/Verbindlichkeiten werden automatisch aus Einkaufs-/Verkaufsbelegen erzeugt, manuelle Verrechnung
6. Dashboard-Daten werden 5 Minuten in Redis gecacht

**Nicht abgedeckte Punkte (für spätere Iterationen):**
- Phase 2: Fertigungsmanagement, Projektmanagement
- WebSocket-Echtzeit-Push (Bestandswarnungen, Genehmigungserinnerungen)
- Multi-Mandanten-Umbau
- Feinere Berechtigungsgranularität (Datenberechtigungen)
- Offline-Modus für Mobilgeräte
