...
**Goal:** 13 Backend-Erweiterungen implementieren: 3 neue Middlewares (CORS, RateLimit, OperationLog), 6 neue Controller, 2 Modelländerungen, Routing + Middleware-Konfiguration.

**Architecture:** dem bestehenden webman-Muster folgen — Middlewares implementieren `MiddlewareInterface::process()`, Controller erben von `BaseController`, Routen werden per Closure oder `[class, method]` definiert. Globale Middlewares werden in `config/middleware.php` registriert, Routen-Middlewares an Routengruppen in `config/route.php` gehängt.
...
### Task 1: AdminUser-Modell um SoftDeletes + Searchable trait erweitern

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **Step 1: AdminUser-Modell ändern**

Nach Zeile 10 in `app/model/AdminUser.php` zwei `use`-Importe einfügen und am Anfang des Klassenrumpfs zwei traits verwenden:
...
Gleichzeitig am Klassenende (nach der `roles()`-Methode, vor der schließenden Klassenklammer) die Methode `toSearchableArray()` hinzufügen:
...
- [ ] **Step 1: Cors-Middleware erstellen**
...
- [ ] **Step 1: RateLimit-Middleware erstellen**
...
### Task 4: OperationLog-Middleware für Betriebsprotokolle

**Files:**
- Create: `app/middleware/OperationLog.php`

- [ ] **Step 1: OperationLog-Middleware erstellen**
...
### Task 5: Globale Middleware-Konfiguration

**Files:**
- Modify: `config/middleware.php`

- [ ] **Step 1: Globale Middlewares registrieren**

Den Inhalt von `config/middleware.php` ändern in:
...
- [ ] **Step 1: HealthController erstellen**
...
### Task 7: Systemkonfigurations-CRUD-Controller

**Files:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **Step 1: ConfigController erstellen**
...
### Task 8: Betriebsprotokoll-Abfragecontroller

**Files:**
- Create: `app/admin/controller/LogController.php`

- [ ] **Step 1: LogController erstellen**
...
### Task 9: Profil-Controller (inkl. Logout)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT-Blacklist-Prüfung)

- [ ] **Step 1: AdminAuth-Middleware ändern, JWT-Blacklist-Prüfung hinzufügen**
...
- [ ] **Step 2: ProfileController erstellen**
...
### Task 10: Datei-Upload-Controller

**Files:**
- Create: `app/admin/controller/UploadController.php`

- [ ] **Step 1: UploadController erstellen**
...
### Task 11: Batch-Operationen für Benutzer

**Files:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **Step 1: Zwei Methoden am Ende der UserController-Klasse hinzufügen (nach der `destroy`-Methode)**
...
- [ ] **Step 1: ImportController erstellen**
...
### Task 13: Routing-Update — alle neuen Routen + Middleware-Bindungen

**Files:**
- Modify: `config/route.php`

- [ ] **Step 1: Routenkonfiguration aktualisieren**

Innerhalb der `/admin`-Routengruppe neue Routen hinzufügen und gleichzeitig die OperationLog-Middleware anhängen. Vollständige Routendatei:
...
```bash
# 1. Syntaxprüfung aller Dateien
find app -name "*.php" -newer app/model/AdminUser.php -exec php -l {} \;
php -l config/route.php
php -l config/middleware.php

# 2. Dienst starten und verifizieren
