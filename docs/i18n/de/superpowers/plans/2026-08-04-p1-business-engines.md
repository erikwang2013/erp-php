# P1 Business-Tiefen-Engines — Implementierungsplan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) to implement this plan task-by-task.

**Goal:** Die drei Kernmodule Finanzwesen, Gehalt und MRP von CRUD-Gerüsten zu industriellen Business-Berechnungs-Engines aufrüsten

**Architecture:** Jede Engine ist eine eigenständige Service-Klasse, folgt der Verzeichnis-Konvention `app/service/{module}/`, injiziert Abhängigkeiten über den Konstruktor, `DB::transaction()` garantiert Datenkonsistenz, PHPUnit-Tests decken die Kernalgorithmen ab

**Tech Stack:** PHP 8.3, workerman/webman-framework 2.x, Eloquent ORM, PHPUnit 12

---

## Task 1: Finanz-Doppik-Engine (doppelte Buchführung)
...
Der vorhandene `FinanceVoucher` macht nur einfaches CRUD, ohne Soll/Haben-Gleichgewichtsprüfung. Die Tabelle `FinanceVoucherItem` hat bereits die Felder `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary`. Kernregel: „Jede Sollbuchung hat eine Habenbuchung, Soll und Haben müssen stets gleich sein."

- [ ] **Step 1: DoubleEntryService erstellen**
...
- [ ] **Step 3: VoucherController-Integration**

In `store()` von `app/controller/finance/VoucherController.php`: wenn der Request `items` enthält, `DoubleEntryService::createVoucher()` aufrufen, sonst auf das ursprüngliche CRUD zurückfallen.
...
## Task 4: Integration in vorhandene Controller
...
- [ ] **Step 1: VoucherController-Integration** — `store()` prüft den `items`-Parameter; vorhanden → `DoubleEntryService`, sonst → ursprüngliches CRUD
- [ ] **Step 2: SalaryController-Integration** — `store()` ruft `SalaryEngineService::calculate()` auf und berechnet die Gehaltsdetails automatisch
- [ ] **Step 3: MrpController-Integration** — `generate()` ruft `MrpEngineService::explodeBom()` + `calculateNetRequirement()` auf
...
- [ ] Belegspeicherung mit ungleichem Soll/Haben → 422 „Soll/Haben ungleich"
- [ ] `SalaryEngineService::calculate(10000)` → gross=10000, si=1050, hf=700, tax korrekt berechnet
- [ ] `MrpEngineService::calculateNetRequirement(100, 30, 0, 0, 20)` → 90
- [ ] BOM wird mehrstufig rekursiv bis zu den Rohstoffen expandiert, Verlustrate wird berücksichtigt
- [ ] `php vendor/bin/phpunit --testdox` läuft vollständig durch
