# P1 Motores de profundidade de negócio — Plano de implementação

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) to implement this plan task-by-task.

**Goal:** Elevar os três módulos principais — financeiro, salários e MRP — de esqueletos CRUD para motores de cálculo de negócio de nível industrial

**Architecture:** Cada motor é uma classe Service independente, seguindo a convenção de diretório `app/service/{module}/`, com injeção de dependências via construtor, `DB::transaction()` garantindo a consistência dos dados, testes PHPUnit cobrindo os algoritmos críticos

**Tech Stack:** PHP 8.3, workerman/webman-framework 2.x, Eloquent ORM, PHPUnit 12

---

## Task 1: Motor de lançamento contábil em partida dobrada
...
O `FinanceVoucher` atual faz apenas CRUD simples, sem validação de equilíbrio débito/crédito. A tabela `FinanceVoucherItem` já possui os campos `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary`. Regra central: «para todo débito há um crédito correspondente; débitos e créditos devem ser iguais».

- [ ] **Step 1: Criar o DoubleEntryService**
...
- [ ] **Step 3: Integração no VoucherController**

Modificar o `store()` de `app/controller/finance/VoucherController.php`: se a requisição contiver `items`, chamar `DoubleEntryService::createVoucher()`; caso contrário, manter o CRUD original.
...
- [ ] **Step 4: Executar os testes e fazer commit**
...
- [ ] **Step 3: Executar os testes e fazer commit**
...
## Task 3: Motor de planejamento de necessidades de materiais MRP

**Files:**
- Create: `app/service/manufacturing/MrpEngineService.php`
- Create: `tests/MrpEngineServiceTest.php`

- [ ] **Step 1: Criar o MrpEngineService**
...
## Task 4: Integração nos controladores existentes
...
- [ ] **Step 1: Integração no VoucherController** — o `store()` detecta o parâmetro `items`; se presente, usa o `DoubleEntryService`; caso contrário, mantém o CRUD original
- [ ] **Step 2: Integração no SalaryController** — o `store()` chama `SalaryEngineService::calculate()` para calcular automaticamente os detalhes do salário
- [ ] **Step 3: Integração no MrpController** — o `generate()` chama `MrpEngineService::explodeBom()` + `calculateNetRequirement()`
- [ ] **Step 4: Executar todos os testes** — `php vendor/bin/phpunit --testdox`
- [ ] **Step 5: Commit**
...
- [ ] Salvar voucher com débito/crédito desiguais → 422 «借贷不平衡»
...
- [ ] Expansão BOM em múltiplos níveis recursivos até as matérias-primas, incluindo taxa de perda
- [ ] `php vendor/bin/phpunit --testdox` passando por completo
