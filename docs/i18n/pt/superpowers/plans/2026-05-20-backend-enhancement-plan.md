# Aprimoramentos de back-end — Plano de implementação

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implementar 13 aprimoramentos de back-end: 3 novos middlewares (CORS, RateLimit, OperationLog), 6 novos controladores, 2 modificações de modelos, roteamento + configuração de middlewares.

**Architecture:** Seguir os padrões existentes do webman — middlewares implementam `MiddlewareInterface::process()`, controladores herdam de `BaseController`, rotas definidas com closures ou `[class, method]`. Middlewares globais são registrados em `config/middleware.php`, middlewares de rota são montados nos grupos de rotas de `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (extensão), PhpSpreadsheet (importação)

**Ordem de dependências:** Modificação de modelos → middlewares → controladores → rotas/configuração

---

### Task 1: Modelo AdminUser com SoftDeletes + Searchable trait
...
Inserir dois `use` imports após a linha 10 de `app/model/AdminUser.php` e usar os dois traits no início do corpo da classe:
...
Também adicionar o método `toSearchableArray()` no final da classe (após o método `roles()`, antes da chave de fechamento da classe):
...
### Task 3: Middleware de rate limit RateLimit
...
Alterar o conteúdo de `config/middleware.php` para:
...
### Task 6: Controlador de health check
...
### Task 7: Controlador CRUD de configurações do sistema
...
### Task 8: Controlador de consulta de logs de operações
...
### Task 9: Controlador do centro pessoal (inclui logout)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (validação de blacklist JWT)

- [ ] **Step 1: Modificar o middleware AdminAuth para adicionar a validação de blacklist JWT**

No método `AdminAuth::process()`, inserir a verificação de blacklist após a extração do token e antes da decodificação do JWT:
...
Também é necessário adicionar o import do Redis. Modificar o cabeçalho do arquivo:
...
### Task 10: Controlador de upload de arquivos
...
- [ ] **Step 1: Adicionar dois métodos no final da classe UserController (após o método `destroy`)**
...
### Task 12: Controlador de importação Excel
...
### Task 13: Atualização das rotas — todas as novas rotas + vínculo de middlewares
...
Adicionar novas rotas dentro do grupo de rotas `/admin` e anexar o middleware OperationLog. Arquivo de rotas completo:
...
Após a implementação, verificar item por item:
