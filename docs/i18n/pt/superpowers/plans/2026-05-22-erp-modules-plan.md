# Plano de implementação dos módulos de negócio ERP

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Objetivo:** implementar os módulos completos de negócio ERP de compras, vendas, inventário + financeiro + CRM sobre a base de administração do sistema webman v2 existente

**Arquitetura:** três camadas Controller → Service → Model. Todos os models compartilham `app/model/`, os controllers são organizados em diretórios por módulo `app/controller/{module}/`, e a lógica de negócio fica em `app/service/{module}/`. Cerca de 50 tabelas no banco de dados com prefixo unificado `erik_` e chaves primárias BIGINT snowflake. A versão da API é controlada pelo cabeçalho de requisição `API-Version` + encaminhada pelo middleware `ApiVersion`.

**Stack de tecnologia:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / pacotes erikwang2013/*
...
- Versão da API: cabeçalho `API-Version` (ex.: `v1`, `v2`), ausência assume `v1`, não aparece na URL

---

## Task 1: Migração de banco de dados — Dados básicos de produtos

**Files:**
- Create: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Step 1: Criar o SQL de migração de dados básicos de produtos**
...
## Task 2: Migração de banco de dados — Módulo de compras

**Files:**
- Create: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Step 1: SQL de migração das 9 tabelas do módulo de compras**
...
## Task 3: Migração de banco de dados — Módulo de vendas

**Files:**
- Create: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Step 1: SQL de migração das 9 tabelas do módulo de vendas**
...
## Task 4: Migração de banco de dados — Módulos de inventário + financeiro + CRM
...
- [ ] **Step 1: 11 tabelas do módulo de inventário**
...
- [ ] **Step 2: 11 tabelas do módulo financeiro**
...
- [ ] **Step 3: 4 tabelas do módulo CRM**
...
- [ ] **Step 4: Executar todas as migrações**
...
## Task 5: Models de dados básicos de produtos
...
- [ ] **Step 2: Criar os demais Models de dados básicos (seguindo o mesmo padrão)**
...
## Task 6: Models de compras + vendas
...
- [ ] **Step 1: Criar todos os Models de compras e vendas**

Todos os models seguem o mesmo padrão: trait `SoftDeletes` (tabelas principais e classes de pedidos), `$incrementing = false; $keyType = 'int';`, relações BelongsTo. Abaixo apenas o PurchaseOrder é mostrado (o mais representativo):
...
**Comando para criar os demais Models de compras/vendas:**
...
Como os Models de compras e vendas totalizam 19 e seguem um padrão uniforme, são criados em lote por script. Os `$fillable`, `$casts` e métodos de relação específicos de cada Model serão complementados conforme necessário na implementação dos Controllers posteriores.
...
## Task 7: Models de inventário + financeiro + CRM
...
- [ ] **Step 1: Criar os Models de inventário/financeiro/CRM em lote**
...
- [ ] **Step 2: Complementar $fillable e relações para os Models principais**
...
## Task 8: Controllers de dados básicos de produtos
...
Todos os controllers herdam de `app\admin\controller\BaseController`, com padrão CRUD consistente com o `UserController` existente. Abaixo, o exemplo completo com `ProductController`:

- [ ] **Step 1: Criar o ProductController**
...
- [ ] **Step 2: Criar os demais Controllers de dados básicos**

Os outros 6 controllers (Category, Brand, Warehouse, Location, Supplier, Customer) são CRUDs padrão, com a mesma estrutura acima. Após criados, escreva em lote nos arquivos:
...
## Task 9: Serviço de inventário — lógica principal de entrada/saída de estoque

Este é o núcleo da integração entre módulos; recebimento de compras/envio de vendas etc. dependem dele.

**Files:**
- Create: `app/service/inventory/InventoryService.php`

- [ ] **Step 1: Criar o InventoryService**
...
- [ ] **Step 2: Criar teste para verificar o serviço de inventário**
...
## Task 10: Serviço financeiro — geração automática e conciliação de contas a receber/pagar

**Files:**
- Create: `app/service/finance/FinanceService.php`

- [ ] **Step 1: Criar o FinanceService**
...
## Task 11: Controller de recebimento de compras (exemplo de integração entre módulos)

Esta é a operação de negócio mais típica entre módulos: recebimento → entrada automática no estoque → geração de conta a pagar. Mostra o código completo de integração.

**Files:**
- Create: `app/controller/purchase/ReceiveController.php`

- [ ] **Step 1: Criar o controller de recebimento de compras**
...
## Task 12: Extensão de rotas + seeds de permissões

**Files:**
- Modify: `config/route.php`
- Create: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Step 1: Estender o arquivo de rotas**

Dentro do grupo de rotas `/admin` em `config/route.php` (antes de `->middleware([...])`), adicione as seguintes rotas:
...
- [ ] **Step 2: Criar o SQL de seeds de permissões do ERP**
...
- [ ] **Step 3: Verificar a sintaxe das rotas**
...
## Task 13: Extensão dos painéis do dashboard

**Files:**
- Modify: `app/admin/controller/DashboardController.php`

- [ ] **Step 1: Atualizar o DashboardController adicionando os painéis de vendas, inventário e financeiro**

Adicione os seguintes métodos ao `DashboardController` existente:
...
É necessário adicionar os `use` no topo do DashboardController:
```php
...
Registro das rotas do cliente (adicionar dentro do grupo `/api` em `config/route.php`):
...
## Task 15: Páginas de frontend Flutter + HarmonyOS

**Diretório de páginas Flutter Web (PC):**
```
...
Cada página segue o padrão de código Flutter existente (Controller GetX + singleton ApiService + tema Material 3).

**Diretório de páginas HarmonyOS:**
```
...
- [ ] **Step 1: Exemplo Flutter — página de listagem de produtos**
...
- [ ] **Step 2: Exemplo HarmonyOS — página de listagem de produtos**
...
## Task 16: Verificação final e testes de integração

- [ ] **Step 1: Verificação de sintaxe PHP**
...
- [ ] **Step 2: Executar os testes existentes**
...
- [ ] **Step 3: Verificar a integridade das rotas**
...
- [ ] **Step 4: Verificar a integridade das tabelas do banco de dados**
...
| Camada | Quantidade | Local dos arquivos |
|------|------|----------|
| Tabelas do banco de dados | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 principais | `app/service/inventory/`, `app/service/finance/` |
| Grupos de rotas | 7 módulos | `config/route.php` |
| Entradas de permissão | ~65 | SQL de seed |
| Painéis do dashboard | 4 | DashboardController |
| API do cliente | expansível sob demanda | `app/api/v1/controller/` |
| Páginas Flutter | 12+ | `apps/flutter/lib/app/pages/` |
| Páginas HarmonyOS | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**Estimativa total de código:** PHP ~8.000 linhas / SQL ~2.000 linhas / Dart ~3.000 linhas / ArkTS ~1.500 linhas

**Decisões de design principais:**
1. Todos os IDs são transmitidos criptografados na camada de API (hashids); o banco armazena os BIGINT originais
2. Campos sensíveis são criptografados/descriptografados automaticamente na camada do banco (cast Encryptable)
3. A integração entre módulos é disparada por eventos na camada de Service, sem chamadas diretas entre módulos
4. O cálculo de custo usa o método de média móvel ponderada
5. Contas a receber/pagar são geradas automaticamente a partir de documentos de compras/vendas, com conciliação manual
6. Os dados do dashboard são armazenados em cache Redis por 5 minutos

**Itens não cobertos (para iterações futuras):**
- Fase 2: gestão de manufatura, gestão de projetos
- Push em tempo real via WebSocket (alertas de inventário, lembretes de aprovação)
- Adaptação multi-tenant
- Granularidade de permissão mais rica (permissões de dados)
