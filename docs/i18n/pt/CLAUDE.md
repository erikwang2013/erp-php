...
**Pontuação geral atual 89/100** — o roadmap completo P0~P3 foi concluído, com cobertura full-stack de 23 módulos, pronto para produção.

| Fase | Prazo | Entregáveis | Status |
|------|------|--------|------|
| 🔵 **P0** Ecossistema front-end | 3-4 semanas | 102 rotas de menu Flutter + 41 páginas HarmonyOS + 4 componentes comuns | ✅ |
...
**Testes**: 1048<!-- stats:tests=1048 --> tests, 5040<!-- stats:assertions=5040 --> assertions (23 skipped) — ALL PASSING. **Flutter**: 0 errors, 0 warnings.
...
| Segurança | 7 camadas de defesa em profundidade (XSS/Injeção SQL/CSRF/rate limit/CSP...) |
...
- Geração de documentação da API: `erikwang2013/apidoc-php` | baseada em anotações, acesse /apidoc
...
- Angular 22 CLI + ng-zorro-antd, diretório de código-fonte `apps/angular/` (painel administrativo Web)
- React 19 + Vite, diretório de código-fonte `apps/react/` (painel administrativo Web)
- Quatro frontends sobre um mesmo backend: Angular / React, assim como o Flutter, usam `/admin/v1`, `/api/v1` e `/open/v1`; em desenvolvimento cada dev server faz proxy para o webman

### Internacionalização (13 idiomas)
- Lista de idiomas: `zh_CN` `en` `ja` `ko` `de` `fr` `es` `pt` `ru` `ar` `hi` `bn` `id`
- Dicionário do backend: `resource/translations/<locale>/{common,modules,validation}.php`, 13 diretórios de idioma; `zh_CN` com 565 entradas, cada um dos outros 11 idiomas com 544 entradas, `en` com 30 (critério: entradas folha dos três arquivos; os rótulos do campo `attributes` de `validation.php` entram na conta, as chaves de grupo não)
  - "inglês é a chave": common/modules de `en` ficam vazios; as chaves de `validation.php` são nomes de regras do framework e apenas os valores são traduzidos
  - Gerador: `scripts/gen-be-locales.mjs`
- Dicionário do frontend (Angular): fonte `apps/angular/src/app/core/zh-en/part1..4.ts` (1456 entradas) → artefato `apps/angular/src/app/core/zh-<code>.ts`
- Dicionário do frontend (React): fonte `apps/react/src/lib/i18n/zhEn.ts` (1451 entradas) → artefato `apps/react/src/lib/i18n/zh<Code>.ts`
  - Os dicionários dos 11 novos idiomas são carregados dinamicamente com `import()`, cada um como chunk independente; entradas ausentes caem de volta para o texto original em chinês
  - Gerador: `scripts/gen-fe-locales.mjs --app angular|react`
- Em tempo de execução: trocar o idioma troca o cabeçalho `Accept-Language`, e o backend devolve o texto conforme o idioma (`app/common/I18n.php` + `config/translation.php`)
...
A versão fica no caminho da URL (`/admin/v1`, `/api/v1`, `/open/v1`), sem cabeçalho de versão:

```bash
curl http://localhost:8788/api/v1/auth/login
```

Para adicionar uma nova versão, basta criar o diretório `app/api/{version}/controller/` e registrar um novo grupo de rotas `/api/v{version}` em `config/route.php` (a versão aparece apenas no caminho da URL, vinculada diretamente ao controlador, sem middleware de cabeçalho de versão — o antigo middleware `ApiVersion` foi removido).
...
- Refresh transparente de Token: em 401, chama automaticamente `/api/v1/auth/refresh`
- Falha no refresh redireciona automaticamente para a página de login

## Dívida técnica conhecida

> A lista abaixo foi medida com `grep -rn "new .*Service(" app/controller/` (45 ocorrências) e é consistente com os fatos do código.
> **P5 não refatora**: instanciar serviços diretamente no controller é o padrão existente; apenas em código novo passe a usar injeção via container (`support\Container`), mantendo o código legado como está.

| Módulo | Serviços instanciados diretamente | Observação |
|------|-----------|------|
| finance | 22 | Contas a receber/pagar, baixa, diário, fechamento, relatórios consolidados |
| wms | 9 | Serviços de fluxo como recebimento, putaway, ondas, picking, embalagem |
| tms | 5 | Conhecimentos de transporte, comparação de preços, rastreamento, faturas de frete |
| oms | 3 | Atendimento, reserva, RMA |
| quality | 2 | Inspeção, tratamento de produtos não conformes |
| hr | 2 | Salários, ponto |
| platform | 1 | Tenant |
| notification | 1 | Canais de notificação |
