# P1 व्यावसायिक गहराई इंजन — कार्यान्वयन योजना

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) to implement this plan task-by-task.

**Goal:** वित्त, वेतन, MRP तीन मुख्य मॉड्यूल को CRUD ढांचे से औद्योगिक-स्तरीय व्यावसायिक गणना इंजन में अपग्रेड करना

**Architecture:** प्रत्येक इंजन स्वतंत्र Service वर्ग के रूप में, `app/service/{module}/` निर्देशिका परंपरा का पालन करता है, कंस्ट्रक्टर से निर्भरताएँ इंजेक्ट होती हैं, DB::transaction() डेटा स्थिरता सुनिश्चित करता है, PHPUnit परीक्षण मुख्य एल्गोरिदम को कवर करते हैं

**Tech Stack:** PHP 8.3, workerman/webman-framework 2.x, Eloquent ORM, PHPUnit 12

---

## Task 1: वित्त दोहरी प्रविष्टि (Double-Entry) लेखा इंजन
...
मौजूदा `FinanceVoucher` केवल साधारण CRUD करता है, बिना डेबिट-क्रेडिट संतुलन जाँच के। `FinanceVoucherItem` तालिका में पहले से `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary` फ़ील्ड हैं। मुख्य नियम: «हर डेबिट के लिए क्रेडिट, डेबिट और क्रेडिट बराबर होने चाहिए»।

- [ ] **Step 1: DoubleEntryService बनाएँ**
...
- [ ] **Step 3: VoucherController एकीकरण**

`app/controller/finance/VoucherController.php` की `store()` में बदलाव: यदि अनुरोध में `items` है तो `DoubleEntryService::createVoucher()` कॉल करें, अन्यथा पुराने CRUD पर वापस जाएँ।
...
## Task 4: मौजूदा कंट्रोलर में एकीकरण
...
- [ ] **Step 1: VoucherController एकीकरण** — `store()` `items` पैरामीटर जाँचता है, होने पर `DoubleEntryService` का उपयोग, नहीं होने पर पुराना CRUD
- [ ] **Step 2: SalaryController एकीकरण** — `store()` `SalaryEngineService::calculate()` कॉल करके वेतन विवरण स्वतः गणना
- [ ] **Step 3: MrpController एकीकरण** — `generate()` `MrpEngineService::explodeBom()` + `calculateNetRequirement()` कॉल करता है
...
- [ ] वाउचर सहेजते समय डेबिट-क्रेडिट असमान → 422 «डेबिट-क्रेडिट असंतुलित»
