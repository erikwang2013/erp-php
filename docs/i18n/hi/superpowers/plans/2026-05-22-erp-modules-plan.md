# ERP व्यावसायिक मॉड्यूल कार्यान्वयन योजना

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** मौजूदा webman v2 सिस्टम प्रशासन आधार पर पूर्ण ERP व्यावसायिक मॉड्यूल लागू करें — खरीद-बिक्री-स्टॉक + वित्त + CRM

**Architecture:** Controller → Service → Model तीन परतें। सभी मॉडल `app/model/` साझा करते हैं, कंट्रोलर मॉड्यूल अनुसार निर्देशिका `app/controller/{module}/` में, व्यावसायिक तर्क `app/service/{module}/` में। डेटाबेस की ~50 तालिकाएँ एकसमान `erik_` उपसर्ग, BIGINT snowflake प्राथमिक कुंजी। API संस्करण `API-Version` अनुरोध हेडर + `ApiVersion` मिडलवेयर से फ़ॉरवर्ड होता है।

**Tech Stack:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / erikwang2013/* पैकेज श्रृंखला
...
- API संस्करण: अनुरोध हेडर `API-Version` (जैसे `v1`, `v2`), अनुपलब्ध होने पर डिफ़ॉल्ट `v1`, URL में प्रदर्शित नहीं

---

## Task 1: डेटाबेस माइग्रेशन — उत्पाद आधार डेटा

**Files:**
- Create: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Step 1: उत्पाद आधार डेटा माइग्रेशन SQL बनाएँ**
...
- [ ] **Step 2: निष्पादित करें और सत्यापित करें**
...
## Task 2: डेटाबेस माइग्रेशन — क्रय मॉड्यूल

**Files:**
- Create: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Step 1: क्रय मॉड्यूल की 9 तालिकाओं का माइग्रेशन SQL**
...
- [ ] **Step 2: माइग्रेशन निष्पादित करें**
...
## Task 3: डेटाबेस माइग्रेशन — विक्रय मॉड्यूल

**Files:**
- Create: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Step 1: विक्रय मॉड्यूल की 9 तालिकाओं का माइग्रेशन SQL**
...
## Task 4: डेटाबेस माइग्रेशन — इन्वेंटरी + वित्त + CRM मॉड्यूल
...
- [ ] **Step 1: इन्वेंटरी मॉड्यूल की 11 तालिकाएँ**
...
- [ ] **Step 2: वित्त मॉड्यूल की 11 तालिकाएँ**
...
- [ ] **Step 3: CRM मॉड्यूल की 4 तालिकाएँ**
...
- [ ] **Step 4: सभी माइग्रेशन निष्पादित करें**
...
## Task 5: उत्पाद आधार डेटा Models
...
- [ ] **Step 2: शेष आधार डेटा Models बनाएँ (समान पैटर्न के अनुसार)**
...
- [ ] **Step 1: सभी क्रय और विक्रय Models बनाएँ**

सभी models एक ही पैटर्न का पालन करते हैं: `SoftDeletes` trait (मुख्य तालिकाएँ और ऑर्डर वर्ग), `$incrementing = false; $keyType = 'int';`, BelongsTo संबंध। नीचे केवल PurchaseOrder (सबसे प्रतिनिधि) दिखाया गया है:
...
**शेष क्रय/विक्रय Models बनाने के आदेश:**
...
क्योंकि क्रय और विक्रय Models कुल 19 हैं और पैटर्न एकसमान है, स्क्रिप्ट से बैच में बनाए गए। प्रत्येक Model के विशिष्ट `$fillable`, `$casts` और संबंध विधियाँ आगे Controller कार्यान्वयन में आवश्यकता अनुसार जोड़ी जाएँगी।
...
## Task 7: इन्वेंटरी + वित्त + CRM Models
...
- [ ] **Step 1: इन्वेंटरी/वित्त/CRM Models बैच में बनाएँ**
...
- [ ] **Step 2: मुख्य Model के लिए $fillable और संबंध पूरा करें**
...
## Task 8: उत्पाद आधार डेटा Controllers
...
सभी कंट्रोलर `app\admin\controller\BaseController` से विरासत में लेते हैं, CRUD पैटर्न मौजूदा `UserController` के अनुरूप है। नीचे `ProductController` का पूर्ण उदाहरण:

- [ ] **Step 1: ProductController बनाएँ**
...
- [ ] **Step 2: शेष आधार डेटा Controllers बनाएँ**

शेष 6 कंट्रोलर (Category, Brand, Warehouse, Location, Supplier, Customer) सभी मानक CRUD हैं, संरचना ऊपर जैसी ही। बनाने के बाद फ़ाइलों में बैच लिखें:
...
## Task 9: इन्वेंटरी सेवा — इनबाउंड/आउटबाउंड मुख्य तर्क

यह क्रॉस-मॉड्यूल एकीकरण का मूल है, क्रय प्राप्ति/विक्रय डिलीवरी आदि सभी इस पर निर्भर हैं।

**Files:**
- Create: `app/service/inventory/InventoryService.php`

- [ ] **Step 1: InventoryService बनाएँ**
...
- [ ] **Step 2: इन्वेंटरी सेवा सत्यापित करने के लिए परीक्षण बनाएँ**
...
## Task 10: वित्त सेवा — प्राप्य/देय स्वतः जनरेशन और निपटान

**Files:**
- Create: `app/service/finance/FinanceService.php`

- [ ] **Step 1: FinanceService बनाएँ**
...
## Task 11: क्रय प्राप्ति Controller (क्रॉस-मॉड्यूल एकीकरण उदाहरण)

यह सबसे विशिष्ट क्रॉस-मॉड्यूल व्यावसायिक ऑपरेशन है: प्राप्ति → स्वतः इनबाउंड → देय उत्पन्न। पूर्ण एकीकरण कोड दिखाया गया है।

**Files:**
- Create: `app/controller/purchase/ReceiveController.php`

- [ ] **Step 1: क्रय प्राप्ति कंट्रोलर बनाएँ**
...
## Task 12: रूट विस्तार + अनुमति सीड

**Files:**
- Modify: `config/route.php`
- Create: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Step 1: रूट फ़ाइल विस्तारित करें**

`config/route.php` के `/admin` रूट समूह में (`->middleware([...])` से पहले), निम्न रूट जोड़ें:
...
- [ ] **Step 2: ERP अनुमति सीड SQL बनाएँ**
...
- [ ] **Step 3: रूट सिंटैक्स सत्यापित करें**
...
## Task 13: डैशबोर्ड पैनल विस्तार

**Files:**
- Modify: `app/admin/controller/DashboardController.php`

- [ ] **Step 1: DashboardController अपडेट करें — विक्रय, इन्वेंटरी, वित्त पैनल जोड़ें**

मौजूदा `DashboardController` में निम्न विधियाँ जोड़ें:
...
DashboardController के शीर्ष पर `use` जोड़ना होगा:
```php
...
क्लाइंट रूट पंजीकरण (`config/route.php` के `/api` समूह में जोड़ें):
...
## Task 15: Flutter + HarmonyOS फ्रंटएंड पेज

**Flutter Web PC पेज निर्देशिका:**
```
...
प्रत्येक पेज मौजूदा Flutter कोड पैटर्न (GetX Controller + ApiService सिंगलटन + Material 3 थीम) का अनुसरण करता है।

**HarmonyOS पेज निर्देशिका:**
```
...
- [ ] **Step 1: Flutter उदाहरण — उत्पाद सूची पेज**
...
- [ ] **Step 2: HarmonyOS उदाहरण — उत्पाद सूची पेज**
...
## Task 16: अंतिम सत्यापन और एकीकरण परीक्षण

- [ ] **Step 1: PHP सिंटैक्स जांच**
...
- [ ] **Step 3: रूट पूर्णता सत्यापित करें**
...
- [ ] **Step 4: डेटाबेस तालिका पूर्णता जांच**
...
| परत | संख्या | फ़ाइल स्थान |
|------|------|----------|
| डेटाबेस तालिकाएँ | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 मुख्य | `app/service/inventory/`, `app/service/finance/` |
| रूट समूह | 7 मॉड्यूल | `config/route.php` |
| अनुमति प्रविष्टियाँ | ~65 | seed SQL |
| डैशबोर्ड पैनल | 4 | DashboardController |
| क्लाइंट API | आवश्यकता अनुसार विस्तार | `app/api/v1/controller/` |
| Flutter पेज | 12+ | `apps/flutter/lib/app/pages/` |
| HarmonyOS पेज | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**कुल कोड मात्रा अनुमान:** PHP ~8,000 पंक्तियाँ / SQL ~2,000 पंक्तियाँ / Dart ~3,000 पंक्तियाँ / ArkTS ~1,500 पंक्तियाँ

**मुख्य डिज़ाइन निर्णय:**
1. सभी ID API परत पर एन्क्रिप्टेड ट्रांसमिट होते हैं (hashids), डेटाबेस में मूल BIGINT संग्रहीत
2. संवेदनशील फ़ील्ड डेटाबेस स्तर पर स्वतः एन्क्रिप्ट/डिक्रिप्ट (Encryptable cast)
3. क्रॉस-मॉड्यूल एकीकरण Service परत इवेंट से ट्रिगर होता है, सीधे एक-दूसरे को कॉल नहीं
4. लागत गणना मूविंग वेटेड एवरेज विधि से
5. प्राप्य/देय क्रय/विक्रय दस्तावेज़ों से स्वतः उत्पन्न, मैनुअल निपटान
6. डैशबोर्ड डेटा Redis कैश 5 मिनट

**कवर नहीं किए गए आइटम (बाद के पुनरावृत्ति में):**
- Phase 2: विनिर्माण प्रबंधन, परियोजना प्रबंधन
- WebSocket रीयल-टाइम पुश (इन्वेंटरी अलर्ट, अनुमोदन सूचना)
- मल्टी-टेनेंट रूपांतरण
- अधिक समृद्ध अनुमति ग्रैन्युलैरिटी (डेटा अनुमति)
- मोबाइल ऑफ़लाइन मोड
