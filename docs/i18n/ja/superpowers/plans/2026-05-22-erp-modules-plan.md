...
**ゴール:** 既存の webman v2 システム管理基盤の上に、仕入れ・在庫・販売（進銷存）+ 財務 + CRM の完全な ERP 業務モジュールを実装する

**アーキテクチャ:** Controller → Service → Model の 3 層構成。すべてのモデルは `app/model/` を共有し、コントローラはモジュールごとに `app/controller/{module}/` に分け、ビジネスロジックは `app/service/{module}/` に置く。データベースは約 50 テーブルを統一 `erik_` プレフィックス、BIGINT snowflake 主キーとする。API バージョンは `API-Version` リクエストヘッダー + `ApiVersion` ミドルウェアで転送する。

**技術スタック:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / erikwang2013/* シリーズパッケージ

**Codebase パターン**（既存コードより）:
- コントローラは `app\admin\controller\BaseController` を継承 — `success()`、`fail()`、`encodeId()`、`decodeId()`、`encodeIds()`、`generateId()`、`confirmPassword()` を提供
- モデルは `support\Model` に `SoftDeletes`、`Searchable`、`Encryptable` cast を使用
- 主キー: `$incrementing = false; $keyType = 'int';`
- ルート: `/admin` グループ配下で `Route::resource()` と明示的なルートを完全なミドルウェアチェーン付きで使用
- 権限: `slug` 形式 `method.admin/module/action`（例: `get.admin/product`）
- バリデーション: webman 組み込みの `validator()` 関数
- レスポンス形式: `{code: 0, message: 'success', data: {...}}`
- API バージョン: リクエストヘッダー `API-Version`（例: `v1`、`v2`）、欠落時はデフォルト `v1`、URL には含めない

---

## Task 1: データベースマイグレーション — 商品基礎データ

**ファイル:**
- 作成: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Step 1: 商品基礎データマイグレーション SQL を作成**
...
## Task 2: データベースマイグレーション — 仕入れ（採購）モジュール

**ファイル:**
- 作成: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Step 1: 仕入れモジュール 9 テーブルのマイグレーション SQL**
...
## Task 3: データベースマイグレーション — 販売（銷售）モジュール

**ファイル:**
- 作成: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Step 1: 販売モジュール 9 テーブルのマイグレーション SQL**
...
## Task 4: データベースマイグレーション — 在庫 + 財務 + CRM モジュール

**ファイル:**
- 作成: `database/migrations/2026_05_22_000006_inventory_tables.sql`
- 作成: `database/migrations/2026_05_22_000007_finance_tables.sql`
- 作成: `database/migrations/2026_05_22_000008_crm_tables.sql`

- [ ] **Step 1: 在庫モジュール 11 テーブル**
...
## Task 5: 商品基礎データ Models

**ファイル:**
- 作成: `app/model/Product.php`
- 作成: `app/model/ProductSku.php`
- 作成: `app/model/ProductUnit.php`
- 作成: `app/model/ProductPrice.php`
- 作成: `app/model/Category.php`
- 作成: `app/model/Brand.php`
- 作成: `app/model/Warehouse.php`
- 作成: `app/model/Location.php`
- 作成: `app/model/Supplier.php`
- 作成: `app/model/Customer.php`
- 作成: `app/model/CustomerLevel.php`

- [ ] **Step 1: Product Model を作成**
...
- [ ] **Step 2: 残りの基礎データ Models を作成（同じパターンで）**
...
## Task 6: 仕入れ（採購）+ 販売（銷售）Models

**ファイル:**
- 作成: `app/model/PurchaseApply.php`, `app/model/PurchaseApplyItem.php`
- 作成: `app/model/PurchaseOrder.php`, `app/model/PurchaseOrderItem.php`
- 作成: `app/model/PurchaseReceive.php`, `app/model/PurchaseReceiveItem.php`
- 作成: `app/model/PurchaseReturn.php`, `app/model/PurchaseReturnItem.php`
- 作成: `app/model/PurchaseSettlement.php`
- 作成: `app/model/SalesQuotation.php`, `app/model/SalesQuotationItem.php`
- 作成: `app/model/SalesOrder.php`, `app/model/SalesOrderItem.php`
- 作成: `app/model/SalesDelivery.php`, `app/model/SalesDeliveryItem.php`
- 作成: `app/model/SalesReturn.php`, `app/model/SalesReturnItem.php`
- 作成: `app/model/SalesSettlement.php`

- [ ] **Step 1: すべての仕入れ・販売 Models を作成**

すべてのモデルは同じパターンに従います: `SoftDeletes` trait（主テーブルと注文クラス）、`$incrementing = false; $keyType = 'int';`、BelongsTo リレーション。ここでは最も代表的な PurchaseOrder のみ示します:
...
仕入れ・販売 Models は計 19 個でパターンが統一されているため、スクリプトで一括作成します。各 Model の具体的な `$fillable`、`$casts` とリレーションメソッドは、以降の Controller 実装時に必要に応じて補足します。
...
## Task 7: 在庫 + 財務 + CRM Models

**ファイル:**
- 作成: `app/model/Inventory.php`, `app/model/InventoryBatch.php`, `app/model/InventorySerial.php`
- 作成: `app/model/InventoryFlow.php`, `app/model/Transfer.php`, `app/model/TransferItem.php`
- 作成: `app/model/CheckTask.php`, `app/model/CheckDetail.php`
- 作成: `app/model/InventoryAlertRule.php`, `app/model/InventoryAlertLog.php`, `app/model/CostRecord.php`
- 作成: `app/model/FinanceAccount.php`, `app/model/FinanceVoucher.php`, `app/model/FinanceVoucherItem.php`
- 作成: `app/model/FinanceArAp.php`, `app/model/FinanceBankAccount.php`
- 作成: `app/model/FinanceReceipt.php`, `app/model/FinancePayment.php`, `app/model/FinanceSettlement.php`
- 作成: `app/model/FinanceCashJournal.php`, `app/model/FinanceExpense.php`, `app/model/FinanceProfit.php`
- 作成: `app/model/CrmFunnelStage.php`, `app/model/CrmOpportunity.php`
- 作成: `app/model/CrmFollowRecord.php`, `app/model/CrmContact.php`

- [ ] **Step 1: 在庫/財務/CRM Models を一括作成**
...
- [ ] **Step 2: 主要 Model に $fillable とリレーションを補足**

```php
<?php
// File: app/model/Inventory.php — 完全版
declare(strict_types=1);
namespace app\model;
use support\Model;
...
## Task 8: 商品基礎データ Controllers

**ファイル:**
- 作成: `app/controller/product/ProductController.php`
- 作成: `app/controller/product/CategoryController.php`
- 作成: `app/controller/product/BrandController.php`
- 作成: `app/controller/product/WarehouseController.php`
- 作成: `app/controller/product/LocationController.php`
- 作成: `app/controller/product/SupplierController.php`
- 作成: `app/controller/product/CustomerController.php`

すべてのコントローラは `app\admin\controller\BaseController` を継承し、CRUD パターンは既存の `UserController` と同一です。以下に `ProductController` の完全な例を示します:

- [ ] **Step 1: ProductController を作成**
...
- [ ] **Step 2: 残りの基礎データ Controllers を作成**

残りの 6 コントローラ（Category、Brand、Warehouse、Location、Supplier、Customer）はいずれも標準 CRUD で、構造は上記と同じです。作成後、ファイルを一括書き込みします:
...
## Task 9: 在庫サービス — 入出庫のコアロジック

これはモジュール間連携の核であり、仕入れの入荷・販売の出荷などすべてがこれに依存します。

**ファイル:**
- 作成: `app/service/inventory/InventoryService.php`

- [ ] **Step 1: InventoryService を作成**
...
## Task 10: 財務サービス — 売掛・買掛の自動生成と消込

**ファイル:**
- 作成: `app/service/finance/FinanceService.php`

- [ ] **Step 1: FinanceService を作成**
...
## Task 11: 仕入れ入荷 Controller（モジュール間連携の例）

これは最も典型的なモジュール間連携の業務操作です: 入荷 → 自動入庫 → 買掛生成。完全な連携コードを示します。

**ファイル:**
- 作成: `app/controller/purchase/ReceiveController.php`

- [ ] **Step 1: 仕入れ入荷コントローラを作成**
...
**ファイル:**
- 変更: `config/route.php`
- 作成: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Step 1: ルートファイルを拡張**

`config/route.php` の `/admin` ルートグループ内（`->middleware([...])` の前）に、次のルートを追加します:

```php
// Route::group('/admin', function () { の内部に追加:
...
## Task 13: ダッシュボードパネル拡張

**ファイル:**
- 変更: `app/admin/controller/DashboardController.php`

- [ ] **Step 1: DashboardController を更新して販売・在庫・財務パネルを追加**

既存の `DashboardController` に次のメソッドを追加します:

```php
<?php
// app/admin/controller/DashboardController.php に追加
...
DashboardController の先頭に `use` を追加する必要があります:
```php
...
## Task 14: クライアント API（Flutter/HarmonyOS から呼び出し）

**ファイル:**
- 作成: `app/api/v1/controller/ProductController.php`
- 作成: `app/api/v1/controller/OrderController.php`

- [ ] **Step 1: クライアント API の例**
...
クライアントルートの登録（`config/route.php` の `/api` グループ内に追加）:
...
## Task 15: Flutter + HarmonyOS フロントエンドページ

**Flutter Web PC 端ページディレクトリ:**
```
apps/flutter/lib/app/pages/
├── product/      # 商品リストページ + フォームページ
├── purchase/     # 仕入れ注文/入荷ページ
├── sales/        # 販売注文/出荷ページ
├── inventory/    # 在庫照会/棚卸/振替ページ
├── finance/      # 入出金/経費/レポートページ
└── crm/          # 商機/フォローアップ/ファネルページ
```

各ページは既存の Flutter コードパターン（GetX Controller + ApiService シングルトン + Material 3 テーマ）に従います。
...
"
# 期待値: 約 55 テーブル（システムテーブル 5 枚 + 業務テーブル 50 枚）
```
...
| 層 | 数量 | ファイル位置 |
|------|------|----------|
| データベーステーブル | 50 枚 | `database/migrations/` |
| Models | 44 個 | `app/model/` |
| Controllers | 約 35 個 | `app/controller/{module}/` |
| Services | 2 個のコア | `app/service/inventory/`, `app/service/finance/` |
| ルートグループ | 7 モジュール | `config/route.php` |
| 権限エントリ | 約 65 件 | シード SQL |
| ダッシュボードパネル | 4 個 | DashboardController |
| クライアント API | 必要に応じて拡張 | `app/api/v1/controller/` |
| Flutter ページ | 12 個以上 | `apps/flutter/lib/app/pages/` |
| HarmonyOS ページ | 6 個以上 | `apps/harmonyos/entry/src/main/ets/pages/` |

**総コード量の見積り:** PHP 約 8,000 行 / SQL 約 2,000 行 / Dart 約 3,000 行 / ArkTS 約 1,500 行

**主要な設計決定:**
1. すべての ID は API 層で暗号化して転送（hashids）、データベースには元の BIGINT を保存
2. 機密フィールドはデータベース層で自動暗号化/復号（Encryptable cast）
3. モジュール間連携は Service 層のイベントトリガーで行い、直接相互呼び出しはしない
4. 原価計算は移動加重平均法を採用
5. 売掛・買掛は仕入れ/販売伝票から自動生成し、手動で消込
6. ダッシュボードデータは Redis に 5 分キャッシュ

**未カバー項目（今後のイテレーションで対応）:**
- Phase 2: 製造管理、プロジェクト管理
- WebSocket リアルタイムプッシュ（在庫アラート、承認リマインダー）
- マルチテナント化
- より細かい権限粒度（データ権限）
- モバイル端オフラインモード
