...
**アーキテクチャ:** Controller → Service → Model の 3 層構成。すべてのモデルは `app/model/` を共有し、コントローラはモジュールごとに `app/controller/{module}/` に分け、ビジネスロジックは `app/service/{module}/` に置く。データベースは約 50 テーブルを統一 `erp_` プレフィックス、BIGINT snowflake 主キーとする。API バージョンは `API-Version` リクエストヘッダー + `ApiVersion` ミドルウェアで転送する。
