...
**目標:** 財務、給与、MRP の 3 大コアモジュールを CRUD スケルトンから産業級の業務計算エンジンへアップグレード

**アーキテクチャ:** 各エンジンは独立した Service クラスとして、`app/service/{module}/` ディレクトリ規約に従い、コンストラクタ注入で依存関係を解決し、DB::transaction() でデータ整合性を保証し、PHPUnit テストで主要アルゴリズムをカバー
...
既存の `FinanceVoucher` は単純な CRUD のみで、借貸バランス検証がない。`FinanceVoucherItem` テーブルには `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary` フィールドが既にある。コアルール：「借方あれば貸方あり、借貸は必ず一致」。

- [ ] **Step 1: DoubleEntryService を作成**
...
- [ ] **Step 3: VoucherController 統合**

`app/controller/finance/VoucherController.php` の `store()` を変更：リクエストに `items` が含まれる場合は `DoubleEntryService::createVoucher()` を呼び、それ以外は元の CRUD にフォールバック。
...
- [ ] **Step 1: SalaryEngineService を作成**
...
- [ ] **Step 1: VoucherController 統合** — `store()` が `items` パラメータを検出し、あれば `DoubleEntryService` 経由、なければ元の CRUD
- [ ] **Step 2: SalaryController 統合** — `store()` が `SalaryEngineService::calculate()` を呼び給与明細を自動計算
- [ ] **Step 3: MrpController 統合** — `generate()` が `MrpEngineService::explodeBom()` + `calculateNetRequirement()` を呼ぶ
- [ ] **Step 4: 全テスト実行** — `php vendor/bin/phpunit --testdox`
- [ ] **Step 5: コミット**
...
- [ ] BOM が多層再帰で原材料まで展開、ロス率を計上
- [ ] `php vendor/bin/phpunit --testdox` 全通過
