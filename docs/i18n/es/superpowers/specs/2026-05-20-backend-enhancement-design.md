...
La tabla `erp_operation_log` solo tiene la columna `created_at` (sin `updated_at`). El `save()` por defecto de Eloquent intenta escribir `updated_at`, causando un error SQL.
