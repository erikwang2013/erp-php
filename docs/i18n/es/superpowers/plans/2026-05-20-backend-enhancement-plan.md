...
**Goal:** Implementar 13 mejoras del backend: 3 middlewares nuevos (CORS, RateLimit, OperationLog), 6 controladores nuevos, 2 modificaciones de modelos, configuración de rutas + middlewares.

**Architecture:** Se sigue el patrón webman existente — los middlewares implementan `MiddlewareInterface::process()`, los controladores heredan de `BaseController`, las rutas se definen con closures o `[class, method]`. Los middlewares globales se registran en `config/middleware.php`; los middlewares a nivel de ruta se montan en los grupos de rutas de `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (extensión), PhpSpreadsheet (importación)

**Orden de dependencias:** modificación de modelos → middlewares → controladores → rutas/configuración

---

### Task 1: Modelo AdminUser con SoftDeletes + trait Searchable

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **Step 1: Modificar el modelo AdminUser**

Insertar dos `use` después de la línea 10 de `app/model/AdminUser.php`, y usar los dos traits al principio del cuerpo de la clase:
...
Añadir además el método `toSearchableArray()` al final de la clase (después del método `roles()`, antes de la llave de cierre de la clase):
...
- [ ] **Step 1: Crear el middleware Cors**
...
### Task 3: Middleware de limitación de frecuencia RateLimit

**Files:**
- Create: `app/middleware/RateLimit.php`

- [ ] **Step 1: Crear el middleware RateLimit**
...
### Task 4: Middleware de registro de operaciones OperationLog

**Files:**
- Create: `app/middleware/OperationLog.php`

- [ ] **Step 1: Crear el middleware OperationLog**
...
### Task 5: Configuración de middlewares globales

**Files:**
- Modify: `config/middleware.php`

- [ ] **Step 1: Registrar los middlewares globales**
...
### Task 6: Controlador de healthcheck
...
### Task 7: Controlador CRUD de configuración del sistema

**Files:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **Step 1: Crear ConfigController**
...
### Task 8: Controlador de consulta de logs de operaciones

**Files:**
- Create: `app/admin/controller/LogController.php`

- [ ] **Step 1: Crear LogController**
...
### Task 9: Controlador de centro personal (incluye cierre de sesión)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (verificación de lista negra JWT)

- [ ] **Step 1: Modificar el middleware AdminAuth para añadir la verificación de lista negra JWT**

En el método `AdminAuth::process()`, insertar la comprobación de lista negra después de la extracción del token y antes del descifrado JWT:
...
También es necesario añadir la importación de Redis. Modificar la cabecera del archivo:
...
- [ ] **Step 2: Crear ProfileController**
...
### Task 10: Controlador de subida de archivos

**Files:**
- Create: `app/admin/controller/UploadController.php`

- [ ] **Step 1: Crear UploadController**
...
### Task 11: Operaciones masivas de usuarios

**Files:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **Step 1: Añadir dos métodos al final de la clase UserController (después del método `destroy`)**
...
### Task 12: Controlador de importación Excel

**Files:**
- Create: `app/admin/controller/ImportController.php`

- [ ] **Step 1: Crear ImportController**
...
### Task 13: Actualización de rutas — todas las rutas nuevas + vinculación de middlewares

**Files:**
- Modify: `config/route.php`

- [ ] **Step 1: Actualizar la configuración de rutas**

Añadir rutas nuevas dentro del grupo de rutas `/admin` y adjuntar el middleware OperationLog. Archivo de rutas completo:
...
Después de completar la implementación, verificar punto por punto:
