# P1 Motores de profundidad de negocio — plan de implementación

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) to implement this plan task-by-task.

**Goal:** Actualizar los tres módulos principales de finanzas, nóminas y MRP de esqueletos CRUD a motores de cálculo empresarial de nivel industrial

**Architecture:** Cada motor es una clase Service independiente que sigue la convención de directorio `app/service/{module}/`, con inyección de dependencias mediante el constructor; `DB::transaction()` garantiza la consistencia de los datos y las pruebas PHPUnit cubren los algoritmos clave

**Tech Stack:** PHP 8.3, workerman/webman-framework 2.x, Eloquent ORM, PHPUnit 12

---

## Task 1: Motor de contabilidad de partida doble
...
El `FinanceVoucher` actual solo hace CRUD simple, sin validación del equilibrio entre débito y crédito. La tabla `FinanceVoucherItem` ya tiene los campos `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary`. Regla central: «no hay débito sin crédito; el débito y el crédito deben ser iguales».

- [ ] **Step 1: Crear DoubleEntryService**
...
- [ ] **Step 3: Integración en VoucherController**

Modificar `store()` de `app/controller/finance/VoucherController.php`: si la solicitud contiene `items`, llamar a `DoubleEntryService::createVoucher()`; de lo contrario, retroceder al CRUD original.
...
- [ ] **Step 4: Ejecutar las pruebas y hacer commit**
...
- [ ] **Step 3: Ejecutar las pruebas y hacer commit**
...
## Task 3: Motor de planificación de necesidades de materiales MRP

**Files:**
- Create: `app/service/manufacturing/MrpEngineService.php`
- Create: `tests/MrpEngineServiceTest.php`

- [ ] **Step 1: Crear MrpEngineService**
...
## Task 4: Integración en los controladores existentes
...
- [ ] **Step 1: Integración en VoucherController** — `store()` detecta el parámetro `items`; si existe usa `DoubleEntryService`, si no usa el CRUD original
- [ ] **Step 2: Integración en SalaryController** — `store()` llama a `SalaryEngineService::calculate()` para calcular automáticamente los detalles de nómina
- [ ] **Step 3: Integración en MrpController** — `generate()` llama a `MrpEngineService::explodeBom()` + `calculateNetRequirement()`
- [ ] **Step 4: Ejecutar todas las pruebas** — `php vendor/bin/phpunit --testdox`
- [ ] **Step 5: Commit**
...
- [ ] Al guardar un comprobante con débito y crédito desiguales → 422 «débito y crédito desequilibrados»
...
- [ ] La expansión BOM recurre por múltiples niveles hasta las materias primas, incluyendo la tasa de pérdidas
- [ ] `php vendor/bin/phpunit --testdox` pasa todo
