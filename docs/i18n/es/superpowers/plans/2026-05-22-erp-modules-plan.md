# Plan de implementación de módulos de negocio ERP

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implementar los módulos de negocio ERP completos de compras, ventas e inventario (进销存) + finanzas + CRM sobre la base de administración de sistema existente de webman v2

**Architecture:** Arquitectura de tres capas Controller → Service → Model. Todos los modelos comparten `app/model/`, los controladores se organizan por módulo en `app/controller/{module}/`, la lógica de negocio reside en `app/service/{module}/`. Unas 50 tablas de base de datos con prefijo unificado `erik_` y claves primarias BIGINT de snowflake. La versión de API se controla mediante la cabecera de solicitud `API-Version` + el reenvío del middleware `ApiVersion`.

**Tech Stack:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / paquetes erikwang2013/*
...
- Versión de API: cabecera de solicitud `API-Version` (p. ej., `v1`, `v2`), por defecto `v1` si falta, no se refleja en la URL

---

## Task 1: Migración de base de datos — datos básicos de productos

**Files:**
- Create: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Step 1: Crear el SQL de migración de datos básicos de productos**
...
## Task 2: Migración de base de datos — módulo de compras

**Files:**
- Create: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Step 1: SQL de migración de las 9 tablas del módulo de compras**
...
## Task 3: Migración de base de datos — módulo de ventas

**Files:**
- Create: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Step 1: SQL de migración de las 9 tablas del módulo de ventas**
...
## Task 4: Migración de base de datos — módulos de inventario + finanzas + CRM
...
- [ ] **Step 1: Las 11 tablas del módulo de inventario**
...
- [ ] **Step 2: Las 11 tablas del módulo de finanzas**
...
- [ ] **Step 3: Las 4 tablas del módulo CRM**
...
- [ ] **Step 4: Ejecutar todas las migraciones**
...
## Task 5: Models de datos básicos de productos
...
- [ ] **Step 2: Crear los demás Models de datos básicos (con el mismo patrón)**
...
## Task 6: Models de compras + ventas
...
- [ ] **Step 1: Crear todos los Models de compras y ventas**

Todos los models siguen el mismo patrón: trait `SoftDeletes` (tablas maestras y de pedidos), `$incrementing = false; $keyType = 'int';`, relaciones BelongsTo. A continuación solo se muestra PurchaseOrder (el más representativo):
...
**Comandos para crear el resto de Models de compras/ventas:**
...
Como los Models de compras y ventas suman 19 y el patrón es uniforme, se crean en lote mediante script. Los `$fillable`, `$casts` y métodos de relación específicos de cada Model se completarán según sea necesario al implementar los Controladores posteriores.
...
## Task 7: Models de inventario + finanzas + CRM
...
- [ ] **Step 1: Crear en lote los Models de inventario/finanzas/CRM**
...
- [ ] **Step 2: Completar $fillable y relaciones de los Models clave**
...
## Task 8: Controladores de datos básicos de productos
...
Todos los controladores heredan de `app\admin\controller\BaseController`, con el patrón CRUD idéntico al `UserController` existente. A continuación el ejemplo completo con `ProductController`:
...
- [ ] **Step 2: Crear los demás Controladores de datos básicos**

Los otros 6 controladores (Category, Brand, Warehouse, Location, Supplier, Customer) son CRUD estándar, con la misma estructura. Tras crearlos, escribir los archivos en lote:
...
## Task 9: Servicio de inventario — lógica central de entradas y salidas

Es el núcleo de la integración entre módulos: la recepción de compras, los envíos de ventas, etc., dependen de él.

**Files:**
- Create: `app/service/inventory/InventoryService.php`

- [ ] **Step 1: Crear InventoryService**
...
- [ ] **Step 2: Crear una prueba para verificar el servicio de inventario**
...
## Task 10: Servicio financiero — generación automática y compensación de CxC/CxP

**Files:**
- Create: `app/service/finance/FinanceService.php`

- [ ] **Step 1: Crear FinanceService**
...
## Task 11: Controlador de recepción de compras (ejemplo de integración entre módulos)

Es la operación de negocio más típica de integración entre módulos: recepción → entrada automática al almacén → generación de cuentas por pagar. Muestra el código completo de la integración.

**Files:**
- Create: `app/controller/purchase/ReceiveController.php`

- [ ] **Step 1: Crear el controlador de recepción de compras**
...
## Task 12: Ampliación de rutas + semillas de permisos

**Files:**
- Modify: `config/route.php`
- Create: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Step 1: Ampliar el archivo de rutas**

Dentro del grupo de rutas `/admin` de `config/route.php` (antes de `->middleware([...])`), añadir las siguientes rutas:
...
- [ ] **Step 2: Crear el SQL de semillas de permisos ERP**
...
- [ ] **Step 3: Verificar la sintaxis de las rutas**
...
## Task 13: Ampliación de paneles del panel de control

**Files:**
- Modify: `app/admin/controller/DashboardController.php`

- [ ] **Step 1: Actualizar DashboardController añadiendo los paneles de ventas, inventario y finanzas**

Añadir los siguientes métodos al `DashboardController` existente:
...
En la parte superior de DashboardController hay que añadir los `use`:
```php
...
Registro de rutas de clientes (añadir dentro del grupo `/api` de `config/route.php`):
...
## Task 15: Páginas frontend de Flutter + HarmonyOS

**Directorio de páginas para Flutter Web PC:**
```
...
Cada página sigue los patrones de código Flutter existentes (Controller GetX + singleton ApiService + tema Material 3).

**Directorio de páginas de HarmonyOS:**
```
...
- [ ] **Step 1: Ejemplo de Flutter — página de lista de productos**
...
- [ ] **Step 2: Ejemplo de HarmonyOS — página de lista de productos**
...
## Task 16: Verificación final y pruebas de integración

- [ ] **Step 1: Verificación de sintaxis PHP**
...
- [ ] **Step 2: Ejecutar las pruebas existentes**
...
- [ ] **Step 3: Verificar la integridad de las rutas**
...
- [ ] **Step 4: Verificación de integridad de tablas de base de datos**
...
| Nivel | Cantidad | Ubicación de archivos |
|------|------|----------|
| Tablas de base de datos | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 núcleos | `app/service/inventory/`, `app/service/finance/` |
| Grupos de rutas | 7 módulos | `config/route.php` |
| Entradas de permisos | ~65 | seed SQL |
| Paneles del panel de control | 4 | DashboardController |
| API de clientes | ampliable según necesidad | `app/api/v1/controller/` |
| Páginas Flutter | 12+ | `apps/flutter/lib/app/pages/` |
| Páginas HarmonyOS | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**Estimación de volumen de código:** PHP ~8.000 líneas / SQL ~2.000 líneas / Dart ~3.000 líneas / ArkTS ~1.500 líneas

**Decisiones de diseño clave:**
1. Todos los IDs se cifran en la capa de API (hashids); la base de datos almacena el BIGINT original
2. Los campos sensibles se cifran/descifran automáticamente a nivel de base de datos (cast Encryptable)
3. La integración entre módulos se activa mediante eventos en la capa de Service, sin llamadas directas entre módulos
4. El cálculo de costos usa el método de promedio ponderado móvil
5. Las cuentas por cobrar y por pagar se generan automáticamente a partir de los documentos de compras/ventas, con compensación manual
6. Los datos del panel de control se almacenan en caché en Redis durante 5 minutos

**Elementos no cubiertos (requieren iteraciones posteriores):**
- Fase 2: gestión de fabricación, gestión de proyectos
- Push en tiempo real por WebSocket (alertas de inventario, avisos de aprobación)
- Adaptación a multitenencia
- Granularidad de permisos más rica (permisos de datos)
