...
| | 매입 세금계산서 풀 + 전자발행 | 매입 세금계산서 관리 + 발행 채널(어댑터 + Mock 채널) | 매출채권·매입채무/수금·지급/일계부/비용 정산/손익계산서/고정자산/세금/다중 통화/예산/원가·이익센터 | 매출채권·매입채무 자동 생성 + 대체 + 종합 재무 관리 |
...
`Accept-Language` 헤더로 자동 전환(zh-CN / en), 기본값은 중국어.
