...
**아키텍처:** Controller → Service → Model 3계층. 모든 모델은 `app/model/` 공유, 컨트롤러는 모듈별 디렉토리 `app/controller/{module}/`, 비즈니스 로직은 `app/service/{module}/`. 데이터베이스 약 50개 테이블은 모두 `erp_` 접두사, BIGINT snowflake 기본키 통일. API 버전은 `API-Version` 요청 헤더 + `ApiVersion` 미들웨어로 전달.
