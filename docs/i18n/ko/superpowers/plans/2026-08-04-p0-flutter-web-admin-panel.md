# P0 Flutter Web 관리 패널 — 구현 계획

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**목표:** Flutter Web 관리 패널을 기존 ~12개 페이지에서 14개 백엔드 모듈 전체를 커버하는 40+ 페이지로 확장하고, 재사용 가능한 공통 컴포넌트 라이브러리를 구축합니다.

**아키텍처:** 설정 기반 동적 메뉴 시스템(`menu_config.dart`)으로 재구성, PC 3단 레이아웃(접이식 사이드바 + 탑 바 + 콘텐츠 영역), 공통 CRUD 컴포넌트(DataTableWrapper/FormDialog/ConfirmDialog)를 각 모듈 페이지에서 재사용.

**기술 스택:** Flutter 3.x, Dart 3.11+, GetX 4.6, Dio 5.4, data_table_2 2.5, fl_chart 0.68, printing 5.12, excel 4.0, responsive_framework 1.4
...
### Task 1: 메뉴 설정과 라우트 재구성

**Files:**
- Create: `apps/flutter/lib/app/config/menu_config.dart`
- Modify: `apps/flutter/lib/app/layouts/admin_layout.dart` (전량 재작성)
- Modify: `apps/flutter/lib/main.dart` (라우트 등록)
- Move: 기존 페이지 파일을 새 디렉토리 구조로 이동

- [ ] **Step 1: menu_config.dart 생성**
...
- [ ] **Step 2: AdminLayout 재작성 — 동적 메뉴 렌더링 지원**
...
- [ ] **Step 3: main.dart 업데이트 — 중앙 집중식 라우트 등록**
...
- [ ] **Step 5: 컴파일 검증** — Run: `cd apps/flutter && flutter analyze`

- [ ] **Step 6: 커밋**

---

### Task 2: 공통 컴포넌트 — DataTableWrapper
...
### Task 3: 공통 컴포넌트 — FormDialog + ConfirmDialog + StatCard
...
### Task 4-20: 모듈 페이지 일괄 생성

> 각 task는 하나의 모듈을 다루며, 모든 목록 페이지는 통일된 CRUD 패턴을 따릅니다.

**CRUD 목록 페이지 표준 템플릿**(페이지당 ~70줄):
...
| Task | 모듈 | 디렉토리 | 페이지 수 |
|------|------|------|--------|
| 4 | 상품 | `pages/product/` | 4 |
| 5 | 거래처 | `pages/partner/` | 4 |
| 6 | 구매 | `pages/purchase/` | 4 |
| 7 | 판매 | `pages/sales/` | 4 |
| 8 | 재고 | `pages/inventory/` | 5 |
| 9 | 재무1 (전표/매출채권·매입채무/수금·지급/일계부/비용정산) | `pages/finance/` | 6 |
| 10 | 재무2 (총계정원장/리포트/자산/세무/통화/예산/원가) | `pages/finance/` | 7 |
| 11 | CRM | `pages/crm/` | 8 |
| 12 | OMS | `pages/oms/` | 4 |
| 13 | WMS | `pages/wms/` | 7 |
| 14 | TMS | `pages/tms/` | 5 |
| 15 | 제조 | `pages/manufacturing/` | 5 |
| 16 | HR | `pages/hr/` | 6 |
| 17 | 프로젝트 | `pages/project/` | 3 |
| 18 | 승인 워크플로 | `pages/workflow/` | 2 |
| 19 | 알림 | `pages/notification/` | 1 |
| 20 | 커스텀 리포트 | `pages/report/` | 2 |

각 task 완료 후: `main.dart` 업데이트로 해당 라우트 추가 + `flutter analyze` 검증.

---

### Task 21: Dashboard 확장
...
- [ ] **Step 1: 기존 4 Tab 위에 OMS/WMS/TMS 보드 추가**

TabBar 추가: `const Tab(text: 'OMS')`, `const Tab(text: 'WMS')`, `const Tab(text: 'TMS')`
TabBarView에 해당 `_omsTab()`, `_wmsTab()`, `_tmsTab()` 메서드 추가, 각각 StatCard 3-4개 포함.

- [ ] **Step 2: 커밋**

---

### Task 22: 전체 라우트 등록 + 최종 검증

- [ ] **Step 1: main.dart 모든 GetPage 등록 보강**
- [ ] **Step 2: `flutter analyze` → No issues found**
- [ ] **Step 3: `flutter run -d chrome` 수동 검수 — 14개 모듈 메뉴와 페이지 전체**
- [ ] **Step 4: 커밋**

---

### Task 23: HarmonyOS 정렬

**Files:** `apps/harmonyos/entry/src/main/ets/pages/` 아래에 새 페이지 생성

- [ ] OMS/WMS/TMS/제조/HR/승인/알림/리포트 총 8개 모듈의 ArkTS 페이지 생성
- [ ] `main_pages.json` 업데이트로 라우트 등록
- [ ] 컴파일 검증

---

**수용 기준 (P0 complete):**
- [ ] Flutter Web 사이드바 14개 모듈 모두 내비게이션 가능
- [ ] 모든 CRUD 목록 페이지 정상 로드, 페이징, 검색
- [ ] 생성/편집 팝업 + 삭제 비밀번호 확인 동작
- [ ] PC/태블릿/폰 반응형 레이아웃 정상
- [ ] JWT 만료 시 자동 갱신
- [ ] HarmonyOS 페이지 수 ≥ Flutter 페이지의 80%
