...
**목표:** 재무, 급여, MRP 3대 핵심 모듈을 CRUD 골격에서 산업급 업무 계산 엔진으로 업그레이드

**아키텍처:** 각 엔진은 독립적인 Service 클래스로, `app/service/{module}/` 디렉토리 규약을 따르며 생성자 주입으로 의존성을 주입하고, DB::transaction()으로 데이터 일관성을 보장, PHPUnit 테스트로 핵심 알고리즘 커버

**기술 스택:** PHP 8.3, workerman/webman-framework 2.x, Eloquent ORM, PHPUnit 12
...
기존 `FinanceVoucher`는 단순 CRUD만 수행하며 차대 균형 검증이 없습니다. `FinanceVoucherItem` 테이블에는 이미 `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary` 필드가 있습니다. 핵심 규칙: 「차변이 있으면 반드시 대변이 있고, 차변과 대변은 반드시 같다」.

- [ ] **Step 1: DoubleEntryService 생성**
...
- [ ] **Step 3: VoucherController 통합**

`app/controller/finance/VoucherController.php`의 `store()` 수정: 요청에 `items`가 포함되면 `DoubleEntryService::createVoucher()` 호출, 아니면 기존 CRUD로 폴백.
...
- [ ] **Step 1: VoucherController 통합** — `store()`가 `items` 파라미터 감지, 있으면 `DoubleEntryService` 경로, 없으면 기존 CRUD
- [ ] **Step 2: SalaryController 통합** — `store()`가 `SalaryEngineService::calculate()` 호출해 급여 명세 자동 계산
- [ ] **Step 3: MrpController 통합** — `generate()`가 `MrpEngineService::explodeBom()` + `calculateNetRequirement()` 호출
- [ ] **Step 4: 전체 테스트 실행** — `php vendor/bin/phpunit --testdox`
- [ ] **Step 5: 커밋**
...
- [ ] BOM 다층 재귀 전개로 원자재까지, 손실률 반영
- [ ] `php vendor/bin/phpunit --testdox` 전체 통과
