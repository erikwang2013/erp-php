# 서브프로젝트 A: 백엔드 강화 — 설계 규격

> Copyright (c) 2026 erik <erik@erik.xyz> — https://erik.xyz

## 범위

이번은 백엔드 강화로 총 15개 기능 포인트, 신규 9개 파일 + 수정 4개 파일 포함.
...
**원리**: JWT는 무상태이므로 로그아웃 시 token을 Redis 블랙리스트에 추가하고, AdminAuth 검증 시 먼저 블랙리스트를 확인합니다.

**AdminAuth 개조**:
- `process()` 시작부에 추가: Redis `jwt_blacklist` 집합에서 현재 token이 블랙리스트인지 확인
- 블랙리스트 매칭 시 401 반환
...
| 메서드 | 라우트 | 설명 |
|------|------|------|
| `index()` | GET `/admin/config` | 페이지 목록, `group` 필터 가능, `page`/`limit` 페이지네이션 |
| `store()` | POST `/admin/config` | 설정 항목 생성, 필수: group, key, value |
| `update()` | PUT `/admin/config/{id}` | 설정 항목 value/type/description 갱신 |
...
증감삭제 제공하지 않음, 로그는 미들웨어가 자동 기록.

### 2.3 개인 센터 (`ProfileController`)

`BaseController` 상속. 현재 로그인 사용자 대상 작업(`$request->adminId`).

| 메서드 | 라우트 | 설명 |
|------|------|------|
| `updateProfile()` | PUT `/admin/profile` | real_name, phone, email 갱신 |
...
| 추세 데이터 | `AdminUser::selectRaw('DATE(created_at) d, count(*) c')->groupBy('d')->orderBy('d','desc')->limit(7)->get()` | 최근 7일 신규 일별 집계 |
...
- 컴포넌트 감지 실패 시 해당 필드 값이 오류 설명 문자열
- 라우트가 `/admin` 접두사 미사용, 전역에 별도 등록
...
테이블 `erik_operation_log`에는 `created_at` 열만 존재(`updated_at` 없음). Eloquent 기본 `save()`는 `updated_at`을 쓰려고 시도해 SQL 오류 발생.

수정: `public $timestamps = false;` + 기록 시 `created_at` 수동 지정.

### 3.2 AdminUser 모델 개조

- `Searchable` trait 추가
- `toSearchableArray()` 구현: username, real_name 반환
- `UserController::index()`가 키워드 감지 시 `AdminUser::search($kw)->get()` 사용(MySQL LIKE 대신)

ES는 먼저 인덱스 생성 필요, Scout 명령으로 가능:
...
- Flutter 프론트엔드 페이지(서브프로젝트 B)
- HarmonyOS Token 갱신(서브프로젝트 C)
