# P0 Panneau d'administration Flutter Web — plan d'implémentation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 将 Flutter Web 管理面板从现有 ~12 页扩展到覆盖全部 14 个后端模块的 40+ 页面，建立可复用的通用组件库。

**Architecture:** 重构为配置驱动的动态菜单系统（`menu_config.dart`），PC 三栏布局（可折叠侧边栏 + 顶栏 + 内容区），通用 CRUD 组件（DataTableWrapper/FormDialog/ConfirmDialog）供各模块页面复用。
...
├── main.dart                          # Modifié : enregistrement centralisé des routes
├── config/
│   └── menu_config.dart               # Nouveau : configuration arborescente du menu de navigation
├── layouts/
│   └── admin_layout.dart              # Réécrit : rendu dynamique du menu
├── services/                          # Inchangé
├── widgets/                           # 5 nouveaux composants génériques
│   ├── data_table_wrapper.dart
│   ├── form_dialog.dart
│   ├── confirm_dialog.dart
│   ├── stat_card.dart
│   └── search_bar.dart
└── pages/                             # Réorganisé en répertoires modulaires
...
### Task 1: Configuration du menu et refactorisation des routes

**Files:**
- Create: `apps/flutter/lib/app/config/menu_config.dart`
- Modify: `apps/flutter/lib/app/layouts/admin_layout.dart` (réécriture complète)
- Modify: `apps/flutter/lib/main.dart` (enregistrement des routes)
- Move: migration des fichiers de pages existants vers la nouvelle structure de répertoires

- [ ] **Step 1: Créer menu_config.dart**
...
- [ ] **Step 2: Réécrire AdminLayout pour supporter le rendu dynamique du menu**
...
- [ ] **Step 3: Mettre à jour main.dart avec l'enregistrement centralisé des routes**

```dart
// apps/flutter/lib/main.dart (changements clés)
...
- [ ] **Step 4: Migrer les répertoires des fichiers de pages existants**
...
- [ ] **Step 5: Vérifier la compilation** — Run: `cd apps/flutter && flutter analyze`

- [ ] **Step 6: Commit**

---

### Task 2: Composants génériques — DataTableWrapper
...
### Task 3: Composants génériques — FormDialog + ConfirmDialog + StatCard
...
### Task 4-20: Création en masse des pages de modules

> Chaque task couvre un module, toutes les pages de listes suivent le modèle CRUD unifié.

**Modèle standard de page de liste CRUD**（~70 lignes par page）：
...
**Liste de création des pages**：

| Task | Module | Répertoire | Nombre de pages |
|------|------|------|--------|
| 4 | Produits | `pages/product/` | 4 |
| 5 | Tiers | `pages/partner/` | 4 |
| 6 | Achats | `pages/purchase/` | 4 |
| 7 | Ventes | `pages/sales/` | 4 |
| 8 | Stocks | `pages/inventory/` | 5 |
| 9 | Finance 1 (pièces/AR-AP/encaissements-décaissements/journaux/remboursements) | `pages/finance/` | 6 |
| 10 | Finance 2 (grand livre/états/immobilisations/fiscalité/devises/budgets/coûts) | `pages/finance/` | 7 |
| 11 | CRM | `pages/crm/` | 8 |
| 12 | OMS | `pages/oms/` | 4 |
| 13 | WMS | `pages/wms/` | 7 |
| 14 | TMS | `pages/tms/` | 5 |
| 15 | Fabrication | `pages/manufacturing/` | 5 |
| 16 | RH | `pages/hr/` | 6 |
| 17 | Projets | `pages/project/` | 3 |
| 18 | Workflow d'approbation | `pages/workflow/` | 2 |
| 19 | Notifications | `pages/notification/` | 1 |
| 20 | Rapports personnalisés | `pages/report/` | 2 |

Après chaque task : mettre à jour `main.dart` avec la route correspondante + vérifier avec `flutter analyze`.

---

### Task 21: Extension du tableau de bord
...
- [ ] **Step 1: Ajouter les tableaux de bord OMS/WMS/TMS aux 4 onglets existants**

Ajouter à la TabBar : `const Tab(text: 'OMS')`, `const Tab(text: 'WMS')`, `const Tab(text: 'TMS')`
Ajouter à la TabBarView les méthodes correspondantes `_omsTab()`, `_wmsTab()`, `_tmsTab()`，chacune contenant 3-4 StatCard.

- [ ] **Step 2: Commit**

---

### Task 22: Enregistrement complet des routes + vérification finale

- [ ] **Step 1: Compléter tous les enregistrements GetPage dans main.dart**
- [ ] **Step 2: `flutter analyze` → No issues found**
- [ ] **Step 3: `flutter run -d chrome` acceptation manuelle des menus et pages des 14 modules**
- [ ] **Step 4: Commit**

---

### Task 23: Alignement HarmonyOS

**Files:** créer de nouvelles pages sous `apps/harmonyos/entry/src/main/ets/pages/`

- [ ] Créer les pages ArkTS des 8 modules OMS/WMS/TMS/Fabrication/RH/Approbation/Notifications/Rapports
- [ ] Mettre à jour `main_pages.json` pour enregistrer les routes
- [ ] Vérifier la compilation

---

**Critères d'acceptation (P0 complete):**
- [ ] Les 14 modules de la barre latérale Flutter Web sont tous navigables
- [ ] Toutes les pages de listes CRUD chargent normalement, avec pagination et recherche
- [ ] Les boîtes de dialogue créer/modifier + confirmation de suppression par mot de passe fonctionnent
- [ ] La mise en page responsive PC/tablette/téléphone fonctionne normalement
- [ ] Le JWT expiré se rafraîchit automatiquement
- [ ] Nombre de pages HarmonyOS ≥ 80 % des pages Flutter
