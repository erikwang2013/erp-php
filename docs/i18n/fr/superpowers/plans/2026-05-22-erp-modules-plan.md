# Plan d'implémentation des modules métier ERP

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Objectif :** Implémenter les modules métier ERP complets (achats-ventes-stocks + finance + CRM) sur la base d'administration système webman v2 existante

**Architecture :** trois couches Controller → Service → Model. Tous les modèles partagent `app/model/`, les contrôleurs sont organisés par module dans `app/controller/{module}/`, la logique métier dans `app/service/{module}/`. Environ 50 tables de base de données avec préfixe unifié `erik_`, clés primaires BIGINT snowflake. La version d'API est transmise via l'en-tête de requête `API-Version` + le middleware `ApiVersion`.

**Pile technique :** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / paquets erikwang2013/*

**Modèles de code** (d'après le code existant) :
- Les contrôleurs étendent `app\admin\controller\BaseController` — fournit `success()`, `fail()`, `encodeId()`, `decodeId()`, `encodeIds()`, `generateId()`, `confirmPassword()`
- Les modèles utilisent `support\Model` avec `SoftDeletes`, `Searchable`, cast `Encryptable`
- Clé primaire : `$incrementing = false; $keyType = 'int';`
- Routes : `Route::resource()` et routes explicites sous le groupe `/admin` avec la chaîne complète de middlewares
- Permissions : format `slug` `method.admin/module/action` (ex. `get.admin/product`)
- Validation : fonction intégrée `validator()` de webman
- Format de réponse : `{code: 0, message: 'success', data: {...}}`
- Version d'API : en-tête de requête `API-Version` (ex. `v1`, `v2`), `v1` par défaut en l'absence, non visible dans l'URL

---

## Tâche 1 : Migration de base de données — données de base produits

**Fichiers :**
- Créer : `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Étape 1 : créer le SQL de migration des données de base produits**
...
## Tâche 2 : Migration de base de données — module achats

**Fichiers :**
- Créer : `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Étape 1 : SQL de migration des 9 tables du module achats**
...
## Tâche 3 : Migration de base de données — module ventes

**Fichiers :**
- Créer : `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Étape 1 : SQL de migration des 9 tables du module ventes**
...
## Tâche 4 : Migration de base de données — modules stocks + finance + CRM

**Fichiers :**
- Créer : `database/migrations/2026_05_22_000006_inventory_tables.sql`
- Créer : `database/migrations/2026_05_22_000007_finance_tables.sql`
- Créer : `database/migrations/2026_05_22_000008_crm_tables.sql`

- [ ] **Étape 1 : 11 tables du module stocks**
...
- [ ] **Étape 2 : 11 tables du module finance**
...
- [ ] **Étape 3 : 4 tables du module CRM**
...
- [ ] **Étape 4 : exécuter toutes les migrations**
...
## Tâche 5 : Models des données de base produits

**Fichiers :**
- Créer : `app/model/Product.php`
- Créer : `app/model/ProductSku.php`
- Créer : `app/model/ProductUnit.php`
- Créer : `app/model/ProductPrice.php`
- Créer : `app/model/Category.php`
- Créer : `app/model/Brand.php`
- Créer : `app/model/Warehouse.php`
- Créer : `app/model/Location.php`
- Créer : `app/model/Supplier.php`
- Créer : `app/model/Customer.php`
- Créer : `app/model/CustomerLevel.php`

- [ ] **Étape 1 : créer le Model Product**
...
- [ ] **Étape 2 : créer les autres Models de données de base (selon le même modèle)**
...
- [ ] **Étape 2 : vérifier le chargement des modèles**
...
## Tâche 6 : Models achats + ventes

**Fichiers :**
- Créer : `app/model/PurchaseApply.php`, `app/model/PurchaseApplyItem.php`
- Créer : `app/model/PurchaseOrder.php`, `app/model/PurchaseOrderItem.php`
- Créer : `app/model/PurchaseReceive.php`, `app/model/PurchaseReceiveItem.php`
- Créer : `app/model/PurchaseReturn.php`, `app/model/PurchaseReturnItem.php`
- Créer : `app/model/PurchaseSettlement.php`
- Créer : `app/model/SalesQuotation.php`, `app/model/SalesQuotationItem.php`
- Créer : `app/model/SalesOrder.php`, `app/model/SalesOrderItem.php`
- Créer : `app/model/SalesDelivery.php`, `app/model/SalesDeliveryItem.php`
- Créer : `app/model/SalesReturn.php`, `app/model/SalesReturnItem.php`
- Créer : `app/model/SalesSettlement.php`

- [ ] **Étape 1 : créer tous les Models achats et ventes**

Tous les modèles suivent le même modèle : trait `SoftDeletes` (tables principales et commandes), `$incrementing = false; $keyType = 'int';`, relations BelongsTo. Ci-dessous seul PurchaseOrder est présenté (le plus représentatif) :
...
**Commande de création des autres Models achats/ventes :**
...
Les 19 Models achats et ventes suivent un modèle unifié et sont créés par script. Les `$fillable`, `$casts` et méthodes de relations spécifiques de chaque Model seront complétés au besoin lors de l'implémentation des contrôleurs.
...
## Tâche 7 : Models stocks + finance + CRM

**Fichiers :**
- Créer : `app/model/Inventory.php`, `app/model/InventoryBatch.php`, `app/model/InventorySerial.php`
- Créer : `app/model/InventoryFlow.php`, `app/model/Transfer.php`, `app/model/TransferItem.php`
- Créer : `app/model/CheckTask.php`, `app/model/CheckDetail.php`
- Créer : `app/model/InventoryAlertRule.php`, `app/model/InventoryAlertLog.php`, `app/model/CostRecord.php`
- Créer : `app/model/FinanceAccount.php`, `app/model/FinanceVoucher.php`, `app/model/FinanceVoucherItem.php`
- Créer : `app/model/FinanceArAp.php`, `app/model/FinanceBankAccount.php`
- Créer : `app/model/FinanceReceipt.php`, `app/model/FinancePayment.php`, `app/model/FinanceSettlement.php`
- Créer : `app/model/FinanceCashJournal.php`, `app/model/FinanceExpense.php`, `app/model/FinanceProfit.php`
- Créer : `app/model/CrmFunnelStage.php`, `app/model/CrmOpportunity.php`
- Créer : `app/model/CrmFollowRecord.php`, `app/model/CrmContact.php`

- [ ] **Étape 1 : créer en masse les Models stocks/finance/CRM**
...
- [ ] **Étape 2 : compléter $fillable et les relations des Models clés**
...
## Tâche 8 : Controllers des données de base produits

**Fichiers :**
- Créer : `app/controller/product/ProductController.php`
- Créer : `app/controller/product/CategoryController.php`
- Créer : `app/controller/product/BrandController.php`
- Créer : `app/controller/product/WarehouseController.php`
- Créer : `app/controller/product/LocationController.php`
- Créer : `app/controller/product/SupplierController.php`
- Créer : `app/controller/product/CustomerController.php`

Tous les contrôleurs héritent de `app\admin\controller\BaseController`, le modèle CRUD est identique à l'existant `UserController`. Ci-dessous un exemple complet avec `ProductController` :

- [ ] **Étape 1 : créer ProductController**
...
- [ ] **Étape 2 : créer les autres Controllers de données de base**

Les 6 autres contrôleurs (Category, Brand, Warehouse, Location, Supplier, Customer) sont des CRUD standard, même structure que ci-dessus. Écriture en masse après création :
...
## Tâche 9 : Service stocks — logique centrale des entrées/sorties

C'est le cœur de la coordination inter-modules, la réception d'achat et l'expédition de vente en dépendent.

**Fichiers :**
- Créer : `app/service/inventory/InventoryService.php`

- [ ] **Étape 1 : créer InventoryService**
...
- [ ] **Étape 2 : créer un test pour vérifier le service stocks**
...
## Tâche 10 : Service finance — génération automatique et lettrage des comptes à recevoir/à payer

**Fichiers :**
- Créer : `app/service/finance/FinanceService.php`

- [ ] **Étape 1 : créer FinanceService**
...
## Tâche 11 : Controller de réception d'achat (exemple de coordination inter-modules)

C'est l'opération métier inter-modules la plus typique : réception → entrée en stock automatique → génération des comptes à payer. Code de coordination complet.

**Fichiers :**
- Créer : `app/controller/purchase/ReceiveController.php`

- [ ] **Étape 1 : créer le contrôleur de réception d'achat**
...
## Tâche 12 : Extension des routes + seed des permissions

**Fichiers :**
- Modifier : `config/route.php`
- Créer : `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Étape 1 : étendre le fichier de routes**

Dans le groupe de routes `/admin` de `config/route.php` (avant `->middleware([...])`), ajouter les routes suivantes :
...
- [ ] **Étape 2 : créer le SQL de seed des permissions ERP**
...
- [ ] **Étape 3 : vérifier la syntaxe des routes**
...
## Tâche 13 : Extension des panneaux du tableau de bord

**Fichiers :**
- Modifier : `app/admin/controller/DashboardController.php`

- [ ] **Étape 1 : mettre à jour DashboardController en ajoutant les panneaux ventes, stocks, finances**

Ajouter les méthodes suivantes dans le `DashboardController` existant :
...
Compléter les `use` en tête de DashboardController :
```php
...
## Tâche 14 : API client (appel depuis Flutter/HarmonyOS)

**Fichiers :**
- Créer : `app/api/v1/controller/ProductController.php`
- Créer : `app/api/v1/controller/OrderController.php`

- [ ] **Étape 1 : exemple d'API client**
...
Enregistrement des routes client (ajouter dans le groupe `/api` de `config/route.php`) :
...
## Tâche 15 : Pages frontend Flutter + HarmonyOS

**Répertoire des pages Web PC Flutter :**
```
...
Chaque page reprend les modèles de code Flutter existants (GetX Controller + singleton ApiService + thème Material 3).

**Répertoire des pages HarmonyOS :**
```
...
- [ ] **Étape 1 : exemple Flutter — page de liste des produits**
...
- [ ] **Étape 2 : exemple HarmonyOS — page de liste des produits**
...
## Tâche 16 : Validation finale et tests d'intégration

- [ ] **Étape 1 : contrôle de syntaxe PHP**
...
- [ ] **Étape 3 : vérifier l'intégrité des routes**
...
- [ ] **Étape 4 : vérifier l'intégrité des tables de base de données**
...
| Couche | Quantité | Emplacement des fichiers |
|------|------|----------|
| Tables de base de données | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 principaux | `app/service/inventory/`, `app/service/finance/` |
| Groupes de routes | 7 modules | `config/route.php` |
| Entrées de permissions | ~65 | SQL de seed |
| Panneaux du tableau de bord | 4 | DashboardController |
| API client | extension au besoin | `app/api/v1/controller/` |
| Pages Flutter | 12+ | `apps/flutter/lib/app/pages/` |
| Pages HarmonyOS | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**Estimation du volume de code total :** PHP ~8 000 lignes / SQL ~2 000 lignes / Dart ~3 000 lignes / ArkTS ~1 500 lignes

**Décisions de conception clés :**
1. Tous les ID sont chiffrés au niveau API (hashids), les BIGINT bruts sont stockés en base
2. Les champs sensibles sont chiffrés/déchiffrés automatiquement au niveau base de données (cast Encryptable)
3. La coordination inter-modules passe par des déclencheurs au niveau Service, sans appels mutuels directs
4. La comptabilité des coûts utilise la méthode du coût moyen pondéré mobile
5. Les comptes à recevoir/à payer sont générés automatiquement à partir des documents d'achat/vente, lettrés manuellement
6. Les données du tableau de bord sont mises en cache Redis 5 minutes

**Éléments non couverts (itération ultérieure nécessaire) :**
- Phase 2 : gestion de la production, gestion de projets
- Push temps réel WebSocket (alertes de stock, rappels d'approbation)
- Refonte multi-tenant
- Granularité de permissions plus riche (permissions sur les données)
