...
**Architecture :** trois couches Controller → Service → Model. Tous les modèles partagent `app/model/`, les contrôleurs sont organisés par module dans `app/controller/{module}/`, la logique métier dans `app/service/{module}/`. Environ 50 tables de base de données avec préfixe unifié `erp_`, clés primaires BIGINT snowflake. La version d'API est transmise via l'en-tête de requête `API-Version` + le middleware `ApiVersion`.
