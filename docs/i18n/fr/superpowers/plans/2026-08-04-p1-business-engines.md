# Moteurs de profondeur métier P1 — plan d'implémentation
...
## Task 1: Moteur comptable en partie double
...
Le `FinanceVoucher` actuel ne fait qu'un simple CRUD, sans validation d'équilibre débit/crédit. La table `FinanceVoucherItem` a déjà les champs `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary`. Règle fondamentale : « pas de débit sans crédit, débit et crédit toujours égaux ».

- [ ] **Step 1: Créer DoubleEntryService**
...
- [ ] **Step 3: Intégration dans VoucherController**

Modifier le `store()` de `app/controller/finance/VoucherController.php` : si la requête contient `items`, appeler `DoubleEntryService::createVoucher()`, sinon revenir à l'ancien CRUD.
...
- [ ] **Step 4: Exécuter les tests et committer**
...
- [ ] **Step 3: Exécuter les tests et committer**
...
## Task 4: Intégration dans les contrôleurs existants
...
- [ ] **Step 1: Intégration VoucherController** — `store()` détecte le paramètre `items`, s'il est présent passe par `DoubleEntryService`, sinon par l'ancien CRUD
- [ ] **Step 2: Intégration SalaryController** — `store()` appelle `SalaryEngineService::calculate()` pour calculer automatiquement les détails de paie
- [ ] **Step 3: Intégration MrpController** — `generate()` appelle `MrpEngineService::explodeBom()` + `calculateNetRequirement()`
- [ ] **Step 4: Exécuter tous les tests** — `php vendor/bin/phpunit --testdox`
- [ ] **Step 5: Committer**
...
- [ ] Enregistrement d'une pièce avec débit ≠ crédit → 422 « 借贷不平衡 » (débit/crédit déséquilibrés)
- [ ] `SalaryEngineService::calculate(10000)` → gross=10000, si=1050, hf=700, tax calculée correctement
- [ ] `MrpEngineService::calculateNetRequirement(100, 30, 0, 0, 20)` → 90
- [ ] Dépliage BOM multi-niveaux récursif jusqu'aux matières premières, taux de perte pris en compte
- [ ] `php vendor/bin/phpunit --testdox` passe intégralement
