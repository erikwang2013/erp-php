...
La table `erp_operation_log` n'a que la colonne `created_at` (pas de `updated_at`). Le `save()` par défaut d'Eloquent tente d'écrire `updated_at`, ce qui provoque une erreur SQL.
