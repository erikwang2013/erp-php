# ERP ব্যবসায়িক মডিউল বাস্তবায়ন পরিকল্পনা
...
- API সংস্করণ: রিকোয়েস্ট হেডার `API-Version` (যেমন `v1`, `v2`), না থাকলে ডিফল্ট `v1`, URL-এ প্রকাশিত হয় না

---

## Task 1: ডাটাবেস মাইগ্রেশন — পণ্য বেস ডেটা

**Files:**
- Create: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **ধাপ 1: পণ্য বেস ডেটা মাইগ্রেশন SQL তৈরি করুন**
...
## Task 2: ডাটাবেস মাইগ্রেশন — ক্রয় মডিউল

**Files:**
- Create: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **ধাপ 1: ক্রয় মডিউলের 9টি টেবিলের মাইগ্রেশন SQL**
...
- [ ] **ধাপ 2: মাইগ্রেশন এক্সিকিউট করুন**
...
## Task 3: ডাটাবেস মাইগ্রেশন — বিক্রয় মডিউল

**Files:**
- Create: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **ধাপ 1: বিক্রয় মডিউলের 9টি টেবিলের মাইগ্রেশন SQL**
...
## Task 4: ডাটাবেস মাইগ্রেশন — ইনভেন্টরি + ফাইন্যান্স + CRM মডিউল
...
- [ ] **ধাপ 1: ইনভেন্টরি মডিউলের 11টি টেবিল**
...
- [ ] **ধাপ 2: ফাইন্যান্স মডিউলের 11টি টেবিল**
...
- [ ] **ধাপ 4: সব মাইগ্রেশন এক্সিকিউট করুন**
...
- [ ] **ধাপ 2: বাকি বেস ডেটা Models তৈরি করুন (একই প্যাটার্ন অনুযায়ী)**
...
- [ ] **ধাপ 1: সব ক্রয় ও বিক্রয় Models তৈরি করুন**

সব models একই প্যাটার্ন অনুসরণ করে: `SoftDeletes` trait (মূল টেবিল ও অর্ডার ক্লাস), `$incrementing = false; $keyType = 'int';`, BelongsTo সম্পর্ক। নিচে কেবল PurchaseOrder দেখানো হয়েছে (সবচেয়ে প্রতিনিধিত্বমূলক):
...
**বাকি ক্রয়/বিক্রয় Models তৈরির কমান্ড:**
...
যেহেতু ক্রয় ও বিক্রয় Models মোট 19টি এবং প্যাটার্ন একই, তাই স্ক্রিপ্ট দিয়ে ব্যাচে তৈরি করা হয়। প্রতিটি Model-এর নির্দিষ্ট `$fillable`, `$casts` ও সম্পর্ক মেথড পরবর্তী Controller বাস্তবায়নে প্রয়োজন অনুযায়ী যোগ করা হবে।
...
## Task 7: ইনভেন্টরি + ফাইন্যান্স + CRM Models
...
- [ ] **ধাপ 1: ইনভেন্টরি/ফাইন্যান্স/CRM Models ব্যাচে তৈরি করুন**
...
- [ ] **ধাপ 2: গুরুত্বপূর্ণ Model-এ $fillable ও সম্পর্ক যোগ করুন**
...
## Task 8: পণ্য বেস ডেটা Controllers
...
সব কন্ট্রোলার `app\admin\controller\BaseController` থেকে উত্তরাধিকার নেয়, CRUD প্যাটার্ন বিদ্যমান `UserController`-এর মতোই। নিচে `ProductController`-এর সম্পূর্ণ উদাহরণ:

- [ ] **ধাপ 1: ProductController তৈরি করুন**
...
- [ ] **ধাপ 2: বাকি বেস ডেটা Controllers তৈরি করুন**

বাকি 6টি কন্ট্রোলার (Category, Brand, Warehouse, Location, Supplier, Customer) সবই স্ট্যান্ডার্ড CRUD, গঠন উপরের মতোই। তৈরি করার পর ব্যাচে ফাইলে লেখা হয়:
...
## Task 9: ইনভেন্টরি সার্ভিস — ইন/আউট কোর লজিক

এটি ক্রস-মডিউল লিংকেজের মূল, ক্রয় রিসিভ/বিক্রয় শিপমেন্ট ইত্যাদি এর উপর নির্ভর করে।

**Files:**
- Create: `app/service/inventory/InventoryService.php`

- [ ] **ধাপ 1: InventoryService তৈরি করুন**
...
- [ ] **ধাপ 2: ইনভেন্টরি সার্ভিস যাচাই করতে টেস্ট তৈরি করুন**
...
## Task 10: ফাইন্যান্স সার্ভিস — প্রাপ্য/পরিশেয় স্বয়ংক্রিয় তৈরি ও নিষ্পত্তি

**Files:**
- Create: `app/service/finance/FinanceService.php`

- [ ] **ধাপ 1: FinanceService তৈরি করুন**
...
## Task 11: ক্রয় রিসিভ Controller (ক্রস-মডিউল লিংকেজ উদাহরণ)

এটি সবচেয়ে সাধারণ ক্রস-মডিউল ব্যবসায়িক অপারেশন: রিসিভ → স্বয়ংক্রিয় স্টক-ইন → প্রাপ্য তৈরি। সম্পূর্ণ লিংকেজ কোড দেখানো হয়েছে।

**Files:**
- Create: `app/controller/purchase/ReceiveController.php`

- [ ] **ধাপ 1: ক্রয় রিসিভ কন্ট্রোলার তৈরি করুন**
...
## Task 12: রাউট সম্প্রসারণ + পারমিশন সিড

**Files:**
- Modify: `config/route.php`
- Create: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **ধাপ 1: রাউট ফাইল সম্প্রসারণ করুন**

`config/route.php`-এর `/admin` রাউট গ্রুপের ভেতরে (`->middleware([...])`-এর আগে) নিচের রাউটগুলো যোগ করুন:
...
- [ ] **ধাপ 2: ERP পারমিশন সিড SQL তৈরি করুন**
...
- [ ] **ধাপ 3: রাউট সিনট্যাক্স যাচাই করুন**
...
## Task 13: ড্যাশবোর্ড প্যানেল সম্প্রসারণ

**Files:**
- Modify: `app/admin/controller/DashboardController.php`

- [ ] **ধাপ 1: DashboardController আপডেট করে বিক্রয়, ইনভেন্টরি, ফাইন্যান্স প্যানেল যোগ করুন**

বিদ্যমান `DashboardController`-এ নিচের মেথডগুলো যোগ করুন:
...
DashboardController-এর শীর্ষে `use` যোগ করতে হবে:
```php
...
ক্লায়েন্ট রাউট রেজিস্ট্রেশন (`config/route.php`-এর `/api` গ্রুপের ভেতরে যোগ করুন):
...
## Task 15: Flutter + HarmonyOS ফ্রন্টএন্ড পেজ

**Flutter Web PC পেজ ডিরেক্টরি:**
```
...
প্রতিটি পেজ বিদ্যমান Flutter কোড প্যাটার্ন অনুসরণ করে (GetX Controller + ApiService সিঙ্গেলটন + Material 3 থিম)।

**HarmonyOS পেজ ডিরেক্টরি:**
```
...
- [ ] **ধাপ 1: Flutter উদাহরণ — পণ্য তালিকা পেজ**
...
- [ ] **ধাপ 2: HarmonyOS উদাহরণ — পণ্য তালিকা পেজ**
...
## Task 16: চূড়ান্ত যাচাই ও ইন্টিগ্রেশন টেস্ট

- [ ] **ধাপ 1: PHP সিনট্যাক্স চেক**
...
- [ ] **ধাপ 3: রাউট সম্পূর্ণতা যাচাই করুন**
...
- [ ] **ধাপ 4: ডাটাবেস টেবিল সম্পূর্ণতা চেক**
...
| স্তর | সংখ্যা | ফাইল অবস্থান |
|------|------|----------|
| ডাটাবেস টেবিল | 50টি | `database/migrations/` |
| Models | 44টি | `app/model/` |
| Controllers | ~35টি | `app/controller/{module}/` |
| Services | 2টি কোর | `app/service/inventory/`, `app/service/finance/` |
| রাউট গ্রুপ | 7টি মডিউল | `config/route.php` |
| পারমিশন এন্ট্রি | ~65টি | seed SQL |
| ড্যাশবোর্ড প্যানেল | 4টি | DashboardController |
| ক্লায়েন্ট API | প্রয়োজন অনুযায়ী | `app/api/v1/controller/` |
| Flutter পেজ | 12+টি | `apps/flutter/lib/app/pages/` |
| HarmonyOS পেজ | 6+টি | `apps/harmonyos/entry/src/main/ets/pages/` |

**মোট কোড পরিমাণ অনুমান:** PHP ~8,000 লাইন / SQL ~2,000 লাইন / Dart ~3,000 লাইন / ArkTS ~1,500 লাইন

**মূল ডিজাইন সিদ্ধান্ত:**
1. সব ID API স্তরে এনক্রিপ্ট হয়ে ট্রান্সফার হয় (hashids), ডাটাবেসে মূল BIGINT সংরক্ষিত থাকে
2. সংবেদনশীল ফিল্ড ডাটাবেস স্তরে স্বয়ংক্রিয় এনক্রিপ্ট/ডিক্রিপ্ট হয় (Encryptable cast)
3. ক্রস-মডিউল লিংকেজ Service স্তরে ইভেন্টের মাধ্যমে ট্রিগার হয়, সরাসরি পরস্পর কল করা হয় না
4. খরচ হিসাব মোবাইল ওয়েটেড অ্যাভারেজ পদ্ধতিতে হয়
5. প্রাপ্য/পরিশেয় ক্রয়/বিক্রয় ডকুমেন্ট থেকে স্বয়ংক্রিয় তৈরি হয়, ম্যানুয়াল নিষ্পত্তি করা হয়
6. ড্যাশবোর্ড ডেটা Redis-এ 5 মিনিট ক্যাশ হয়

**আচ্ছাদিত হয়নি এমন বিষয় (পরবর্তী ইটারেশনে):**
- Phase 2: ম্যানুফ্যাকচারিং ম্যানেজমেন্ট, প্রজেক্ট ম্যানেজমেন্ট
- WebSocket রিয়েল-টাইম পুশ (স্টক সতর্কতা, অনুমোদন রিমাইন্ডার)
- মাল্টি-টেন্যান্ট রূপান্তর
- আরও সমৃদ্ধ পারমিশন গ্রানুলারিটি (ডেটা পারমিশন)
- মোবাইল অফলাইন মোড
