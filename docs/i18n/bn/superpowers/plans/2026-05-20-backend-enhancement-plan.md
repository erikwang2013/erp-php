# ব্যাকএন্ড উন্নতি — বাস্তবায়ন পরিকল্পনা

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 13টি ব্যাকএন্ড উন্নতি বাস্তবায়ন: 3টি নতুন মিডলওয়্যার (CORS, RateLimit, OperationLog)、6টি নতুন কন্ট্রোলার、2টি মডেল পরিবর্তন、রুট + মিডলওয়্যার কনফিগারেশন।

**Architecture:** বিদ্যমান webman প্যাটার্ন অনুসরণ করে — মিডলওয়্যার `MiddlewareInterface::process()` বাস্তবায়ন করে, কন্ট্রোলার `BaseController` থেকে উত্তরাধিকারী, রুট ক্লোজার বা `[class, method]` দিয়ে সংজ্ঞায়িত। গ্লোবাল মিডলওয়্যার `config/middleware.php`-এ নিবন্ধিত, রুট-লেভেল মিডলওয়্যার `config/route.php`-এর রুট গ্রুপে মাউন্ট করা হয়।

**Tech Stack:** PHP 8.3+, webman v2, Redis (এক্সটেনশন), PhpSpreadsheet (ইমপোর্ট)

**নির্ভরতার ক্রম:** মডেল পরিবর্তন → মিডলওয়্যার → কন্ট্রোলার → রুট/কনফিগারেশন

---

### Task 1: AdminUser মডেলে SoftDeletes + Searchable trait যোগ করা

**Files:**
- Modify: `app/model/AdminUser.php`

- [ ] **ধাপ 1: AdminUser মডেল সংশোধন করুন**

`app/model/AdminUser.php`-এর ১০ম লাইনের পরে দুটি `use` ইমপোর্ট সন্নিবেশ করুন এবং ক্লাস বডির একদম শুরুতে দুটি trait ব্যবহার করুন:
...
একই সাথে ক্লাসের শেষে (`roles()` মেথডের পরে, ক্লাস সমাপ্তি ব্রেসের আগে) `toSearchableArray()` মেথড যোগ করুন:
...
- [ ] **ধাপ 1: Cors মিডলওয়্যার তৈরি করুন**
...
- [ ] **ধাপ 1: RateLimit মিডলওয়্যার তৈরি করুন**
...
- [ ] **ধাপ 1: OperationLog মিডলওয়্যার তৈরি করুন**
...
- [ ] **ধাপ 1: গ্লোবাল মিডলওয়্যার নিবন্ধন করুন**
...
- [ ] **ধাপ 1: HealthController তৈরি করুন**
...
### Task 7: সিস্টেম কনফিগারেশন CRUD কন্ট্রোলার

**Files:**
- Create: `app/admin/controller/ConfigController.php`

- [ ] **ধাপ 1: ConfigController তৈরি করুন**
...
### Task 8: অপারেশন লগ কোয়েরি কন্ট্রোলার

**Files:**
- Create: `app/admin/controller/LogController.php`

- [ ] **ধাপ 1: LogController তৈরি করুন**
...
### Task 9: ব্যক্তিগত সেন্টার কন্ট্রোলার (লগআউটসহ)

**Files:**
- Create: `app/admin/controller/ProfileController.php`
- Modify: `app/middleware/AdminAuth.php` (JWT ব্ল্যাকলিস্ট যাচাই)

- [ ] **ধাপ 1: AdminAuth মিডলওয়্যার সংশোধন করুন, JWT ব্ল্যাকলিস্ট যাচাই যোগ করুন**

`AdminAuth::process()` মেথডে, token বের করার পরে, JWT ডিকোডের আগে ব্ল্যাকলিস্ট যাচাই সন্নিবেশ করুন:
...
একই সাথে Redis ইমপোর্ট যোগ করতে হবে। ফাইল হেডার সংশোধন করুন:
...
- [ ] **ধাপ 2: ProfileController তৈরি করুন**
...
- [ ] **ধাপ 1: UploadController তৈরি করুন**
...
### Task 11: ব্যবহারকারী ব্যাচ অপারেশন

**Files:**
- Modify: `app/admin/controller/UserController.php`

- [ ] **ধাপ 1: UserController ক্লাসের শেষে দুটি মেথড যোগ করুন (`destroy` মেথডের পরে)**
...
- [ ] **ধাপ 1: ImportController তৈরি করুন**
...
### Task 13: রুট আপডেট — সব নতুন রুট + মিডলওয়্যার বাইন্ডিং

**Files:**
- Modify: `config/route.php`

- [ ] **ধাপ 1: রুট কনফিগারেশন আপডেট করুন**

`/admin` রুট গ্রুপের ভেতরে নতুন রুট যোগ করুন, একই সাথে OperationLog মিডলওয়্যার যুক্ত করুন। সম্পূর্ণ রুট ফাইল:
...
বাস্তবায়ন সম্পন্ন হওয়ার পর, ধাপে ধাপে যাচাই করুন:
