# P0 Flutter Web অ্যাডমিন প্যানেল — বাস্তবায়ন পরিকল্পনা

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Flutter Web অ্যাডমিন প্যানেলকে বিদ্যমান ~12 পেজ থেকে সম্প্রসারিত করে সব 14টি ব্যাকএন্ড মডিউল কভার করা 40+ পেজ তৈরি করা, এবং পুনর্ব্যবহারযোগ্য সাধারণ কম্পোনেন্ট লাইব্রেরি প্রতিষ্ঠা করা।

**Architecture:** কনফিগারেশন-চালিত ডাইনামিক মেনু সিস্টেমে (`menu_config.dart`) পুনর্গঠন, PC থ্রি-কলাম লেআউট (কোলাপসিবল সাইডবার + হেডার + কনটেন্ট এরিয়া), সাধারণ CRUD কম্পোনেন্ট (DataTableWrapper/FormDialog/ConfirmDialog) যা সব মডিউল পেজ পুনরায় ব্যবহার করে।
...
### Task 1: মেনু কনফিগারেশন ও রুট পুনর্গঠন

**Files:**
- Create: `apps/flutter/lib/app/config/menu_config.dart`
- Modify: `apps/flutter/lib/app/layouts/admin_layout.dart` (সম্পূর্ণ পুনর্লিখন)
- Modify: `apps/flutter/lib/main.dart` (রুট নিবন্ধন)
- Move: বিদ্যমান পেজ ফাইলগুলো নতুন ডিরেক্টরি স্ট্রাকচারে স্থানান্তর

- [ ] **ধাপ 1: menu_config.dart তৈরি করুন**
...
- [ ] **ধাপ 2: ডাইনামিক মেনু রেন্ডারিংয়ের জন্য AdminLayout পুনর্লিখন**
...
- [ ] **ধাপ 3: main.dart-এ কেন্দ্রীভূত রুট নিবন্ধন আপডেট**
...
- [ ] **ধাপ 4: বিদ্যমান পেজ ফাইল ডিরেক্টরি স্থানান্তর**
...
- [ ] **ধাপ 5: কম্পাইল যাচাই** — Run: `cd apps/flutter && flutter analyze`

- [ ] **ধাপ 6: কমিট**

---

### Task 2: সাধারণ কম্পোনেন্ট — DataTableWrapper

**Files:** Create: `apps/flutter/lib/app/widgets/data_table_wrapper.dart`

- [ ] **ধাপ 1: কম্পোনেন্ট তৈরি করুন**
...
### Task 3: সাধারণ কম্পোনেন্ট — FormDialog + ConfirmDialog + StatCard
...
### Task 4-20: মডিউল পেজ ব্যাচ তৈরি

> প্রতিটি task একটি মডিউল কভার করে, সব লিস্ট পেজ একীভূত CRUD প্যাটার্ন অনুসরণ করে।

**CRUD লিস্ট পেজ স্ট্যান্ডার্ড টেমপ্লেট** (প্রতি পেজ ~70 লাইন):
...
| Task | মডিউল | ডিরেক্টরি | পেজ সংখ্যা |
|------|------|------|--------|
| 4 | পণ্য | `pages/product/` | 4 |
| 5 | লেনদেন পক্ষ | `pages/partner/` | 4 |
| 6 | ক্রয় | `pages/purchase/` | 4 |
| 7 | বিক্রয় | `pages/sales/` | 4 |
| 8 | ইনভেন্টরি | `pages/inventory/` | 5 |
| 9 | ফাইন্যান্স 1 (ভাউচার/প্রাপ্য-পরিশেয়/রসিদ/জার্নাল/রিইম্বার্সমেন্ট) | `pages/finance/` | 6 |
| 10 | ফাইন্যান্স 2 (লেজার/রিপোর্ট/সম্পদ/কর/মুদ্রা/বাজেট/খরচ) | `pages/finance/` | 7 |
| 11 | CRM | `pages/crm/` | 8 |
| 12 | OMS | `pages/oms/` | 4 |
| 13 | WMS | `pages/wms/` | 7 |
| 14 | TMS | `pages/tms/` | 5 |
| 15 | নির্মাণ | `pages/manufacturing/` | 5 |
| 16 | HR | `pages/hr/` | 6 |
| 17 | প্রকল্প | `pages/project/` | 3 |
| 18 | অ্যাপ্রুভাল ওয়ার্কফ্লো | `pages/workflow/` | 2 |
| 19 | নোটিফিকেশন | `pages/notification/` | 1 |
| 20 | কাস্টম রিপোর্ট | `pages/report/` | 2 |

প্রতিটি task শেষে: `main.dart` আপডেট করে সংশ্লিষ্ট রুট যোগ করুন + `flutter analyze` দিয়ে যাচাই করুন।

---

### Task 21: Dashboard সম্প্রসারণ
...
- [ ] **ধাপ 1: বিদ্যমান 4 Tab-এর ভিত্তিতে OMS/WMS/TMS ড্যাশবোর্ড যোগ করুন**

TabBar-তে যোগ করুন: `const Tab(text: 'OMS')`, `const Tab(text: 'WMS')`, `const Tab(text: 'TMS')`
TabBarView-তে সংশ্লিষ্ট `_omsTab()`, `_wmsTab()`, `_tmsTab()` মেথড যোগ করুন, প্রতিটিতে 3-4টি StatCard।

- [ ] **ধাপ 2: কমিট**

---

### Task 22: সম্পূর্ণ রুট নিবন্ধন + চূড়ান্ত যাচাই

- [ ] **ধাপ 1: main.dart-এ সব GetPage নিবন্ধন পূরণ করুন**
- [ ] **ধাপ 2: `flutter analyze` → No issues found**
- [ ] **ধাপ 3: `flutter run -d chrome` দিয়ে ম্যানুয়ালি সব 14 মডিউল মেনু ও পেজ গ্রহণ করুন**
- [ ] **ধাপ 4: কমিট**

---

### Task 23: HarmonyOS সারিবদ্ধকরণ

**Files:** `apps/harmonyos/entry/src/main/ets/pages/`-এর অধীনে নতুন পেজ তৈরি করুন

- [ ] OMS/WMS/TMS/নির্মাণ/HR/অ্যাপ্রুভাল/নোটিফিকেশন/রিপোর্ট মোট 8টি মডিউলের ArkTS পেজ তৈরি করুন
- [ ] রুট নিবন্ধনের জন্য `main_pages.json` আপডেট করুন
- [ ] কম্পাইল যাচাই

---

**গ্রহণযোগ্যতার মানদণ্ড (P0 complete):**
- [ ] Flutter Web সাইডবারের 14টি মডিউল সব নেভিগেবল
- [ ] সব CRUD লিস্ট পেজ সঠিকভাবে লোড, পেজিনেশন, সার্চ
- [ ] তৈরি/সম্পাদনা ডায়ালগ + ডিলিট পাসওয়ার্ড কনফার্মেশন কার্যকর
- [ ] PC/ট্যাবলেট/মোবাইল রেসপন্সিভ লেআউট সঠিক
- [ ] JWT মেয়াদ শেষে স্বয়ংক্রিয় রিফ্রেশ
- [ ] HarmonyOS পেজ সংখ্যা ≥ Flutter পেজের 80%
