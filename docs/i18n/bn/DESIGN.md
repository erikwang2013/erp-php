# ওপেন অ্যাডমিন ব্যাকএন্ড — ডিজাইন ডকুমেন্ট

> Copyright (c) 2026 erik <erik@erik.xyz> — https://erik.xyz

> বিস্তারিত Mermaid আর্কিটেকচার ডায়াগ্রাম [ARCHITECTURE.md](ARCHITECTURE.md) দেখুন (GitHub/GitLab/VS Code এ স্বয়ংক্রিয় রেন্ডার হয়)।
...
- সুবিধা: গ্লোবালি ইউনিক, ডিস্ট্রিবিউটেড-ফ্রেন্ডলি, ট্রেন্ড ইনক্রিজিং ইনডেক্সের জন্য উপযোগী, বিজনেস ভলিউম প্রকাশ করে না
- কনফিগ: datacenter_id(0-31) + worker_id(0-31), 1024 নোড কনকারেন্সি সাপোর্ট
...
Redis Sorted Set স্লাইডিং উইন্ডো অ্যালগরিদম ভিত্তিক, অ্যাটমিক Lua স্ক্রিপ্ট এক্সিকিউশন:
...
| 403 | অনুমতি নেই | ইউজার রোলে প্রয়োজনীয় অনুমতি নেই |
| 404 | নেই | রিসোর্স পাওয়া যায়নি |
| 422 | ভ্যালিডেশন ব্যর্থ | ফর্ম প্যারামিটার নিয়ম মানে না / পাসওয়ার্ড কনফার্মেশন ব্যর্থ |
...
### 4.7 সংবেদনশীল অপারেশনে দ্বিতীয় কনফার্মেশন

ইউজার/রোল/পারমিশন ডিলিট ইত্যাদি সংবেদনশীল অপারেশনে, রিকোয়েস্ট বডিতে বর্তমান ইউজারের পাসওয়ার্ড দিয়ে আইডেন্টিটি পুনরায় যাচাই করতে হবে:
...
ফ্রন্টএন্ড ডিলিট অপারেশন ট্রিগার করার আগে কনফার্মেশন ডায়ালগ পপ-আপ করে, ইউজারের পাসওয়ার্ড সংগ্রহ করে রিকোয়েস্ট পাঠায়।

## 5. ফ্রন্টএন্ড ডিজাইন

### 5.1 Flutter Web ম্যানেজমেন্ট ব্যাকএন্ড
...
বৈশিষ্ট্য: সাইডবার কোলাপসিবল, Material 3 ডুয়াল থিম, হাই-ডেনসিটি ডেটা টেবিল, পপ-আপ Dialog, মাউস হোভার ইন্টারঅ্যাকশন

### 5.2 HarmonyOS মোবাইল এন্ড
...
| DashboardPage | `pages/DashboardPage` | স্ট্যাটিসটিক্স কার্ড + সাম্প্রতিক অপারেশন |
| UserListPage | `pages/UserListPage` | ইউজার লিস্ট, সার্চ + ড্রপ-ডাউন রিফ্রেশ + উপরে স্ক্রল লোড |
| UserDetailPage | `pages/UserDetailPage` | যোগ/এডিট/দেখা/ডিলিট (AlertDialog কনফার্মেশন) |
...
| লেয়ার | ব্যবস্থা |
|------|------|
| মেথড রেস্ট্রিকশন | SecurityFilter HTTP মেথড হোয়াইটলিস্ট, শুধুমাত্র GET/POST/PUT/DELETE/OPTIONS/HEAD অনুমোদিত, নন-স্ট্যান্ডার্ড মেথডে 405 রিটার্ন |
| অ্যাটাক ইন্টারসেপশন | SecurityFilter মিডলওয়্যার, XSS/SQL ইনজেকশন/পাথ ট্রাভার্সাল/কমান্ড ইনজেকশন/CSRF ডিটেকশন ইন্টারসেপশন |
| হিউম্যান ভেরিফিকেশন | ক্লিক ক্যাপচা (Click Captcha), লগইন/রেজিস্ট্রেশনে বাধ্যতামূলক ভ্যালিডেশন |
| অ্যাকাউন্ট লক | টানা ৫ বার লগইন ব্যর্থ হলে অ্যাকাউন্ট ১৫ মিনিট লক, লক অবস্থায় 429 রিটার্ন |
| সেশন লিমিট | একই ইউজারের সর্বোচ্চ ৩টি কনকারেন্ট Token, অতিরিক্ত হলে সবচেয়ে পুরনো Token স্বয়ংক্রিয় ব্ল্যাকলিস্ট |
| রেট লিমিট | RateLimit মিডলওয়্যার, Redis স্লাইডিং উইন্ডো, Lua অ্যাটমিক |
| CSP | Content-Security-Policy হেডার রিসোর্স সোর্স সীমিত করে, XSS ও ডেটা ইনজেকশন প্রতিরোধ |
| অপারেশন কনফার্মেশন | ডিলিট ইত্যাদি সংবেদনশীল অপারেশনে বর্তমান ইউজারের পাসওয়ার্ড দিয়ে দ্বিতীয় কনফার্মেশন প্রয়োজন |
| ট্রান্সমিশন | HTTPS + JWT Bearer Token |
| ইন্টারফেস ID | Hashids এনক্রিপশন, বাইরে থেকে প্রকৃত ID রিভার্স করা যায় না |
| রিকোয়েস্ট বডি | AES-256-CBC সংবেদনশীল ফিল্ড এনক্রিপশন |
| ডেটাবেস | BIGINT প্রাইমারি কী (অটো-ইনক্রিমেন্ট প্রকাশ করে না) |
| ডেটাবেস | AES-128-ECB সংবেদনশীল ফিল্ড এনক্রিপ্টেড স্টোরেজ |
| অথেনটিকেশন | JWT HS256, 2h মেয়াদ + refresh token |
| অথোরাইজেশন | RBAC, method.path গ্র্যানুলারিটি পারমিশন কন্ট্রোল |
| অডিট | OperationLog সব অপারেশন রেকর্ড করে (source সোর্স এন্ড স্বয়ংক্রিয় ডিটেকশন সহ) |
...
|------|------|------|
| লিস্ট ডিসপ্লে | phone | ডিসেন্সিটাইজেশন: 138****1234 |
| লিস্ট ডিসপ্লে | email | ডিসেন্সিটাইজেশন: a***@example.com |
| ডিটেইল দেখা | phone/email | ডিক্রিপশন ইন্টারফেস প্রয়োজন |
| Excel এক্সপোর্ট | phone/email | ডিসেন্সিটাইজ করে এক্সপোর্ট |
| PDF এক্সপোর্ট | সব ফিল্ড | ডিসেন্সিটাইজেশন + অপসারণযোগ্য নয় কপিরাইট ওয়াটারমার্ক |
| স্টোরেজ | phone/email/id_card | encryptable এনক্রিপশনে সাইফারটেক্সট |
...
### 8.2 Docker Compose (প্রোডাকশন পরিবেশের জন্য প্রস্তাবিত)

প্রজেক্ট রুটের `docker-compose.yml` উপরের টপোলজির সব সার্ভিস অর্কেস্ট্রেট করে:

| সার্ভিস | ইমেজ/বিল্ড | পোর্ট | ব্যাখ্যা |
|------|----------|------|------|
| `nginx` | nginx:alpine | 80, 443 | রিভার্স প্রক্সি + স্ট্যাটিক ফাইল + Gzip |
| `app` | লোকাল `Dockerfile` বিল্ড | 8788 | PHP 8.3 + OPcache + webman |
| `mysql` | mysql:8.0 | 3306 | মেইন ডেটাবেস, ডেটা ভলিউম পার্সিস্টেন্স |
| `redis` | redis:7-alpine | 6379 | ক্যাশ / রেট লিমিট / ক্যাপচা |
| `elasticsearch` | elasticsearch:8.x | 9200 | ফুল-টেক্সট রিট্রিভাল |

চালু করার আগে `docker-compose.yml` এর `JWT_SECRET`、`HASHIDS_SALT`、`ENCRYPTION_KEY` ইত্যাদি কী র্যান্ডম স্ট্রিং দিয়ে প্রতিস্থাপন করুন।
...
`database/backup/backup.sh` — mysqldump + gzip ব্যাকআপ, ৩০ দিন আগের পুরনো ব্যাকআপ স্বয়ংক্রিয় ক্লিনআপ।
`database/backup/restore.sh` — ইন্টারঅ্যাকটিভ নির্বাচন ও রিস্টোর।

### 8.5 মনিটরিং

`GET /metrics` এন্ডপয়েন্ট (`MetricsController`) Prometheus text format-এ ৫টি gauge মেট্রিক এক্সপোজ করে: HTTP রিকোয়েস্ট মোট সংখ্যা, সক্রিয় ইউজার সংখ্যা, ডেটাবেস/Redis কানেকশন স্ট্যাটাস, মেমোরি ব্যবহারের পরিমাণ।
...
| Elasticsearch | 7.x | 8.x 3 নোড ক্লাস্টার |
