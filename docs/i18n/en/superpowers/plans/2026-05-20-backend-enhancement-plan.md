...
**Goal:** Implement 13 backend enhancements: 3 new middleware (CORS, RateLimit, OperationLog), 6 new controllers, 2 model changes, routing + middleware configuration.

**Architecture:** Follow the existing webman patterns — middleware implements `MiddlewareInterface::process()`, controllers extend `BaseController`, routes are defined with closures or `[class, method]`. Global middleware is registered in `config/middleware.php`, route-level middleware is attached to route groups in `config/route.php`.

**Tech Stack:** PHP 8.3+, webman v2, Redis (extension), PhpSpreadsheet (import)

**Dependency order:** model changes → middleware → controllers → routing/configuration

---

### Task 1: Add SoftDeletes + Searchable traits to AdminUser model
...
Insert two `use` imports after line 10 of `app/model/AdminUser.php`, and use the two traits at the very beginning of the class body:
...
Also add the `toSearchableArray()` method at the end of the class (after the `roles()` method, before the closing brace):
...
Change the contents of `config/middleware.php` to:
...
### Task 9: Profile Controller (with Logout)
...
The Redis import must also be added. Modify the file header:
...
Add the new routes inside the `/admin` route group and append the OperationLog middleware. Full route file:
