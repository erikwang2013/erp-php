# ERP Business Module Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** implement complete ERP business modules — inventory/purchase/sales + finance + CRM — on top of the existing webman v2 system-management base

**Architecture:** three layers: Controller → Service → Model. All models are shared under `app/model/`, controllers are organized per module in `app/controller/{module}/`, business logic lives in `app/service/{module}/`. The ~50 database tables uniformly use the `erik_` prefix and BIGINT snowflake primary keys. API versions are handled via the `API-Version` request header + `ApiVersion` middleware.

**Tech Stack:** PHP 8.3+ / webman v2 / MySQL 8.0+ / Eloquent ORM / erikwang2013/* package family
...
- API version: request header `API-Version` (e.g., `v1`, `v2`), defaults to `v1` when missing, not reflected in the URL

---

## Task 1: Database Migration — Product Base Data

**Files:**
- Create: `database/migrations/2026_05_22_000003_product_base_tables.sql`

- [ ] **Step 1: Create the product base data migration SQL**
...
## Task 2: Database Migration — Purchase Module

**Files:**
- Create: `database/migrations/2026_05_22_000004_purchase_tables.sql`

- [ ] **Step 1: Purchase module 9-table migration SQL**
...
## Task 3: Database Migration — Sales Module

**Files:**
- Create: `database/migrations/2026_05_22_000005_sales_tables.sql`

- [ ] **Step 1: Sales module 9-table migration SQL**
...
## Task 4: Database Migration — Inventory + Finance + CRM Modules
...
- [ ] **Step 1: Inventory module 11 tables**
...
- [ ] **Step 2: Finance module 11 tables**
...
## Task 5: Product Base Data Models
...
- [ ] **Step 2: Create the remaining base data Models (following the same pattern)**
...
- [ ] **Step 1: Create all purchase and sales Models**

All models follow the same pattern: `SoftDeletes` trait (for main tables and order classes), `$incrementing = false; $keyType = 'int';`, BelongsTo relationships. Only PurchaseOrder (the most representative) is shown below:
...
**Commands to create the remaining purchase/sales Models:**
...
Since the 19 purchase and sales Models share a uniform pattern, they are created in bulk via a script. Each Model's specific `$fillable`, `$casts`, and relationship methods will be filled in as needed when the corresponding Controllers are implemented later.
...
## Task 7: Inventory + Finance + CRM Models
...
- [ ] **Step 1: Bulk-create inventory/finance/CRM Models**
...
- [ ] **Step 2: Complete the $fillable and relationships for key Models**
...
## Task 8: Product Base Data Controllers
...
All controllers extend `app\admin\controller\BaseController`, with the same CRUD pattern as the existing `UserController`. Below is the complete `ProductController` as an example:
...
- [ ] **Step 2: Create the remaining base data Controllers**

The other 6 controllers (Category, Brand, Warehouse, Location, Supplier, Customer) are standard CRUD with the same structure as above. After creation, write them to files in bulk:
...
---
## Task 9: Inventory Service — Core Stock In/Out Logic

This is the core of cross-module integration; purchase receiving, sales delivery, etc. all depend on it.
...
- [ ] **Step 2: Create a test to verify the inventory service**
...
## Task 10: Finance Service — AR/AP Auto-Generation and Settlement

**Files:**
- Create: `app/service/finance/FinanceService.php`

- [ ] **Step 1: Create FinanceService**
...
## Task 11: Purchase Receive Controller (Cross-Module Integration Example)

This is the most typical cross-module business operation: receiving → auto stock-in → AP generation. It shows the full integration code.

**Files:**
- Create: `app/controller/purchase/ReceiveController.php`

- [ ] **Step 1: Create the purchase receive controller**
...
## Task 12: Route Extension + Permission Seeds

**Files:**
- Modify: `config/route.php`
- Create: `database/migrations/2026_05_22_000009_seed_erp_permissions.sql`

- [ ] **Step 1: Extend the route file**

Append the following routes inside the `/admin` route group in `config/route.php` (before `->middleware([...])`):
...
- [ ] **Step 2: Create the ERP permission seed SQL**
...
---
## Task 13: Dashboard Panel Extension

**Files:**
- Modify: `app/admin/controller/DashboardController.php`

- [ ] **Step 1: Update DashboardController to add sales, inventory, and finance panels**

Append the following methods to the existing `DashboardController`:
...
The top of DashboardController also needs the `use` statements:
```php
...
Client route registration (append inside the `/api` group in `config/route.php`):
...
## Task 15: Flutter + HarmonyOS Frontend Pages

**Flutter Web PC page directories:**
```
...
Each page follows the existing Flutter code patterns (GetX Controller + ApiService singleton + Material 3 theme).

**HarmonyOS page directories:**
```
...
- [ ] **Step 1: Flutter example — product list page**
...
- [ ] **Step 2: HarmonyOS example — product list page**
...
## Task 16: Final Verification and Integration Tests
...
- [ ] **Step 4: Database table completeness check**
...
| Layer | Count | File Location |
|------|------|----------|
| Database tables | 50 | `database/migrations/` |
| Models | 44 | `app/model/` |
| Controllers | ~35 | `app/controller/{module}/` |
| Services | 2 core | `app/service/inventory/`, `app/service/finance/` |
| Route groups | 7 modules | `config/route.php` |
| Permission entries | ~65 | seed SQL |
| Dashboard panels | 4 | DashboardController |
| Client APIs | extended as needed | `app/api/v1/controller/` |
| Flutter pages | 12+ | `apps/flutter/lib/app/pages/` |
| HarmonyOS pages | 6+ | `apps/harmonyos/entry/src/main/ets/pages/` |

**Estimated total code volume:** PHP ~8,000 lines / SQL ~2,000 lines / Dart ~3,000 lines / ArkTS ~1,500 lines

**Key design decisions:**
1. All IDs are encrypted in transit at the API layer (hashids); the database stores the raw BIGINT
2. Sensitive fields are automatically encrypted/decrypted at the database layer (Encryptable cast)
3. Cross-module integration is triggered through the Service layer via events, not direct inter-module calls
4. Costing uses the moving weighted average method
5. AR/AP is auto-generated from purchase/sales documents, settled manually
6. Dashboard data is Redis-cached for 5 minutes

**Not covered (needs follow-up iterations):**
- Phase 2: manufacturing management, project management
- WebSocket real-time push (inventory alerts, approval reminders)
- Multi-tenancy refactor
- Finer-grained permissions (data permissions)
- Mobile offline mode
