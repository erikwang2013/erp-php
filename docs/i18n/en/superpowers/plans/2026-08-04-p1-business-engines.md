# P1 Business Depth Engines — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) to implement this plan task-by-task.

**Goal:** Upgrade the three core modules — finance, payroll, MRP — from CRUD skeletons into industrial-grade business calculation engines

**Architecture:** Each engine is an independent Service class following the `app/service/{module}/` directory convention, with dependencies injected via constructors, `DB::transaction()` guaranteeing data consistency, and PHPUnit tests covering the key algorithms

**Tech Stack:** PHP 8.3, workerman/webman-framework 2.x, Eloquent ORM, PHPUnit 12

---

## Task 1: Finance Double-Entry Accounting Engine
...
The existing `FinanceVoucher` only does simple CRUD with no debit/credit balance validation. The `FinanceVoucherItem` table already has the `voucher_id`, `account_subject_id`, `debit_amount`, `credit_amount`, `summary` fields. Core rule: "every debit has a credit, and debits must equal credits".

- [ ] **Step 1: Create DoubleEntryService**
...
Modify the `store()` method of `app/controller/finance/VoucherController.php`: if the request contains `items`, call `DoubleEntryService::createVoucher()`; otherwise fall back to the original CRUD.
...
- [ ] **Step 1: Create SalaryEngineService**
...
## Task 4: Integrate into Existing Controllers
...
- [ ] **Step 1: VoucherController integration** — `store()` detects the `items` parameter; if present it goes through `DoubleEntryService`, otherwise the original CRUD
- [ ] **Step 2: SalaryController integration** — `store()` calls `SalaryEngineService::calculate()` to auto-compute payroll details
- [ ] **Step 3: MrpController integration** — `generate()` calls `MrpEngineService::explodeBom()` + `calculateNetRequirement()`
...
- [ ] Saving a voucher with unequal debits and credits → 422「借贷不平衡」
...
- [ ] BOM expands multi-level recursion down to raw materials, including loss rates
- [ ] `php vendor/bin/phpunit --testdox` all passing
