...
- Modify: `apps/flutter/lib/app/layouts/admin_layout.dart` (全量重写)
- Modify: `apps/flutter/lib/main.dart` (路由注册)
- Move: 现有页面文件迁移到新目录结构

- [ ] **Step 1: 创建 menu_config.dart**
...
- [ ] **Step 2: 重写 AdminLayout 支持动态菜单渲染**
...
- [ ] **Step 5: 验证编译** — Run: `cd apps/flutter && flutter analyze`

- [ ] **Step 6: Commit**

---

### Task 2: 通用组件 — DataTableWrapper
...
### Task 3: 通用组件 — FormDialog + ConfirmDialog + StatCard
...
> 每个 task 覆盖一个模块，所有列表页遵循统一的 CRUD 模式。

**CRUD 列表页标准模板**（每个页面 ~70 行）：
...
每个 task 完成后：更新 `main.dart` 添加对应路由 + `flutter analyze` 验证。

---

### Task 21: Dashboard 扩展
...
- [ ] **Step 1: 在现有 4 Tab 基础上增加 OMS/WMS/TMS 看板**

TabBar 增加: `const Tab(text: 'OMS')`, `const Tab(text: 'WMS')`, `const Tab(text: 'TMS')`
TabBarView 增加对应的 `_omsTab()`, `_wmsTab()`, `_tmsTab()` 方法，各含 3-4 个 StatCard。
...
- [ ] **Step 1: 补全 main.dart 所有 GetPage 注册**
- [ ] **Step 2: `flutter analyze` → No issues found**
- [ ] **Step 3: `flutter run -d chrome` 手动验收全部 14 模块菜单和页面**
- [ ] **Step 4: Commit**

---

### Task 23: HarmonyOS 对齐

**Files:** 在 `apps/harmonyos/entry/src/main/ets/pages/` 下创建新页面

- [ ] 创建 OMS/WMS/TMS/制造/HR/审批/通知/报表共 8 个模块的 ArkTS 页面
- [ ] 更新 `main_pages.json` 注册路由
- [ ] 验证编译

---

**验收标准 (P0 complete):**
- [ ] Flutter Web 侧边栏 14 模块全部可导航
- [ ] 所有 CRUD 列表页正常加载、分页、搜索
- [ ] 创建/编辑弹窗 + 删除密码确认可用
- [ ] PC/平板/手机响应式布局正常
- [ ] JWT 过期自动刷新
- [ ] HarmonyOS 页面数 ≥ Flutter 页面的 80%
