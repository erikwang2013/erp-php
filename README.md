...
| | 多组织 + 合并报表 | 多公司/账套核算 + 抵消分录（权益法/成本法）|
| | 存货/生产成本核算 | 生产领料→人工/制费归集→完工成本→成本差异结转 |
| | 承兑汇票 + 银企对账 | 票据台账 + 银行对账单导入自动核销 |
| | 进项发票池 + 数电票 | 进项发票管理 + 开票出口（适配器 + Mock 通道）|
| 🤝 CRM | 客户/联系人/跟进记录/营销活动/服务工单/分析报表/销售漏斗/公海池/报价/合同 | 客户全生命周期管理 |
| | 会员价值引擎 | 储值/积分/卡券会员运营 |
| ✅ 审批工作流 | 工作流定义/提交审批/批准/拒绝/撤回/我的审批 | 多节点审批流程引擎 |
| | 可视化流程设计器 | 画布配置节点/分支/驳回回边，复用审批引擎 |
| 🔔 消息通知 | 通知列表/已读标记/未读计数/全部已读 | 实时消息推送与状态追踪 |
| | 多渠道通知 | 短信/邮件渠道驱动（Mock 通道 + 日志 + 重试）|
| 📐 项目管理 | 项目/任务/工时记录 | 项目进度跟踪与资源管理 |
| | 项目成本与预算 | 工时×费率→项目成本归集 + 预算偏差 |
| 👤 人力资源 | 部门/员工/职位/考勤/请假/薪资 | 全面人事管理 |
| | 招聘/绩效/培训/社保 | 招聘漏斗 + KPI/360 考核 + 课程学分 + 社保基数规则与工资条 |
| 🏭 生产制造 | BOM/生产订单/工艺路线/工作站/MRP | 物料需求计划与生产执行 |
| | 工序报工/计件工资/委外核销 | MES 工序执行层 + 委外订单发料核销 |
| | 产能负荷分析 | 工作站日历 + 粗能力负荷报表 |
| | 批次/序列号追溯 | 正反向追溯链 + 近效期预警 |
...
| 🛠 设备管理(EAM) | 设备台账/保养计划/维修工单/备件 | 设备全生命周期管理 |
| | 点检扫码闭环 | 扫码点检，异常自动联动维修工单 |
| 🌐 平台与开放 | API 路径版本化 | 管理端 /admin/v1、客户端 /api/v1、开放 /open/v1（无版本请求头）|
...
├── tests/                      # PHPUnit 测试（113<!-- stats:test_files=113 --> 个测试文件，1048<!-- stats:tests=1048 --> 个测试方法，5040<!-- stats:assertions=5040 --> 条断言；口径见 docs/FUNCTIONS.md 附录）
├── resource/
...
启动服务后访问 `http://localhost:8788/install`，按引导完成 4 步安装：环境检查 → 数据库配置 → 管理员账号 → 一键安装。 在「数据库配置」一步可勾选**同时导入演示数据**（商品/规格/SKU/客户/供应商等，ID 段 41…，可按段清理）；默认不导入，生产环境保持不勾选。
