...
class _TimesheetPageState extends State<TimesheetPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await ApiService.instance.get('/admin/project/timesheet', params: params);
      final d = res['data'];
  ...
  Future<void> _create() async {
    await FormDialog.show(context, title: '新增', fields: _formFields(), onSubmit: (data) async {
      await ApiService.instance.post('/admin/project/timesheet', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    await FormDialog.show(context, title: '编辑', fields: _formFields(), initialData: row, onSubmit: (data) async {
      await ApiService.instance.put('/admin/project/timesheet/${row['id']}', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    await ConfirmDialog.show(context, title: '确认删除', content: '确定要删除「${row['name'] ?? row['code'] ?? ''}」吗？', onConfirm: (password) async {
      await ApiService.instance.delete('/admin/project/timesheet/${row['id']}', data: {'password': password});
      _load(); return true;
    });
  }
