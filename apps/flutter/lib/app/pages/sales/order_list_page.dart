...
class _SalesOrderListPageState extends State<SalesOrderListPage> {
  ...
  final int _limit = 20;
  String _keyword = '';
  static const List<String> _statuses = ['待审核', '已审核', '部分发货', '已发货', '已取消'];
  ...
  Future<void> _create() async {
    await FormDialog.show(context, title: '新增', fields: _formFields(), onSubmit: (data) async {
      await ApiService.instance.post('/admin/sales/order', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    await FormDialog.show(context, title: '编辑', fields: _formFields(), initialData: row, onSubmit: (data) async {
      await ApiService.instance.put('/admin/sales/order/${row['id']}', data: data);
      _load(); return true;
    });
  }
