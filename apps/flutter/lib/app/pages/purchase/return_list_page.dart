...
class _PurchaseReturnListPageState extends State<PurchaseReturnListPage> {
  ...
  Future<void> _create() async {
    ...
    await FormDialog.show(context, title: l10n.commonAdd, fields: _formFields(), onSubmit: (data) async {
      await ApiService.instance.post('/admin/v1/purchase/return', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    ...
    await FormDialog.show(context, title: l10n.commonEdit, fields: _formFields(), initialData: row, onSubmit: (data) async {
      await ApiService.instance.put('/admin/v1/purchase/return/${row['id']}', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    ...
    await ConfirmDialog.show(context, title: l10n.commonDeleteConfirm,
        content: l10n.purchaseDeleteConfirmMsg('${row['name'] ?? row['code'] ?? row['id']}'),
  ...
  List<FormFieldConfig> _formFields() => [
    FormFieldConfig(name: 'name', label: AppL10n.current.purchaseName, required: true),
    FormFieldConfig(name: 'code', label: AppL10n.current.purchaseCode),
  ];
  ...
  List<String> _columns() => [AppL10n.current.purchaseName, AppL10n.current.purchaseCode, AppL10n.current.commonAction];

  Map<String, dynamic> _rowToMap(Map<String, dynamic> r) => {
    AppL10n.current.purchaseName: r['name'] ?? '',
