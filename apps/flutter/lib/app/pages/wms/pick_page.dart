...
class _PickPageState extends State<PickPage> {
  ...
  /// 仓库下拉：后端建单必填外键（install.sql erp_wms_pick_task `warehouse_id` NOT NULL
  ...
  /// 按钮由行 status 门控（后端状态机：0=待拣货 1=拣货中 2=已完成）。
  /// 注：「拣货确认」(/confirm) 需 items 明细（实际拣货数量），本页无明细数据源，故未接入。
  ...
  Future<void> _create() async {
    ...
    await FormDialog.show(
      ...
      fields: _formFields(),
      onSubmit: (data) async {
        await ApiService.instance.post('/admin/v1/wms/pick', data: data);
        _load();
        return true;
      },
    );
  ...
  /// erp_wms_pick_task 无 `name` 列（原 name 字段提交后被后端丢弃）：只留真实列与必填外键。
  ...
  Widget build(BuildContext context) => DataTableWrapper(
    ...
    pageTitle: AppL10n.of(context).wmsPickTitle,
    moduleKey: 'wms',
  ...
  /// erp_wms_pick_task 无 `name` 列 → 原「名称」列恒空，只留真实列：单号 + 操作。
        ...
        children: [
          if ('${r['status']}' == '0')
            IconButton(
              ...
              tooltip: l.wmsPickStart,
              onPressed: () => _runAction(l.wmsPickStart, '/admin/v1/wms/pick/${r['id']}/start'),
            ),
          IconButton(
