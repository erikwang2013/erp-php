...
class _PackPageState extends State<PackPage> {
  ...
  /// 仓库下拉：后端建单必填外键（install.sql erp_wms_pack_task `warehouse_id` NOT NULL
  ...
  /// 按钮由行 status 门控（后端状态机：0=待打包 1=打包中 2=已完成，complete 要求 1）。
  /// /wms/pack/{id}/start 是「开始指定任务」(PackController::startTask)，
  /// 与 /wms/pack/start「按仓库新建任务」语义不同。
  ...
  Future<void> _create() async {
    ...
    await FormDialog.show(
      ...
      fields: _formFields(),
      onSubmit: (data) async {
        await ApiService.instance.post('/admin/v1/wms/pack', data: data);
        _load();
        return true;
      },
    );
  ...
  /// erp_wms_pack_task 无 `name` 列（原 name 字段提交后被后端丢弃）：只留真实列与必填外键。
  ...
  Widget build(BuildContext context) => DataTableWrapper(
    ...
    pageTitle: AppL10n.of(context).wmsPackTitle,
    moduleKey: 'wms',
  ...
  /// erp_wms_pack_task 无 `name` 列 → 原「名称」列恒空，只留真实列：单号 + 操作。
        ...
        children: [
          if ('${r['status']}' == '0')
            IconButton(
              ...
              tooltip: l.wmsPackStart,
              onPressed: () => _runAction(l.wmsPackStart, '/admin/v1/wms/pack/${r['id']}/start'),
            ),
          if ('${r['status']}' == '1')
            IconButton(
              icon: Icon(Icons.check_circle, size: 18, color: AppColors.of(context).success),
              tooltip: l.wmsPackComplete,
              onPressed: () => _runAction(l.wmsPackComplete, '/admin/v1/wms/pack/${r['id']}/complete'),
            ),
          IconButton(
