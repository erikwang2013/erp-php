...
class _ReceivingPageState extends State<ReceivingPage> {
  ...
  /// 仓库下拉：后端建单必填外键（install.sql erp_wms_receiving `warehouse_id` NOT NULL
  ...
  /// 按钮由行 status 门控（后端状态机：0=待收货 1=收货中 2=已完成）。
  ...
  Future<void> _create() async {
    ...
    await FormDialog.show(
      ...
      fields: _formFields(),
      onSubmit: (data) async {
        await ApiService.instance.post('/admin/v1/wms/receiving', data: data);
        _load();
        return true;
      },
    );
  ...
  /// erp_wms_receiving 无 `name` 列（原 name 字段提交后被后端丢弃）：只留真实列与必填外键。
  ...
  /// erp_wms_receiving 无 `name` 列 → 原「名称」列恒空，只留真实列：单号 + 操作。
        ...
        children: [
          if ('${r['status']}' == '0')
            IconButton(
              ...
              tooltip: l.wmsReceivingStart,
              onPressed: () => _runAction(l.wmsReceivingStart, '/admin/v1/wms/receiving/${r['id']}/start'),
            ),
          IconButton(
