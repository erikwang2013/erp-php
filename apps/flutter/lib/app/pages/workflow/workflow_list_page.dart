...
class _WorkflowListPageState extends State<WorkflowListPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await ApiService.instance.get('/admin/v1/workflow', params: params);
      final d = res['data'];
  ...
  Future<void> _create() async {
    ...
    await FormDialog.show(context, title: l10n.commonAdd, fields: _formFields(), onSubmit: (data) async {
      await ApiService.instance.post('/admin/v1/workflow', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    ...
    await FormDialog.show(context, title: l10n.commonEdit, fields: _formFields(), initialData: row, onSubmit: (data) async {
      await ApiService.instance.put('/admin/v1/workflow/${row['id']}', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    ...
    await ConfirmDialog.show(context, title: l10n.commonDeleteConfirm, content: l10n.commonDeleteContent(row['name'] ?? row['code'] ?? '${row['id']}'), onConfirm: (password) async {
      await ApiService.instance.delete('/admin/v1/workflow/${row['id']}', data: {'password': password});
      _load(); return true;
    });
  }
  ...
  Future<void> _submit(Map<String, dynamic> row) async {
    ...
    await FormDialog.show(context, title: l10n.workflowSubmitTitle(row['name'] ?? ''),
      ...
      onSubmit: (data) async {
        ...
        _load();
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.workflowSubmitSuccess)));
        }
        return true;
      },
    );
  ...
  Map<String, dynamic> _rowToMap(Map<String, dynamic> r) {
    ...
    return {
      ...
      l10n.commonAction: Row(mainAxisSize: MainAxisSize.min, children: [
        IconButton(icon: const Icon(Icons.send, size: 18, color: Colors.blue), tooltip: l10n.workflowSubmit, onPressed: () => _submit(r)),
