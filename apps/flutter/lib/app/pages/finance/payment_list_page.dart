...
/// 付款单页 — 覆盖 GET/POST/PUT/DELETE /admin/v1/finance/payment
/// 契约（erp_finance_payment，无 name 列）：code/supplier_id/bank_account_id/
/// amount/method(cash|bank|wechat|alipay|other)/status(0待审核|1已审核)/remark/paid_at
...
class _PaymentListPageState extends State<PaymentListPage> {
  ...
  /// 供应商选项（id→名称，id 为供应商列表行 hashid），打开新增/编辑弹窗前懒加载。
  Map<String, String> _supplierOptions = {};
  bool _suppliersLoaded = false;
  /// 付款账户下拉（可选字段；后端缺省 0），加载失败仅空选项不阻断提交。
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await ApiService.instance.get('/admin/v1/finance/payment', params: params);
      final d = res['data'];
  ...
  /// 加载供应商下拉选项（supplier_id 必填且为 hashid，取自 /admin/v1/supplier 列表行 id）。
  Future<bool> _ensureSuppliers() async {
    if (_suppliersLoaded) return true;
    try {
      final res = await ApiService.instance.get('/admin/v1/supplier', params: {'limit': '500'});
      final list = List<Map<String, dynamic>>.from((res['data'] ?? {})['list'] ?? []);
      _supplierOptions = { for (final s in list) '${s['id']}': '${s['name'] ?? s['code'] ?? ''}' };
  ...
  Future<void> _create() async {
    if (!await _ensureSuppliers() || !mounted) return;
    await _ensureBanks();
    if (!mounted) return;
    await FormDialog.show(context, title: AppL10n.of(context).commonAdd, fields: _formFields(), onSubmit: (data) async {
      await ApiService.instance.post('/admin/v1/finance/payment', data: _buildPayload(data));
      _load(); return true;
    });
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    ...
    await _ensureBanks();
    if (!mounted) return;
    // paid_at 为时间列，后端下发 ISO-UTC；输入框回填本地 `Y-m-d H:i:s`
    await FormDialog.show(context, title: AppL10n.of(context).commonEdit,
      ...
      initialData: {...row, 'paid_at': fmtDateTime(row['paid_at'])}, onSubmit: (data) async {
      await ApiService.instance.put('/admin/v1/finance/payment/${row['id']}', data: _buildPayload(data));
      _load(); return true;
    });
  }
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    await ConfirmDialog.show(context, title: AppL10n.of(context).commonDeleteConfirm,
      ...
      await ApiService.instance.delete('/admin/v1/finance/payment/${row['id']}', data: {'password': password});
      _load(); return true;
    });
  }

  // 与后端 PaymentController::store 契约对齐：code 留空自动生成 PAY+时间戳
  // （uk_code 唯一）；supplier 必填下拉；method 枚举下拉缺省 bank；银行账户可选
  ...
  List<FormFieldConfig> _formFields({Map<String, String>? methodOptions}) {
    ...
    return [
      FormFieldConfig(name: 'code', label: AppL10n.of(context).financePaymentCode, hint: AppL10n.of(context).financePaymentCodeHint),
      FormFieldConfig(
        name: 'supplier_id',
        label: AppL10n.of(context).fieldSupplier,
        required: true,
        type: FormFieldType.dropdown,
        options: _supplierOptions.keys.toList(),
        optionLabels: _supplierOptions,
      ),
      ...
      FormFieldConfig(name: 'paid_at',
        label: AppL10n.of(context).financePaidAt,
  ...
  Map<String, dynamic> _buildPayload(Map<String, String> data) {
    ...
    if (code.isEmpty) {
      final now = DateTime.now();
      code = 'PAY${now.year}${_p2(now.month)}${_p2(now.day)}${_p2(now.hour)}${_p2(now.minute)}${_p2(now.second)}';
    }
  ...
  Widget build(BuildContext context) => DataTableWrapper(
    ...
    pageTitle: AppL10n.current.financePaymentTitle,
    moduleKey: 'finance',
  ...
  List<String> _columns() => [AppL10n.current.financePaymentCode, AppL10n.current.fieldSupplier,
  ...
  Map<String, dynamic> _rowToMap(Map<String, dynamic> r) => {
    AppL10n.current.financePaymentCode: r['code'] ?? '',
    AppL10n.current.fieldSupplier: r['supplier_name'] ?? '',
