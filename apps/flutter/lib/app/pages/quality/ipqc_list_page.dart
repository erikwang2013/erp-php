...
class _IpqcListPageState extends State<IpqcListPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await ApiService.instance.get('/admin/quality/ipqc', params: params);
      final d = res['data'];
  ...
  Future<void> _create() async {
    await FormDialog.show(context, title: '新增', fields: _formFields(), onSubmit: (data) async {
      await ApiService.instance.post('/admin/quality/ipqc', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    await FormDialog.show(context, title: '编辑', fields: _formFields(), initialData: row, onSubmit: (data) async {
      await ApiService.instance.put('/admin/quality/ipqc/${row['id']}', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    await ConfirmDialog.show(context, title: '确认删除', content: '确定要删除「${row['code'] ?? ''}」吗？', onConfirm: (password) async {
      await ApiService.instance.delete('/admin/quality/ipqc/${row['id']}', data: {'password': password});
      _load(); return true;
    });
  }
  ...
  List<FormFieldConfig> _formFields() => const [
    ...
    FormFieldConfig(name: 'production_order_id', label: '生产工单ID', type: FormFieldType.number),
    FormFieldConfig(name: 'product_id', label: '商品ID', type: FormFieldType.number),
    FormFieldConfig(name: 'workstation_id', label: '工作站ID', type: FormFieldType.number),
  ...
  List<String> _columns() => ['检验单号', '工单ID', '商品ID', '检验/合格/不合格', '结果', '检验员', '操作'];

  Map<String, dynamic> _rowToMap(Map<String, dynamic> r) => {
    '检验单号': r['code'] ?? '',
    '工单ID': r['production_order_id'] ?? '',
    '商品ID': r['product_id'] ?? '',
