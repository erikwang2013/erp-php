...
class _IpqcListPageState extends State<IpqcListPage> {
  ...
  Map<String, String> _workOrders = {};
  Map<String, String> _products = {};
  Map<String, String> _workstations = {};
  ...
  /// 预取四张关联表；失败返回 false（提示后不弹表单）——空下拉会让用户以为无处可选，
  /// 口径同 wms/pack_page.dart::_ensureRefs。
  Future<bool> _ensureRefs({Map<String, dynamic>? row}) async {
    try {
      _workOrders = await _refOptions('/admin/v1/mfg/production', 'code');
      _products = await _refOptions('/admin/v1/product', 'name');
      _workstations = await _refOptions('/admin/v1/mfg/workstation', 'name');
    ...
    if (row != null) {
      _workOrders = _primed(_workOrders, row, 'production_order_id', 'production_order_code');
      _products = _primed(_products, row, 'product_id', 'product_name');
      _workstations = _primed(_workstations, row, 'workstation_id', 'workstation_name');
  ...
  List<FormFieldConfig> _formFields() {
    ...
    return [
      ...
      FormFieldConfig(name: 'production_order_id', label: l10n.fieldWorkOrderId, type: FormFieldType.dropdown, options: _workOrders.keys.toList(), optionLabels: _workOrders),
      ...
      FormFieldConfig(name: 'workstation_id', label: l10n.fieldWorkstationId, type: FormFieldType.dropdown, options: _workstations.keys.toList(), optionLabels: _workstations),
  ...
  Widget build(BuildContext context) => DataTableWrapper(
    ...
    pageTitle: AppL10n.current.qualityIpqcTitle,
    moduleKey: 'quality',
  ...
  List<String> _columns() {
    final l10n = AppL10n.current;
    return [l10n.fieldInspectNo, l10n.fieldWorkOrderIdShort, l10n.fieldProductName, l10n.qualityQtySummary, l10n.fieldResult, l10n.fieldInspector, l10n.commonAction];
  }
  ...
  /// `状态: 0=待处理 1=已完成`（erp_quality_ipqc_record:4163）——
  ...
  Map<String, dynamic> _rowToMap(Map<String, dynamic> r) {
    ...
    return {
      ...
      // 工单编码/商品名由后端 leftJoin 带出；关联行缺失/未关联（FK=0）落「-」——裸 hashid 不上屏
      l10n.fieldWorkOrderIdShort: r['production_order_code'] ?? '-',
