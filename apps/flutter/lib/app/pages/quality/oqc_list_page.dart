...
class OqcListPage extends StatefulWidget {
  const OqcListPage({super.key});
  @override
  State<OqcListPage> createState() => _OqcListPageState();
}

class _OqcListPageState extends State<OqcListPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await ApiService.instance.get('/admin/quality/oqc', params: params);
      final d = res['data'];
  ...
  Future<void> _create() async {
    await FormDialog.show(context, title: '新增', fields: _formFields(), onSubmit: (data) async {
      await ApiService.instance.post('/admin/quality/oqc', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _edit(Map<String, dynamic> row) async {
    await FormDialog.show(context, title: '编辑', fields: _formFields(), initialData: row, onSubmit: (data) async {
      await ApiService.instance.put('/admin/quality/oqc/${row['id']}', data: data);
      _load(); return true;
    });
  }
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    await ConfirmDialog.show(context, title: '确认删除', content: '确定要删除「${row['code'] ?? ''}」吗？', onConfirm: (password) async {
      await ApiService.instance.delete('/admin/quality/oqc/${row['id']}', data: {'password': password});
      _load(); return true;
    });
  }
  ...
  List<FormFieldConfig> _formFields() => const [
    ...
    FormFieldConfig(name: 'delivery_id', label: '发货单ID', type: FormFieldType.number),
  ...
  List<String> _columns() => ['检验单号', '发货单ID', '商品ID', '检验/合格/不合格', '结果', '检验员', '操作'];
