...
/** 图标名（运行时经 ui/icon 映射到 antd 图标；与 React 端 IconName 同名同义） */
...
export interface FilterDef {
  key: string;
  label: string;
  /** 静态选项（与 source 二选一），首项「全部」value 为 null */
  options?: FieldOption[];
  /** 远程选项：复用表单的 FieldSource 机制（OptionSource：按 endpoint 缓存 + in-flight 去重） */
  source?: FieldSource;
}
...
export interface ResourceConfig {
  ...
  /** 筛选下拉：单个或一组（数组即多个下拉，每项一个查询参数；值为 null 的不下发该参数） */
