 ...
 * leave/other（registry 外但确实会出现），与 HarmonyOS ApprovalPage 的 TARGET_TYPE_KEYS 逐字一致；
 * 文案本身即词典 key（「销售订单」等 6 条已在 zh-en 词典，其余 11 语种随之译出）。
 * 表外值（后端 submit 无白名单）由 kind:'map' 原样直出，不落 '-'。
 */
