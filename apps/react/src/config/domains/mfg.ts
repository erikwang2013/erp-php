...
import { dateCol, docStatus, intCol, statusCol, textCol } from '@/config/cells';
...
export const mfgMenus: MenuGroup[] = [
  ...
  },
  {
    ...
    moduleKey: 'quality',
    children: [
      { label: '检验标准', path: '/quality/standard', cfg: res('检验标准', '/admin/v1/quality/standard', { moduleKey: 'quality', deleteNeedsPassword: true, fields: [{ key: 'name', label: '标准名称', required: true }] }) },
      { label: '来料检验 IQC', path: '/quality/iqc', cfg: res('来料检验', '/admin/v1/quality/iqc', { moduleKey: 'quality', deleteNeedsPassword: true, filters: QC.filter, columns: [textCol('code', '编号', true), intCol('inspected_qty', '检验数量'), intCol('passed_qty', '合格数'), statusCol(QC.dict), dateCol('created_at', '创建时间')], fields: [{ key: 'code', label: '检验单号', required: true }, { key: 'inspected_qty', label: '检验数量', required: true, type: 'number' }, { key: 'result', label: '检验结果', required: true, type: 'select', options: [{ label: '合格', value: 'pass' }, { label: '不合格', value: 'reject' }] }] }) },
      { label: '过程检验 IPQC', path: '/quality/ipqc', cfg: res('过程检验', '/admin/v1/quality/ipqc', { moduleKey: 'quality', deleteNeedsPassword: true, filters: QC.filter, fields: [{ key: 'code', label: '检验单号', required: true }, { key: 'inspected_qty', label: '检验数量', required: true, type: 'number' }, { key: 'result', label: '检验结果', required: true, type: 'select', options: [{ label: '合格', value: 'pass' }, { label: '不合格', value: 'reject' }] }] }) },
      { label: '出货检验 OQC', path: '/quality/oqc', cfg: res('出货检验', '/admin/v1/quality/oqc', { moduleKey: 'quality', deleteNeedsPassword: true, filters: QC.filter, fields: [{ key: 'code', label: '检验单号', required: true }, { key: 'inspected_qty', label: '检验数量', required: true, type: 'number' }, { key: 'result', label: '检验结果', required: true, type: 'select', options: [{ label: '合格', value: 'pass' }, { label: '不合格', value: 'reject' }] }] }) },
      { label: '不合格品', path: '/quality/nonconformity', cfg: res('不合格品', '/admin/v1/quality/nonconformity', { moduleKey: 'quality', deleteNeedsPassword: true, columns: [textCol('code', '编号', true), intCol('defect_qty', '缺陷数量'), statusCol(NC.dict)], fields: [{ key: 'code', label: '不合格品单号', required: true }, { key: 'defect_type', label: '缺陷类型', required: true }, { key: 'defect_qty', label: '缺陷数量', required: true, type: 'number' }] }) },
    ],
  },
];
