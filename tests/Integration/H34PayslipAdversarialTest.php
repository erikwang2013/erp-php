...
class H34PayslipAdversarialTest extends IntegrationTestCase
{
    ...
    private function createMirroredTables(): void
    {
        ...
        static::createTableIfMissing(self::T_SALARY, function (Blueprint $t): void {
            $t->unsignedBigInteger('id')->primary();
            $t->unsignedBigInteger('employee_id');
            $t->unsignedInteger('period_year');
            $t->unsignedTinyInteger('period_month');
            $t->decimal('base_salary', 10, 2)->default(0);
            $t->decimal('performance', 10, 2)->default(0);
            $t->decimal('piece_wage', 10, 2)->default(0);
            $t->decimal('overtime', 10, 2)->default(0);
            $t->decimal('deduction', 10, 2)->default(0);
            $t->decimal('tax', 10, 2)->default(0);
            $t->decimal('net_salary', 10, 2)->default(0);
            $t->unsignedTinyInteger('status')->default(0);
            $t->dateTime('created_at')->nullable();
            $t->dateTime('updated_at')->nullable();
            $t->index('employee_id', 'idx_employee_id');
            $t->index(['period_year', 'period_month'], 'idx_period');
        });
        static::createTableIfMissing(self::T_ITEM, function (Blueprint $t): void {
            $t->unsignedBigInteger('id')->primary();
            $t->string('code', 50);
            $t->string('name', 100);
            $t->unsignedTinyInteger('type')->default(1);
            $t->unsignedTinyInteger('is_taxable')->default(0);
    ...
    private function newSalaryRow(int $employeeId, array $amounts = []): int
    {
        ...
        Capsule::table(self::T_SALARY)->insert($row);
    ...
    private function newRatedRule(string $base): array
    {
        ...
        $this->ruleIds[] = $rule;
        $this->social()->bind($e = $this->newEmployee(), $rule, $base);
    ...
    {
        [$rule, $e] = $this->newRatedRule('5000.00');
        $salaryId = $this->newSalaryRow($e);
        $payload = $this->payslip()->view($salaryId);
        self::assertIsArray($payload);
        self::assertSame(['salary', 'items', 'social'], array_keys($payload));
        ...
        self::assertSame(2026, $header['period_year']);
        self::assertSame(8, $header['period_month']);
        self::assertSame($e, (int) $header['employee']['id']);
        self::assertSame('H34P-' . $e, $header['employee']['code']);
        $codes = ['base_salary', 'performance', 'piece_wage', 'overtime', 'deduction', 'tax', 'net_salary'];
        $types = [1, 1, 1, 1, 2, 2, 3];
        ...
        self::assertSame('3523.45', $payload['items'][0]['amount']);
        self::assertSame('0.00', $payload['items'][2]['amount']);
        self::assertSame('0.00', $payload['items'][5]['amount']);
        self::assertSame('4673.05', $payload['items'][6]['amount']);
        self::assertIsArray($payload['social'], '在职且已绑规则应返回社保补充 payload');
        self::assertSame($e, (int) $payload['social']['employee_id']);
        self::assertSame('5000.00', $payload['social']['base_amount']);
    }
    ...
    public function test_payslip_view_custom_item_name_overrides_default(): void
    {
        ...
        $salaryId = $this->newSalaryRow($e);
        $now = date('Y-m-d H:i:s');
        Capsule::table(self::T_ITEM)->insert(['id' => self::nextId(), 'code' => 'base_salary', 'name' => '底薪', 'type' => 1, 'is_taxable' => 0, 'default_amount' => 0, 'created_at' => $now, 'updated_at' => $now]);
        $this->itemCodes[] = 'base_salary';
        $items = $this->payslip()->view($salaryId)['items'];
        self::assertSame('底薪', $items[0]['name'], '定义表名称应覆盖默认文案');
        self::assertSame('绩效工资', $items[1]['name'], '无定义行 code 仍回退默认');
    }
    ...
    public function test_payslip_view_unbound_and_soft_deleted_employee_social_null(): void
    {
        ...
        self::assertNull($unbound['social'], '未绑定社保规则的员工 → social null');
        self::assertCount(7, $unbound['items'], '主体不受 social 缺失影响');
        Capsule::table(self::T_EMPLOYEE)->where('id', $e)->update(['deleted_at' => date('Y-m-d H:i:s')]);
        $soft = $this->payslip()->view($salaryId);
        self::assertNull($soft['social'], '软删除员工（SoftDeletes 排除）→ social null');
        self::assertNull($soft['salary']['employee'], 'with(employee) 应过滤软删除行');
        self::assertCount(7, $soft['items'], '工资条主体完整');
    }
    ...
    public function test_payslip_view_orphan_rule_keeps_body_intact(): void
    {
        ...
        self::assertNull($payload['social'], '孤儿绑定 → social null 而非异常');
        self::assertCount(7, $payload['items']);
        self::assertSame('3523.45', $payload['items'][0]['amount']);
    }
}
